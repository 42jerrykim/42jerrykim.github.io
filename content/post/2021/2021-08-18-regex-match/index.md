...
`Regex.Replace`에는 여러 오버로드가 있다. 그중 다음 시그니처는 <strong>각 매치마다 `Match` 객체를 인자로 받는 대리자(또는 람다)</strong>를 호출하고, 반환된 문자열로 치환한다.
...
정규식과 C# 문자열 리터럴 모두에서 <strong>역슬래시(`\`)와 따옴표(`"`)</strong>는 이스케이프가 필요하다.
