...
유닉스/리눅스에서 환경변수는 **키=값** 쌍이며, 프로세스 메모리와 **execve(2)** 인자 전달 형식상 <strong>널 문자(`\0`)</strong>로 구분된다. 공백·줄바꿈은 값 안에 들어갈 수 있으므로 구분자로 쓰이지 않는다. 따라서 `cat /proc/<pid>/environ`을 하면 한 줄로 이어져 보이고, 변수 경계를 구분하기 어렵다.
