...
- Jekyll 블로그에서 다이어그램을 오래 유지하고 수정할 계획이라면, <strong>방식 1(GitHub Action + .plantuml 파일)</strong>을 우선 적용하고, 필요 시 공식 가이드([PlantUML Guide](https://plantuml.com/guide), [generate-plantuml-action](https://github.com/grassedge/generate-plantuml-action))를 참고해 확장하는 것을 권한다.
