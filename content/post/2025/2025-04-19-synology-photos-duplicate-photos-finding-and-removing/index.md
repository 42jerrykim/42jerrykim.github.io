...
<strong>스택(Stack)</strong>이란 Synology Photos에서 서로 비슷한 사진들을 한 묶음으로 보여 주는 단위다. 완전히 동일한 파일뿐 아니라, 같은 장면을 조금 다르게 찍은 연사·유사 샷도 하나의 스택으로 묶인다. 스택에는 **대표 사진 한 장**이 타임라인에 보이고, 우측 상단에 **겹쳐진 사진 모양의 스택 아이콘**과 함께 스택에 포함된 **사진 개수**가 표시된다.
