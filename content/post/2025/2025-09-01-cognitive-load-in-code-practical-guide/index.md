...
코드의 <strong>인지 부하(Cognitive Load)</strong>를 낮추면 읽기·수정이 쉬워지고, 온보딩 시간이 줄고, 팀 전체 속도가 붙는다. 이 글에서는 이론부터 실전 체크리스트·아키텍처 원리·지표 활용까지 한 번에 정리한다.
...
- 중첩을 줄이고 <strong>조기 반환(guard clauses)</strong>으로 해피 패스를 위쪽에 둔다.
- 한 조건문에 AND/OR가 많이 붙으면, 이름 붙은 불리언으로 빼서 “무엇을 검사하는지”가 드러나게 한다.

### 2) 흐름 제어 평탄화

- 깊은 `if`–`else` 대신 <strong>빠른 실패(fail fast)</strong>를 쓰고, 예외·에러는 경계에서 한 번에 처리한다.
