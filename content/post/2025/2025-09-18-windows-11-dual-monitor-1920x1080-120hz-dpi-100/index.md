...
Windows 11에서 듀얼 모니터를 쓰며 **해상도 1920×1080**, **주사율 120Hz**, <strong>배율 100%(96 DPI)</strong>를 모두 동일하게 맞추고 싶을 때, 설정 앱을 일일이 열지 않고 한 번에 적용하는 PowerShell 스크립트를 정리했다. 외부 유틸 없이 `user32.dll`을 P/Invoke로 호출해 해상도·주사율을 적용하고, DPI는 레지스트리를 통해 100%로 설정한다.
