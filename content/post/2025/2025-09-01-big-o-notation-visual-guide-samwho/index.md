...
- **핵심**: Big O는 입력 크기에 따른 실행 시간 <strong>증가율(성장 차수)</strong>을 표현합니다. 벽시계 시간 자체보다 **입력과 시간의 관계**를 작게 기술하는 표기법입니다.
...
- **Big O의 목적**: 절대 시간이 아니라, 입력이 커질수록 시간이 <strong>어떻게 늘어나는지(성장률)</strong>를 표현합니다. 같은 작업이라도 구현 방식에 따라 성장 차수가 달라질 수 있습니다.
...
- **중복 계산**: 팩토리얼처럼 부분 문제가 반복되는 경우 <strong>메모이제이션(캐시)</strong>으로 평균 성능을 크게 개선할 수 있습니다.
