...
Jekyll로 블로그나 문서 사이트를 운영할 때, 본문 이미지에 <strong>캡션(설명 문구)</strong>을 달고 싶은 경우가 많다. 테마에 따라 기본 지원이 있기도 하고, 없으면 Markdown과 CSS만으로 직접 구현해야 한다. 이 포스트에서는 **Minimal Mistakes** 테마를 기준으로, 이미지 캡션을 넣는 **세 가지 방법**을 소개하고 각각의 장단점과 선택 기준을 정리한다.
