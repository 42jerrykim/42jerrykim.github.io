...
원문은 세 서비스(A, B, C)가 각각 두 개의 서브컴포넌트를 가진 예시 시스템 하나를 도구 비교와 critical path 설명 전체에 걸쳐 계속 사용한다. 요청은 Service A로 들어와 A1으로 넘어가고, A1은 A2·B1·B2에 의존하며, A2·B2·C2는 모두 C1을 호출한다. 아래 다이어그램은 원문 Figure 1의 화살표 관계를 그대로 재구성한 것이다(이미지 자체를 캡쳐한 것은 아니며, C2를 호출하는 상위 컴포넌트는 원문에 표시돼 있지 않다).
...
B2는 A1·A2·B1 세 곳 모두로부터 호출을 받는다 — 바로 다음 절에서 볼 "RPC 텔레메트리 한계"가 이 지점을 정확히 짚는다: A1과 A2가 (같은 Service A 소속임에도) 각각 독립적으로 B2에 요청을 보내는데, RPC 텔레메트리는 Service A→Service B 트래픽을 하나로 뭉뚱그려 집계하기 때문에 두 호출의 서로 다른 지연시간 특성을 구분하지 못한다.
...
1. **병렬성을 모른다.** 앞서 본 예시 시스템에서, A2가 CPU 위주 작업을 하면서 동시에 B2·C1·C2의 응답을 기다리고 있다고 하자. 이때 B2·C1·C2의 지연시간을 아무리 줄여도 전체 성능은 개선되지 않는다 — A2는 애초에 그 응답들 때문에 진행이 막혀 있는 게 아니기 때문이다. RPC 텔레메트리는 이 사실을 알려주지 않는다.
2. **반복 호출에 평균이 숨는다.** A2가 C1에 수백 번 요청을 보낸다고 하자. 그중 단 한 번의 느린 호출이 전체 지연의 원인일 수 있는데도 평균 지연시간은 여전히 "빠름"으로 나타난다. 게다가 이 수백 번의 요청이 병렬로 일어나는지 순차로 일어나는지도 RPC 텔레메트리는 구분하지 못한다.
3. **서비스 내부의 서브컴포넌트를 구분하지 못한다.** 위 다이어그램처럼 A1과 A2(둘 다 Service A 소속)가 각각 독립적으로 B2에 요청을 보내는 경우, Service A→Service B 트래픽 텔레메트리는 이 둘을 뭉뚱그려 집계해버린다. 두 호출의 지연시간 특성이 서로 다를 수 있는데도 말이다.
...
아래는 세 시나리오를 같은 시간 축 위에 놓고 비교한 것이다. 굵은 주황색 구간이 critical path, 회색·점선 구간은 critical path에서 제외되는 부분이다.

![병렬 실행 시나리오 (a) 순차, (b) B1·A2 병렬, (c) 부모·자식 노드 중첩을 같은 시간 축에서 비교한 타임라인](scenario-timeline.svg)

원문 Figure 2를 그대로 캡쳐한 것이 아니라, 본문 표에 정리된 수치(A1=5ms, B1=20ms 등)를 근거로 새로 구성한 개념도다.

**(a) 순차 실행 — 35ms.** A1이 B1을 완전히 기다린 뒤에야 A2를 호출하므로, 네 구간이 모두 한 줄로 이어져 critical path 전체를 이룬다.

**(b) B1·A2 병렬 실행 — 33ms.** A1이 B1과 A2를 동시에 호출하면, 둘 중 더 느린 B1만 critical path에 남는다. A2(2ms)는 B1(20ms)보다 먼저 끝나 다음 단계가 시작되는 시점에는 이미 완료돼 있으므로 critical path에서 완전히 제외된다. A2를 아무리 최적화해도 전체 지연시간에는 아무 영향이 없다—애초에 병목이 아니었다. 반면 A1과 B1을 최적화하면 실제로 응답 시간이 줄어든다. <strong>critical path 분석의 존재 이유는 바로 이 "최적화해도 소용없는 곳에 시간을 쓰지 않게 하는 것"</strong>이다.

**(c) 부모·자식 노드 중첩 실행 — 27ms.** A1이 B1에 RPC를 보낸 뒤 응답을 기다리지 않고 다른 연산을 계속하는 경우로, 부모(A1)와 자식(B1)이 겹쳐 실행된다. B1(14ms)이 A1의 병렬 연산보다 나중에 끝나므로 critical path에는 B1이 배정되고, A1의 병렬 연산(원문에 정확한 길이가 명시되지 않아 다이어그램에서는 점선으로 표시)은 제외된다.
...
flame graph는 critical path를 계층별 상자 열로 표현해, 어떤 서브컴포넌트가 critical path의 어느 구간을 차지하는지 한눈에 보여준다. 예컨대 시나리오 (b)에서 B1의 20ms 구간을 한 단계 더 파고들면, B1이 자체적으로 계산한 하위 critical path(원문 예시: B1=4ms, B2=12ms)를 이어붙인 아래와 같은 그림이 된다.

![critical path를 계층별 상자로 표현한 flame graph 개념도. 상위 행은 A1·B1·A1, 그 아래 B1 구간에 B1·B2·B1 기타로 세분된 하위 행이 중첩돼 있다](flame-graph-example.svg)

실제 Google 내부 도구가 생성하는 화면은 아니며, 원문이 설명하는 병합 예시(B1={B1=4ms, B2=12ms})를 바탕으로 재구성한 개념도다. 짙은 색일수록 critical path에서 차지하는 비중이 크다는 것을 나타낸다.
