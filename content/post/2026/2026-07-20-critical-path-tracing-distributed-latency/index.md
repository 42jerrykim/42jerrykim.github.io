...
원문은 세 서비스(A, B, C)가 각각 두 개의 서브컴포넌트를 가진 예시 시스템으로 이를 설명한다. 요청은 Service A로 들어와 A1으로 넘어가고, A1은 B1·B2·A2에 의존하며, A2·B2·C2는 모두 C1을 호출한다. 아래 다이어그램은 원문 Figure 1의 화살표 관계를 그대로 재구성한 것이다(이미지 자체를 캡쳐한 것은 아니며, C2를 호출하는 상위 컴포넌트는 원문에 표시돼 있지 않다).
