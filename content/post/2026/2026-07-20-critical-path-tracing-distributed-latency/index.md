...
원문은 세 서비스(A, B, C)가 각각 두 개의 서브컴포넌트를 가진 예시 시스템으로 이를 설명한다. A1이 B1과 A2에 의존하는 상황에서, 이 둘이 순차로 실행되는지 병렬로 실행되는지에 따라 critical path 자체가 달라진다.
...
시나리오 (b)가 핵심을 보여준다. A2를 아무리 최적화해도 전체 지연시간에는 아무 영향이 없다—A2는 B1보다 먼저 끝나기 때문에 애초에 병목이 아니었다. 반면 A1과 B1을 최적화하면 실제로 응답 시간이 줄어든다. **critical path 분석의 존재 이유는 바로 이 "최적화해도 소용없는 곳에 시간을 쓰지 않게 하는 것"**이다.

아래는 시나리오 (b)를 개념적으로 표현한 것이다.

```mermaid
graph LR
    Req(["요청 도착"]) --> A1a["A1 연산 5ms"]
    A1a --> B1c["B1 호출 20ms"]
    A1a --> A2c["A2 호출 2ms"]
    B1c --> A1b["A1 마무리 8ms"]
    A2c --> A1b
    A1b --> Resp(["응답 반환, critical path 총 33ms"])
```

`A2c`(2ms)는 `B1c`(20ms)보다 먼저 끝나 `A1b`가 시작되는 시점에는 이미 완료돼 있으므로, 이 경로는 critical path에서 제외된다.
