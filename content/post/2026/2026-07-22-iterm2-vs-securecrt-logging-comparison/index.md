...
네 옵션 중 실무에서 가장 자주 쓰이는 조합은 "평소엔 Plain text로 가볍게, 장애 재현이 필요할 때만 Raw data나 asciinema로 전환"하는 방식이다. Plain text는 파일 용량이 작고 grep·로그 수집기에 바로 넣기 편하지만, 커서 이동이나 화면 지우기 같은 제어 시퀀스가 사라지므로 vim·htop처럼 화면을 계속 다시 그리는 프로그램을 실행한 세션의 로그는 읽기 어려운 텍스트 덩어리가 된다. 이럴 때는 Raw data나 asciinema로 전환해 원본 화면 흐름을 그대로 보존하는 편이 문제 재현에 유리하다.
...
- iTerm2와 SecureCRT 각각에서 자동 로깅과 세션별 수동 로깅을 켜는 메뉴 경로를 헷갈리지 않고 설명할 수 있다.
- 로그를 재생·분석 도구에 넘길 목적인지, grep으로 검색할 목적인지, 색상 그대로 브라우저에서 볼 목적인지에 따라 iTerm2의 어느 포맷(Raw data/Plain text/HTML/asciinema)을 골라야 하는지 구분할 수 있다.
- SecureCRT의 `%` 치환 파라미터와 iTerm2의 Interpolated String이 같은 문제(파일명에 세션 정보 반영)를 서로 다른 문법으로 푼다는 점을 설명할 수 있다.
- "로그를 켜 두면 원본 화면과 100% 동일한 내용이 항상 남는다"는 오해가 왜 틀렸는지, 포맷별 차이를 근거로 반박할 수 있다.

흔한 오해 하나를 짚어두면, "로그 파일을 열어보면 터미널 화면에 보이던 색상이 그대로 남아 있을 것"이라는 기대는 포맷에 따라 다르다. iTerm2의 Plain text와 SecureCRT의 기본 텍스트 로그는 둘 다 제어 시퀀스(색상 코드 포함)를 남기지 않으므로, 화면에서 빨갛게 보이던 에러 메시지도 로그 파일에서는 색상 정보 없는 평문으로만 남는다. 색상까지 보존하려면 iTerm2에서는 HTML을, SecureCRT에서는 별도의 색상 보존 옵션 유무를 먼저 확인해야 한다.
