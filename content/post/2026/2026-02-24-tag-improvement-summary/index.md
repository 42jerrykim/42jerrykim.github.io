...
블로그의 <strong>모든 게시물(draft 제외)</strong>이 최소 50개 이상의 태그를 갖도록 통일한 개선 작업의 배경, 구현 전략, 변경 사항, 실행 결과, 사용 방법을 정리한다. 경로·카테고리·키워드 기반 추론만으로는 50개에 미달하는 글이 많았기 때문에, **컬렉션별 fallback 태그 풀**을 도입해 부족분을 자동으로 채우는 방식으로 849개 파일에 총 20,460개 태그를 추가했다.
