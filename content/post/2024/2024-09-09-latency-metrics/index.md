...
description: "지연 시간 메트릭(Latency Metrics) 핵심 정리. P90·P95·P99 백분위수, 평균·중앙값·최대 지연 시간 정의와 계산, SLA 설정, Prometheus·AWS CloudWatch 모니터링, 성능 병목 분석과 최적화 전략을 실무 관점에서 다룬다. API·백엔드 성능 평가와 사용자 경험 개선에 필요한 기초부터 활용까지 담았다."
...
지연 시간 메트릭(Latency Metrics)은 서비스·애플리케이션의 성능을 평가하는 핵심 지표다. P90, P95, P99를 관찰하면 잠재적 병목을 찾고 사용자 경험을 최적화할 수 있다. 배포 직전 “평균·중앙값·최대 지연 시간이 얼마인가?”, “P90·P95·P99는?”이라는 질문을 받는 상황을 막으려면, 평균(Mean)·중앙값(Median)·최대(Max)와 백분위수(P90, P95, P99)의 정의·계산·모니터링을 미리 익혀 두는 것이 좋다. 이 글에서는 지연 시간 메트릭의 개념, SLA(서비스 수준 계약), Prometheus·AWS 활용 모니터링, 성능 최적화 전략까지 실무에 바로 쓸 수 있도록 정리한다.

|![지연 시간 메트릭 개요](/assets/images/2024/2024-09-09-latency-metrics.png)|
|:---:|
|지연 시간 메트릭 개요|

### 이 글에서 다루는 내용 (목차)

- **개요** — Latency Metrics의 중요성, P90·P95·P99 정의와 역할
- **Latency Metrics 이해하기** — Latency 정의, 측정 단위, 사용자 경험과의 관계
- **SLA (Service Level Agreement)** — 정의, 중요성, 샘플 보고서
- **P90, P95, P99의 이해** — 각 백분위수 의미, PXX Latency 활용
- **Latency Metrics 계산 방법** — Mean, Median, Max, P99 vs Median
- **Latency Metrics 모니터링** — 도구·기술, Prometheus·PromQL, AWS CloudWatch
- **예제** — 계산 예제, SLA 예제, P90·P95·P99 예제
- **FAQ** — 고지연 대처, SLA 설정 시 고려사항, P90·P95·P99 차이
- **관련 기술** — API 성능 최적화, 모니터링·로깅 도구, 클라우드와 Latency
- **결론** — 요약 및 지속 모니터링 필요성
- **참고 문헌** — 블로그·문서·공식 자료
...
```mermaid
graph TD
    LatencyMetrics["Latency Metrics"] --> P90Node["P90"]
    LatencyMetrics --> P95Node["P95"]
    LatencyMetrics --> P99Node["P99"]
    P90Node --> E90["90% 요청 처리"]
    P95Node --> E95["95% 요청 처리"]
    P99Node --> E99["99% 요청 처리"]
```
...
```mermaid
graph TD
    Request["요청"] --> ServerProcess["서버 처리"]
    ServerProcess --> Response["응답"]
...
```mermaid
graph TD
    SLA["Service Level Agreement"] --> SvcDesc["Service Description"]
    SLA --> PerfMetrics["Performance Metrics"]
    SLA --> Responsibilities["Responsibilities"]
    SLA --> ReportingReview["Reporting and Review"]
    SvcDesc --> SvcType["Service Type"]
    SvcDesc --> SvcAvail["Service Availability"]
    PerfMetrics --> RespTime["Response Time"]
    PerfMetrics --> UptimePct["Uptime Percentage"]
    Responsibilities --> ProvResp["Provider Responsibilities"]
    Responsibilities --> CustResp["Customer Responsibilities"]
    ReportingReview --> MonthlyRpt["Monthly Reports"]
    ReportingReview --> QuarterlyRpt["Quarterly Reviews"]
```
...
```mermaid
graph TD
    TotalReqs["Total Requests"] -->|"90%"| P90Val["P90"]
    TotalReqs -->|"95%"| P95Val["P95"]
    TotalReqs -->|"99%"| P99Val["P99"]
    P90Val --> Desc90["90% of requests are below this latency"]
    P95Val --> Desc95["95% of requests are below this latency"]
    P99Val --> Desc99["99% of requests are below this latency"]
```
...
```mermaid
graph TD
    LatencyVals["Latency Values"] --> MeanL["Mean Latency"]
    LatencyVals --> MedianL["Median Latency"]
    LatencyVals --> MaxL["Max Latency"]
    LatencyVals --> P99L["P99 Latency"]
    MeanL --> AvgPerf["Average Performance"]
    MedianL --> TypPerf["Typical Performance"]
    MaxL --> WorstPerf["Worst Case Performance"]
    P99L --> HighPctPerf["High Percentile Performance"]
```
...
```mermaid
graph TD
    CW["CloudWatch"] --> MetricsColl["Metrics Collection"]
    CW --> DashboardCreate["Dashboard Creation"]
    CW --> AlertConfig["Alert Configuration"]
    MetricsColl --> LatencyM["Latency Metrics"]
    DashboardCreate --> VizLatency["Visualize Latency"]
    AlertConfig --> RecvAlerts["Receive Alerts"]
```
...
```mermaid
graph TD
    LatData["Latency Data"] --> MeanL2["Mean Latency"]
    LatData --> MedianL2["Median Latency"]
    LatData --> MaxL2["Max Latency"]
    LatData --> P90L["P90"]
    LatData --> P95L["P95"]
    LatData --> P99L2["P99"]
```
...
```mermaid
graph TD
    TotalReqs2["Total Requests"] -->|"100%"| P90Faq["P90"]
    TotalReqs2 -->|"95%"| P95Faq["P95"]
    TotalReqs2 -->|"99%"| P99Faq["P99"]
    P90Faq -->|"10%"| AboveP90["Above P90"]
    P95Faq -->|"5%"| AboveP95["Above P95"]
    P99Faq -->|"1%"| AboveP99["Above P99"]
```
...
```mermaid
graph TD
    ApiRequest["API 요청"] -->|"비동기 처리"| MsgQueue["메시지 큐"]
    MsgQueue --> BgJob["백그라운드 작업"]
    BgJob --> ResultReturn["결과 반환"]
```
...
```mermaid
graph TD
    LatencyM2["Latency Metrics"] --> MonTools["Monitoring Tools"]
    MonTools --> RealtimeAnalysis["Real-time Analysis"]
    RealtimeAnalysis --> PerfOpt["Performance Optimization"]
    PerfOpt --> ImprovedUX["Improved User Experience"]
```
...
```mermaid
graph TD
    LatencyM3["Latency Metrics"] --> P90Ref["P90"]
    LatencyM3 --> P95Ref["P95"]
    LatencyM3 --> P99Ref["P99"]
    P90Ref --> UXImprove["사용자 경험 개선"]
    P95Ref --> SLASetup["SLA 설정"]
    P99Ref --> MonToolsUse["모니터링 도구 활용"]
```
...
1. [Mastering Latency Metrics: P90, P95, P99 (Medium, Javarevisited)](https://medium.com/javarevisited/mastering-latency-metrics-p90-p95-p99-d5427faea879) — Anil Gudigar, API·백분위수·SLA 개요.
2. [P90, P95, P99 백분위수 (yonghyuc.wordpress.com)](https://yonghyuc.wordpress.com/2020/10/09/p90-p95-p99-%EB%B0%B1%EB%B6%84%EC%9C%84%EC%88%98/) — 백분위수 정의와 AWS 메트릭 활용.
3. [인스턴스 지표 - AWS Elastic Beanstalk (공식 문서)](https://docs.aws.amazon.com/ko_kr/elasticbeanstalk/latest/dg/health-enhanced-metrics.html) — p90, p95, p99 등 웹 서버·OS 지표 설명.
4. [Understanding P90, P95, and P99 (Medium, YoHoSo)](https://medium.com/@yohoso/understanding-p90-p95-and-p99-dd4d7424ec0d) — P90·P95·P99 개념 요약.
