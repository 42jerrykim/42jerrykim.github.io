...
정규식(정규 표현식, Regular Expression)은 문자열에서 **특정 패턴을 찾고, 검증하고, 치환**하는 데 쓰이는 강력한 도구다. Python에서는 표준 라이브러리 `re` 모듈로 정규식을 지원하며, `match`, `search`, `findall`, `finditer`, `sub`, `split` 등으로 문자열 검색·치환·분할을 할 수 있다. 이 글에서는 **메타 문자·특수 시퀀스**부터 **re 모듈 기본 함수·컴파일·그룹·전방·후방 탐색**, **그리디/비그리디·성능·Raw String·가독성**까지 기초부터 실전 활용·주의사항을 예제와 함께 다룬다.

**이 글에서 다루는 내용**
- **개요**: 정규 표현식 정의, 활용 분야(데이터 검증·텍스트 검색·치환·웹 스크래핑)
- **기초**: 문자 클래스 `[]`, Dot `.`, 반복 `*` `+` `?` `{m,n}`, `^` `$`, `\b` `\B`, 특수 시퀀스 `\d` `\w` `\s` 등
- **활용**: `re` 모듈 함수(match/search/findall/finditer/sub), 컴파일, 이메일·전화번호 예제, 그룹·역참조·전방·후방 탐색
- **최적화**: 그리디 vs 비그리디, 성능 고려사항
- **실무 주의사항**: 문자열 메서드와의 선택, 백슬래시·Raw String, 가독성(re.VERBOSE)
- **FAQ·관련 기술·참고 자료**
...
graph TD
    ...
    WebScraping["웹 스크래핑"]
    RegexExpr --> DataValidation
    RegexExpr --> TextSearch
    RegexExpr --> DataTransform
    RegexExpr --> WebScraping
```
...
graph TD
    ...
    WebScrape["웹 스크래핑"]
    LogAnalysis["로그 분석"]
    RegexSummary --> StrSearch
    RegexSummary --> StrReplace
    RegexSummary --> DataValid
    RegexSummary --> WebScrape
    RegexSummary --> LogAnalysis
```
...
```mermaid
graph TD
    RegexResources["정규 표현식 자료"]
    Books["서적"]
    Online["온라인 자료"]
    PyDocs["Python 공식 문서"]
    Practice["연습 사이트"]
    Book1["정규 표현식 완벽 가이드"]
    Book2["Mastering Regular Expressions"]
    RegexResources --> Books
    RegexResources --> Online
    RegexResources --> PyDocs
    RegexResources --> Practice
    Books --> Book1
    Books --> Book2
    Online --> Regex101["Regex101"]
    Online --> Regexr["Regexr"]
    PyDocs --> ReModule["Python re 모듈 공식 문서"]
    PyDocs --> RealPy["Real Python - Regular Expressions"]
    Practice --> RegexOne["RegexOne"]
    Practice --> RegExr["RegExr"]
```
