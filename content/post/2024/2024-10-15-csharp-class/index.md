...
lastmod: 2026-03-17
title: "[CSharp] 클래스 소개"
description: "C# 클래스의 정의·참조 형식·선언·생성자·상속·캡슐화·다형성·Partial 클래스까지, 실무 코드와 Mermaid 다이어그램으로 정리한 객체지향 핵심 가이드. 참조/값 형식 차이, 가비지 수집, 액세스 한정자, required 한정자, abstract·sealed·인터페이스 구현을 다루며 OOP 입문자와 .NET 개발자 참고용."
categories: CSharp
draft: false
...
```mermaid
graph TD
    ClassType["클래스"] -->|"참조 형식"| RefObj["객체"]
    ClassType -->|"값 형식"| StructType["구조체"]
    RefObj --> MemAddr["메모리 주소"]
    StructType --> ActualData["실제 데이터"]
```
...
```mermaid
graph TD
    ObjCreate["객체 생성"] --> RefKeep["참조 유지"]
    ObjCreate --> RefRelease["참조 해제"]
    RefRelease --> GCTarget["가비지 수집 대상"]
    GCTarget --> MemFree["메모리 해제"]
```
...
```mermaid
graph TD
    CarClass["Car Class"] -->|"new"| MyCarObj["myCar Object"]
    MyCarObj -->|"Reference"| MemLoc["Memory Location"]
```
...
```mermaid
graph TD
    SampleClass["SampleClass"] --> File1["File1.cs"]
    SampleClass --> File2["File2.cs"]
    File1 --> MethodA["MethodA"]
    File2 --> MethodB["MethodB"]
```
...
- **Microsoft 공식 문서**: Microsoft의 공식 웹사이트에서는 C# 언어 사양을 포함한 다양한 자료를 제공한다. 이곳은 C#에 대한 가장 권위 있는 정보 출처 중 하나이다. [C# 언어 사양(읽기)](https://learn.microsoft.com/ko-kr/dotnet/csharp/language-reference/language-specification/readme)
...
```mermaid
graph LR
    LangSpec["C# Language Specification"] --> Syntax["Syntax"]
    LangSpec --> Semantics["Semantics"]
    LangSpec --> Types["Types"]
    LangSpec --> Expressions["Expressions"]
    LangSpec --> Statements["Statements"]
    LangSpec --> ClassesStructs["Classes and Structs"]
    LangSpec --> Interfaces["Interfaces"]
    LangSpec --> Delegates["Delegates"]
    LangSpec --> Events["Events"]
```
...
classDiagram
    ...
    ClassBlueprint <|-- ObjectInstance : "creates"
```
...
- [C# 프로그래밍 가이드](https://learn.microsoft.com/ko-kr/dotnet/csharp/programming-guide/) — Microsoft 공식 C# 가이드
- [C# 형식 시스템 - 클래스](https://learn.microsoft.com/ko-kr/dotnet/csharp/fundamentals/types/classes) — 참조 형식·개체 만들기·생성자·상속 요약
- [상속(C#)](https://learn.microsoft.com/ko-kr/dotnet/csharp/fundamentals/object-oriented/inheritance) — 기본 클래스와 파생 클래스

이 외에도 [.NET 문서](https://learn.microsoft.com/ko-kr/dotnet/)에서 C# 및 클래스 관련 주제를 검색해 볼 수 있다.

### 추천 도서

- **C# in a Nutshell** — C# 언어 전반과 클래스·객체 지향을 다루는 참고서.
- **Head First Object-Oriented Analysis and Design** — 객체 지향 설계 기초와 클래스 개념 정리.
...
1. [C# 형식 시스템 - 클래스](https://learn.microsoft.com/ko-kr/dotnet/csharp/fundamentals/types/classes) — Microsoft Learn, 참조 형식·클래스 선언·개체 만들기·생성자·상속 요약.
2. [상속 - C#](https://learn.microsoft.com/ko-kr/dotnet/csharp/fundamentals/object-oriented/inheritance) — 기본 클래스·파생 클래스·추상·가상·sealed.
3. [C# 언어 사양(읽기)](https://learn.microsoft.com/ko-kr/dotnet/csharp/language-reference/language-specification/readme) — C# 구문·의미론 공식 사양.
