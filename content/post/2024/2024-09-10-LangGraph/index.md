...
1. [개요](#개요) — LangGraph 소개, LangChain과의 관계, 주요 기능 및 이점  
2. [LangGraph의 기본 개념](#langgraph의-기본-개념) — 노드·엣지, 상태 관리, 그래프 구조  
3. [LangGraph 설치 및 설정](#langgraph-설치-및-설정) — 설치 방법, 환경 설정  
4. [LangGraph 활용 예제](#langgraph-활용-예제) — 단일/멀티 에이전트, Human-in-the-Loop  
5. [LangGraph의 고급 기능](#langgraph의-고급-기능) — 순환 그래프, 상태 지속성·오류 복구, 스트리밍  
6. [LangGraph와 CrewAI 비교](#langgraph와-crewai-비교) — 사용 용이성, 기능·커스터마이징, 성능·안정성  
7. [FAQ](#faq) — 주요 사용 사례, 사전 지식, 성능 최적화  
8. [관련 기술](#관련-기술) — LangChain, LLM, DAG  
9. [결론](#결론) — 장단점 요약, 활용 방향  
10. [참고 자료](#reference) — 공식 문서, 아티클, 블로그
...
```mermaid
graph TD
    userInput["사용자 입력"] --> agentProcess["에이전트 처리"]
    agentProcess --> resultCheck{"결과 판단"}
    resultCheck -->|"성공"| resultOutput["결과 출력"]
    resultCheck -->|"실패"| errorProcess["오류 처리"]
```
...
```mermaid
graph TD
    nodeA["노드 A"] -->|"엣지 1"| nodeB["노드 B"]
    nodeA -->|"엣지 2"| nodeC["노드 C"]
    nodeB -->|"엣지 3"| nodeD["노드 D"]
```
...
```mermaid
graph TD
    startNode["Start"] --> installNode["Install LangGraph"]
    installNode --> envNode["Set Environment Variables"]
    envNode --> createNode["Create Nodes and Edges"]
    createNode --> runNode["Run Your Application"]
    runNode --> endNode["End"]
```
...
```mermaid
graph TD
    langGraph["LangGraph"] --> lgEase["사용 용이성"]
    langGraph --> lgFunc["기능 및 커스터마이징"]
    langGraph --> lgPerf["성능 및 안정성"]
    crewAI["CrewAI"] --> crEase["사용 용이성"]
    crewAI --> crFunc["기능 및 커스터마이징"]
    crewAI --> crPerf["성능 및 안정성"]
    lgEase -->|"우수"| crEase
    lgFunc -->|"유연성"| crFunc
    lgPerf -->|"신뢰성"| crPerf
```
...
```mermaid
graph TD
    dataAnalysis["데이터 분석"] --> sqlExec["SQL 쿼리 실행"]
    dataAnalysis --> resultViz["결과 시각화"]
    nlp["자연어 처리"] --> questionInput["질문 입력"]
    nlp --> answerGen["답변 생성"]
    processAuto["프로세스 자동화"] --> supportSys["고객 지원 시스템"]
```
...
```mermaid
graph TD
    nodeEdgeOpt["노드 및 엣지 최적화"] --> dedupNode["중복 노드 제거"]
    nodeEdgeOpt --> edgeMin["엣지 수 최소화"]
    stateMgmt["상태 관리 개선"] --> effSave["효율적 저장"]
    stateMgmt --> fastLoad["빠른 불러오기"]
    asyncProc["비동기 처리"] --> concurrentTask["동시 작업 처리"]
```
...
```mermaid
graph TD
    langGraphNode["LangGraph"] --> langChainNode["LangChain"]
    langGraphNode --> llmNode["LLM"]
    langGraphNode --> dagNode["DAG"]
    langChainNode --> llmNode
    llmNode --> dagNode
```
...
```mermaid
graph TD
    userReq["사용자 요청"] --> agentOne["에이전트 1"]
    userReq --> agentTwo["에이전트 2"]
    agentOne --> dbQuery["데이터베이스 쿼리"]
    agentTwo --> apiCall["외부 API 호출"]
    dbQuery --> resultReturn["결과 반환"]
    apiCall --> resultReturn
    resultReturn --> finalResp["최종 응답"]
```
...
- [LangGraph 공식 문서](https://langchain-ai.github.io/langgraph/) — LangGraph 개요, 설치, 핵심 개념, 배포·디버깅
- [From Basics to Advanced: Exploring LangGraph (Towards Data Science)](https://towardsdatascience.com/from-basics-to-advanced-exploring-langgraph-e8c1cf4db787) — 단일·멀티 에이전트, Human-in-the-Loop 실습
- [LangGraph Retrieval Agent를 활용한 동적 문서 검색 (테디노트)](https://teddylee777.github.io/langgraph/langgraph-agentic-rag/) — Agentic RAG, StateGraph·노드·엣지 구현
- [LangGraph 개념과 간단한 예시 (All I Need Is Data)](https://data-newbie.tistory.com/997) — 그래프 구조·상태 관리·간단한 워크플로 예제
- [LangGraph 1. 개요 (normalstory)](https://normalstory.tistory.com/entry/LangGraph-01) — StateGraph·Nodes·Edges·컴파일 개요
