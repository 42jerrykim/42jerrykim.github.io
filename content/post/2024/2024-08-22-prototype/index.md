...
프로토타입 패턴은 <strong>기존 객체를 복제하여 새로운 객체를 만드는 생성 패턴(Creational Pattern)</strong>이다. 객체 생성 비용이 크거나, 복잡한 초기화를 피하고 싶거나, 런타임에 객체 타입을 결정해야 할 때 유용하다. 이 글에서는 정의·의도·문제·적용 조건·구현·예제·FAQ·관련 패턴·참고 문헌까지 체계적으로 다룬다.
