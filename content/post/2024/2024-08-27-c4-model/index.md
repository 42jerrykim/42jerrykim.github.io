...
**언제 C4 모델을 쓰고 언제 보완할지**를 정리하면 아래와 같다. **사용을 권장하는 경우**: 팀·이해관계자와 아키텍처를 공유할 때, 온보딩·문서화·애자일 환경에서 경량 문서가 필요할 때, 도구·노테이션에 구애받지 않는 공통 언어가 필요할 때. **보완이 필요한 경우**: 런타임 동작·배포 인프라·데이터 스키마를 상세히 표현할 때는 C4의 동적·배포 다이어그램 또는 UML 시퀀스·ERD를 함께 사용하는 것이 좋다. <strong>코드 다이어그램(레벨 4)</strong>은 최신화 부담이 크므로 실무에서는 IDE 역공학이나 필요 시에만 사용하는 것이 일반적이다.
