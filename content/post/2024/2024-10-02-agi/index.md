...
graph TD
    AgiNode["AGI"]
    VariousTasks["다양한 작업 수행"]
    CreativeSolve["창의적 문제 해결"]
    NarrowAi["Narrow AI"]
    LimitedFunc["제한된 기능"]
    DomainOpt["특정 분야 최적화"]
    AgiNode -->|"범용성"| VariousTasks
    AgiNode -->|"인간 유사 지능"| CreativeSolve
    NarrowAi -->|"특정 작업"| LimitedFunc
    NarrowAi -->|"전문성"| DomainOpt
```
...
graph TD
    ...
    DataAnalyze --> PatternRec
    PatternRec -->|"예측"| ResultOut
    PatternRec -->|"결론"| Decision
```
...
```mermaid
graph TD
    LlmNode["대규모 언어 모델(LLMs)"]
    Nlu["자연어 이해"]
    Nlg["자연어 생성"]
    AgiLang["AGI의 언어적 능력 향상"]
    LlmNode --> Nlu
    LlmNode --> Nlg
    Nlu --> AgiLang
    Nlg --> AgiLang
...
graph TD
    ...
    CustomSol["맞춤형 솔루션 제공"]
    MarketPred["시장 트렌드 예측"]
    StrategyDec["전략적 결정 지원"]
    AgiUse --> CustSvc
    AgiUse --> DataAna
    AgiUse --> DecisionSupport
    CustSvc --> CustomSol
    DataAna --> MarketPred
    DecisionSupport --> StrategyDec
```
...
graph TD
    ...
    ResearchSupport["의료 연구 지원"]
    PatientData["환자 데이터 분석"]
    PersonalRx["개인 맞춤형 치료"]
    NewFindings["새로운 발견"]
    AgiMed --> DiagSupport
    AgiMed --> TreatmentPlan
    AgiMed --> ResearchSupport
    DiagSupport --> PatientData
    TreatmentPlan --> PersonalRx
    ResearchSupport --> NewFindings
```
...
graph TD
    AgiEdu["AGI 활용"]
    PersonalLearn["개인 맞춤형 학습"]
    ContentGen["교육 콘텐츠 생성"]
    RealtimeQ["실시간 질문 응답"]
    StyleAnalyze["학습 스타일 분석"]
    AutoContent["자동 콘텐츠 생성"]
    EffectMax["학습 효과 극대화"]
    AgiEdu --> PersonalLearn
    AgiEdu --> ContentGen
    AgiEdu --> RealtimeQ
    PersonalLearn --> StyleAnalyze
    ContentGen --> AutoContent
    RealtimeQ --> EffectMax
```
...
```mermaid
graph TD
    EthicsUse["AI의 윤리적 사용"]
    Fairness["공정성"]
    Transparency["투명성"]
    BiasMin["편향 최소화"]
    Explainability["결정 과정의 설명 가능성"]
    EthicsUse --> Fairness
    EthicsUse --> Transparency
    EthicsUse --> BiasMin
...
graph TD
    ...
    SocialPrep["사회적 준비"]
    Retrain["재교육 프로그램 필요"]
    JobChange --> Automation
    JobChange --> NewJobs
    JobChange --> SocialPrep
    Automation --> Retrain
```
...
graph TD
    ...
    RiskMin["위험 최소화"]
    RegNeed["규제 필요"]
    Guidelines["명확한 가이드라인"]
    SocialBenefit["사회적 혜택"]
    SafetyNode --> RiskMin
    SafetyNode --> RegNeed
    RegNeed --> Guidelines
    Guidelines --> SocialBenefit
```
...
```mermaid
graph TD
    InnovChange["AGI의 혁신적 변화"]
    BizEff["비즈니스 효율성 증가"]
    MedDiag["의료 진단 개선"]
    CustomEdu["개인 맞춤형 교육"]
    InnovChange --> BizEff
    InnovChange --> MedDiag
    InnovChange --> CustomEdu
```
...
graph TD
    ...
    DataGather["데이터 수집"]
    ModelTrain["모델 훈련"]
    ModelDeploy["AGI 모델 배포"]
    FeedbackCollect["사용자 피드백 수집"]
    ModelImprove["모델 개선"]
    ProjStart --> DataGather
    DataGather --> ModelTrain
    ModelTrain --> ModelDeploy
    ModelDeploy --> FeedbackCollect
    FeedbackCollect --> ModelImprove
```
...
graph TD
    ...
    LearnNew["Learning New Skills"]
    AiRoot -->|"Narrow AI"| SpecificTasks
    AiRoot -->|"AGI"| GeneralTasks
    GeneralTasks --> LearnNew
```
...
graph TD
    ...
    JobShift["일자리 변화"]
    SocialImpact["사회적 영향"]
    ResearchNode --> TechProgress
    ResearchNode --> EthicsConsider
    ResearchNode --> SocialAccept
    TechProgress --> ConversationalAi
    TechProgress --> ReasoningAi
    TechProgress --> AutonomousAi
    EthicsConsider --> AiSafety
    EthicsConsider --> AiRegulation
    SocialAccept --> JobShift
    SocialAccept --> SocialImpact
```
...
graph TD
    ...
    Books["서적"]
    Websites["웹사이트"]
    SurveyPaper["Artificial General Intelligence: A Survey"]
    EthicsPaper["The Ethics of Artificial Intelligence"]
    DeepReview["Deep Learning for AGI: A Review"]
    BostromBook["Superintelligence: Paths, Dangers, Strategies"]
    GoertzelBook["Artificial General Intelligence"]
    AgiSociety["AGI Society"]
    OpenAiSite["OpenAI"]
    AgiResearch --> Papers
    AgiResearch --> Books
    AgiResearch --> Websites
    Papers --> SurveyPaper
    Papers --> EthicsPaper
    Papers --> DeepReview
    Books --> BostromBook
...
1. **OpenAI** — [OpenAI 공식 사이트](https://www.openai.com): AGI 연구·제품·논문 소개.
2. **Google DeepMind** — [Levels of AGI for Operationalizing Progress on the Path to AGI](https://deepmind.google/research/publications/66938/): AGI 단계 분류 프레임워크(성능·일반성·자율성), 벤치마크 요구사항.
3. **연합뉴스** — [오픈AI "현재 AI기술, AGI로 가는 5단계 중 2단계 직전 수준"](https://www.yna.co.kr/view/AKR20240712054300009): OpenAI 5단계 분류체계 보도(2024.07).
4. **Nick Bostrom** — *Superintelligence: Paths, Dangers, Strategies*: AGI·초지능 위험과 전략.
5. **Ben Goertzel & Cassio Pennachin** — *Artificial General Intelligence*: AGI 이론과 접근법.
