---
description: "RCU(Read-Copy-Update)는 Linux 커널의 동기화 메커니즘으로, 읽기 다수·쓰기 소수 환경에서 락 없이 안전한 동시 접근을 제공한다. 제거·회수 단계 분리, 핵심 API·활용 예제, Reader-Writer Lock·Reference Counting과의 비교, 사용 시 주의사항과 참고 문헌을 정리했다."
date: "2024-11-27T09:22:56+09:00"
lastmod: "2026-03-17"
title: "[Linux] RCU(Read-Copy-Update) 개념·API·활용 정리"
categories:
  - Linux
  - Kernel
  - Concurrency
...
```mermaid
graph TD
    ReaderNode[Reader] -->|Read| RcuData[RCU Data Structure]
    RcuData -->|Copy| UpdatedData[Updated Data]
    UpdatedData -->|Synchronize| WriterNode[Writer]
    WriterNode -->|Update| RcuData
```
...
```mermaid
graph TD
    RcuReadLock["rcu_read_lock()"] --> ReadPhase[읽기 작업]
    ReadPhase --> CopyPhase[데이터 복사]
    CopyPhase --> AssignPtr["rcu_assign_pointer()"]
    AssignPtr --> UpdateDone[업데이트 완료]
    UpdateDone --> SyncRcu["synchronize_rcu()"]
    SyncRcu --> FreeOld[이전 데이터 해제]
```
...
```mermaid
graph TD
    RcuLock["rcu_read_lock()"] --> DataRead[데이터 읽기]
    DataRead --> RcuUnlock["rcu_read_unlock()"]
    RcuUnlock --> SyncRcuApi["synchronize_rcu()"]
    SyncRcuApi --> CallRcu["call_rcu()"]
    CallRcu --> AssignPointer["rcu_assign_pointer()"]
    AssignPointer --> DerefRcu["rcu_dereference()"]
```
...
```mermaid
graph TD
    RcuRead[RCU Read] -->|"Read Data"| MyStruct[my_struct]
    MyStruct -->|"Update Data"| NewStruct[New my_struct]
    NewStruct -->|"Assign Pointer"| RcuPointer[RCU Pointer]
    RcuPointer -->|Synchronize| FreeOldStruct[Free Old Struct]
```
...
```mermaid
graph TD
    HeadNode[Head Node] -->|"Add Node"| NewNode[New Node]
    NewNode -->|"Next Pointer"| NextNode[Next Node]
    NextNode -->|"Remove Node"| NodeToRemove[Node to Remove]
    NodeToRemove -->|"Call RCU"| FreeNode[Free Node]
```
...
```mermaid
graph TD
    NmiHandler[NMI Handler] -->|"Read Data"| NmiData[NMI Data]
    NmiData -->|"Update Data"| NewNmiData[New NMI Data]
    NewNmiData -->|"Assign Pointer"| RcuPtrNmi[RCU Pointer]
    RcuPtrNmi -->|Synchronize| FreeOldData[Free Old Data]
```
...
```mermaid
graph TD
    UpdaterThread[업데이트 스레드] -->|"데이터 업데이트"| CallRcuInvoke["call_rcu() 호출"]
    CallRcuInvoke --> RcuSyncWait[RCU 동기화 대기]
    RcuSyncWait -->|"동기화 완료"| CallbackInvoke[콜백 호출]
    CallbackInvoke --> MemFree[메모리 해제]
```
...
```mermaid
graph TD
    ReadWork[읽기 작업] -->|"락 없이"| DataAccess[데이터 접근]
    WriteWork[쓰기 작업] -->|"데이터 복사"| NewData[새로운 데이터]
    NewData -->|교체| UpdateComplete[업데이트 완료]
    UpdateComplete -->|"비동기 해제"| FreePrev[이전 데이터 해제]
```
...
```mermaid
graph TD
    RwLock[Reader-Writer Lock] -->|읽기| ReadAllowed[읽기 작업 허용]
    RwLock -->|쓰기| AllBlocked[모든 작업 차단]
    RcuCompare[RCU] -->|읽기| RcuReadAllowed[읽기 작업 허용]
    RcuCompare -->|쓰기| WriteDeferred[쓰기 작업 지연]
```
...
```mermaid
graph TD
    ReadOp[Read Operation] -->|"rcu_read_lock"| AccessData[Access Data]
    AccessData --> RcuUnlockEdge[rcu_read_unlock]
    UpdateOp[Update Operation] -->|call_rcu| FreeData[Free Data]
    UpdateOp -->|"rcu_assign_pointer"| UpdatePtr[Update Pointer]
    UpdatePtr --> NewDataNode[New Data]
    NewDataNode -->|rcu_head| FreeData
```
...
```mermaid
graph TD
    RcuApiRoot[RCU API] --> ReadApi[읽기 API]
    RcuApiRoot --> WriteApi[쓰기 API]
    RcuApiRoot --> UtilApi[유틸리티 API]
    ReadApi --> RcuDeref["rcu_dereference()"]
    ReadApi --> RcuLockUnlock["rcu_read_lock()/rcu_read_unlock()"]
    WriteApi --> RcuAssign["rcu_assign_pointer()"]
    WriteApi --> CallRcuApi["call_rcu()"]
    WriteApi --> SyncRcuApi["synchronize_rcu()"]
```
...
```mermaid
graph TD
    RcuUsage[RCU 사용] --> ReadOpt[읽기 작업 최적화]
    RcuUsage --> AsyncFree[비동기 메모리 해제]
    RcuUsage --> UpdatePattern[업데이트 패턴 분석]
    ReadOpt --> UseRcuLock["rcu_read_lock() 사용"]
    ReadOpt --> UseRcuUnlock["rcu_read_unlock() 사용"]
    AsyncFree --> UseSyncRcu["synchronize_rcu() 사용"]
    UpdatePattern --> ReduceUpdate[불필요한 업데이트 줄이기]
```
...
```mermaid
graph TD
    RcuUseCase[RCU 사용 사례] --> KernelProtect[커널 모듈 데이터 보호]
    RcuUseCase --> FastCache[고속 캐시 시스템]
    RcuUseCase --> MultiThread[대규모 멀티스레드 애플리케이션]
    KernelProtect --> ReadFrequent[읽기 작업 빈번]
    FastCache --> WriteRare[쓰기 작업 드문]
    MultiThread --> PerfOpt[성능 최적화]
```
...
```mermaid
graph TD
    RcuFaq[RCU] -->|"높은 동시성"| ReadWorkFaq[읽기 작업]
    RcuFaq -->|"지연된 업데이트"| WriteWorkFaq[쓰기 작업]
    LockBased[Lock-based] -->|"동시성 저하"| ReadLock[읽기 작업]
    LockBased -->|"즉각적인 업데이트"| WriteLock[쓰기 작업]
```
...
```mermaid
graph TD
    MainTh[Main Thread] -->|Create| Worker1[Worker Thread 1]
    MainTh -->|Create| Worker2[Worker Thread 2]
    Worker1 -->|"Read Data"| SharedData[Shared Data]
    Worker2 -->|"Read Data"| SharedData
    Worker1 -->|"Update Data"| SharedData
    Worker2 -->|"Update Data"| SharedData
```
...
```mermaid
graph TD
    UserSpace[User Space] -->|"System Call"| KernelSpace[Kernel Space]
    KernelSpace -->|Lock| MutexNode[Mutex]
    KernelSpace -->|Lock| SpinlockNode[Spinlock]
    KernelSpace -->|RCU| RcuSync[Read-Copy-Update]
    MutexNode -->|"Access Data"| SharedRes[Shared Resource]
    SpinlockNode -->|"Access Data"| SharedRes
    RcuSync -->|"Access Data"| SharedRes
```
...
```mermaid
graph TD
    ReadOpConclusion[Read Operation] -->|Non-blocking| RcuReadLockNode[RCU Read Lock]
    RcuReadLockNode --> AccessDataNode[Access Data]
    AccessDataNode --> RcuReadUnlockNode[RCU Read Unlock]
    UpdateOpConclusion[Update Operation] -->|Deferred| RcuUpdateNode[RCU Update]
    RcuUpdateNode --> DataStructUpdate[Data Structure Update]
```
...
| 항목 | 내용 |
|------|------|
| **정의** | 읽기 다수·쓰기 소수 환경에서 락 없이 안전한 동시 접근을 제공하는 Linux 커널 동기화 메커니즘 |
| **핵심 원리** | 업데이트를 제거(removal) 단계와 회수(reclamation) 단계로 분리하여, 독자와 동시에 제거를 수행하고 회수는 grace period 이후로 지연 |
| **핵심 API** | `rcu_read_lock()` / `rcu_read_unlock()`, `rcu_dereference()`, `rcu_assign_pointer()`, `synchronize_rcu()` / `call_rcu()` |
| **적합한 경우** | 읽기 빈번·쓰기 드문 데이터 구조, 커널 모듈·캐시·라우팅 테이블 등 |
| **주의** | RCU 읽기 크리티컬 섹션 내 블로킹 금지, `synchronize_rcu()` 오용 시 성능 저하, 참조는 크리티컬 섹션 밖으로 빼면 안 됨 |
...
- [What is RCU? Part 2: Usage (LWN.net)](https://lwn.net/Articles/263130/)
