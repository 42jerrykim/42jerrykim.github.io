...
배열은 동일한 데이터 타입의 요소들을 연속적으로 저장하는 자료구조로, 인덱스를 통해 개별 요소에 접근할 수 있다. C# 배열은 0부터 시작하는(zero-based) 인덱스를 가지며, `A[0]`처럼 대괄호 안에 인덱스를 넣어 접근한다. 1차원부터 최대 32차원까지 지원하며, 다차원 배열은 **Rectangular 배열**(각 차원 크기 고정)과 **가변 배열(Jagged Array)**(행마다 길이 상이)로 구분된다. 선언은 `자료형[] 배열명 = new 자료형[크기]` 형식을 사용하고, 초기화 시 중괄호로 값을 직접 넣을 수 있으며, `System.Array`를 통해 정렬·복사·검색 등 다양한 메서드를 쓸 수 있다.

**이 포스트에서 다루는 내용:** 배열의 정의와 인덱스·차원, 1차원·다차원·가변 배열의 종류와 선언·초기화, 요소 접근·길이·메서드 전달·복사·클론, System.Array 메서드·속성, 실전 예제(성적 관리·이미지·가변 배열·게임), FAQ, LINQ·컬렉션·메모리·다른 언어와의 비교, 결론 및 참고 문헌.
...
C#에서 배열은 차원 수와 메모리 레이아웃에 따라 다음과 같이 구분된다.

```mermaid
flowchart LR
  subgraph CSharpArray["C# 배열 종류"]
    OneDim["1차원 배열</br>int 배열"]
    MultiDim["다차원 배열"]
    Rect["Rectangular</br>고정 행 열"]
    Jagged["Jagged</br>행마다 길이 상이"]
  end
  MultiDim --> Rect
  MultiDim --> Jagged
  CSharpArray --> OneDim
  CSharpArray --> MultiDim
```
...
- [C# 배열 (C# Study)](https://www.csharpstudy.com/CSharp/CSharp-array.aspx) — C# 배열 기초, 가변 배열, 전달 방식
- [C# 배열 (Microsoft Learn)](https://learn.microsoft.com/ko-kr/dotnet/csharp/language-reference/builtin-types/arrays) — 공식 언어 참조
- [C# 강좌 9편. 배열(Array) (Hexabrain)](https://blog.hexabrain.net/136) — 선언·초기화·System.Array 예제
- [Array 클래스 (Microsoft Learn)](https://learn.microsoft.com/ko-kr/dotnet/api/system.array?view=net-7.0) — System.Array API 문서
