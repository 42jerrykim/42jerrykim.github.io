...
- Beginner
- Advanced
title: "[Database] API 개발에서 흔한 DB 성능 실수 5가지와 해결 방법"
---
...
```mermaid
graph LR
    Client["클라이언트"] -->|"요청"| ApiServer["API 서버"]
    ApiServer -->|"쿼리 실행"| Database["데이터베이스"]
    Database -->|"결과 반환"| ApiServer
    ApiServer -->|"응답"| Client
```
...
```mermaid
graph LR
    UserReq["사용자 요청"] --> CacheCheck{"캐시 존재?"}
    CacheCheck -- "예" --> ReturnFromCache["캐시에서 데이터 반환"]
    CacheCheck -- "아니오" --> DbFetch["데이터베이스에서 데이터 조회"]
    DbFetch --> SaveCache["캐시에 데이터 저장"]
    SaveCache --> ReturnFromCache
```

이와 같이 정적 데이터 캐싱을 활용하면 데이터베이스 성능을 최적화할 수 있으며, API의 응답 속도를 향상시킬 수 있다. 데이터베이스에 대한 불필요한 요청을 줄이는 것은 성능 최적화의 중요한 요소임을 잊지 말아야 한다.

### 데이터베이스 과다 사용 상태 페이지 체크
...
```mermaid
graph LR
    UserReq["사용자 요청"] -->|"캐시 조회"| CacheCheck{"캐시 존재?"}
    CacheCheck -- "예" --> ReturnFromCache["캐시에서 데이터 반환"]
    CacheCheck -- "아니오" --> DbQuery["데이터베이스 쿼리"]
    DbQuery --> DbResult["데이터베이스에서 데이터 반환"]
    DbResult --> SaveCache["캐시에 데이터 저장"]
    SaveCache --> ReturnFromCache
```

이와 같은 방식으로 캐싱을 활용하면 데이터베이스의 부하를 효과적으로 줄일 수 있으며, 전체 시스템의 성능을 향상시킬 수 있다. 데이터베이스의 상태 체크와 캐싱 전략을 적절히 활용하는 것이 중요하다.

### 너무 많은 쿼리를 사용하는 API 인증
...
graph LR
    UserReq["사용자 요청"] -->|"캐시 조회"| CacheCheck{"캐시 존재?"}
    CacheCheck -- "있음" --> ReturnFromCache["캐시에서 데이터 반환"]
    CacheCheck -- "없음" --> DbLookup["데이터베이스 조회"]
    DbLookup --> UserExists{"사용자 존재?"}
    UserExists -- "있음" --> SaveCache["캐시에 데이터 저장"]
    UserExists -- "없음" --> AuthFail["인증 실패"]
    SaveCache --> ReturnFromCache
```

이와 같은 캐싱 전략을 통해 API 인증의 성능을 최적화할 수 있으며, 데이터베이스의 부하를 줄이는 동시에 사용자 경험을 개선할 수 있다.

### 루프 내에서 쿼리하는 객체 관계 매퍼
...
```mermaid
graph TD
    UserReq["사용자 요청"] --> OrmQuery["ORM 쿼리 실행"]
    OrmQuery --> NPlusOneCheck{"N+1 쿼리 문제?"}
    NPlusOneCheck -->|"예"| QueryOpt["쿼리 최적화"]
    NPlusOneCheck -->|"아니오"| ReturnResult["결과 반환"]
    QueryOpt --> ReturnResult
```
...
```mermaid
graph TD
    ApiReq["API 요청"] --> Query1["쿼리 1 실행"]
    ApiReq --> Query2["쿼리 2 실행"]
    ApiReq --> Query3["쿼리 3 실행"]
    Query1 --> Resp1["쿼리 1 응답"]
    Query2 --> Resp2["쿼리 2 응답"]
    Query3 --> Resp3["쿼리 3 응답"]
    Resp1 --> ApiResp["API 응답"]
    Resp2 --> ApiResp
    Resp3 --> ApiResp
```
...
```mermaid
graph TD
    Unnormalized["비정규형"] -->|"1NF"| FirstNF["제1정규형"]
    FirstNF -->|"2NF"| SecondNF["제2정규형"]
    SecondNF -->|"3NF"| ThirdNF["제3정규형"]
    ThirdNF -->|"BCNF"| BCNF["보이스-코드 정규형"]
```
...
```mermaid
graph TD
    CustomerTable["고객 테이블"] -->|"중복"| Customer1["고객1"]
    CustomerTable -->|"중복"| Customer2["고객2"]
    CustomerTable -->|"중복"| Customer3["고객3"]
    Customer1 -->|"이메일: example@example.com"| Info1["고객1 정보"]
    Customer2 -->|"이메일: example@example.com"| Info2["고객2 정보"]
    Customer3 -->|"이메일: example@example.com"| Info3["고객3 정보"]
```
...
```mermaid
graph TD
    UserTable["User"] -->|"has"| OrderTable["Order"]
    UserTable -->|"has"| ProfileTable["Profile"]
    OrderTable -->|"contains"| OrderItemTable["OrderItem"]
    ProfileTable -->|"contains"| AddressTable["Address"]
```
...
```mermaid
graph TD
    SecurityPolicy["보안 정책"] --> AccessControl["데이터 접근 제어"]
    SecurityPolicy --> DataEncryption["데이터 암호화"]
    SecurityPolicy --> SecurityUpdate["정기적인 보안 업데이트"]
    SecurityPolicy --> UserEducation["사용자 교육"]
```
...
```mermaid
graph TD
    QueryExec["쿼리 실행"] --> IndexExists{"인덱스 존재?"}
    IndexExists -- "예" --> FastSearch["빠른 검색"]
    IndexExists -- "아니오" --> FullScan["전체 테이블 스캔"]
    FullScan --> PerfDegrade["성능 저하"]
    FastSearch --> PerfImprove["성능 향상"]
```
...
```mermaid
graph TD
    NoIndex["인덱스 없음"] -->|"전체 테이블 스캔"| Time500["500ms"]
    NoIndex -->|"인덱스 추가"| Time10["10ms"]
    Time500 --> PerfDown["성능 저하"]
    Time10 --> PerfUp["성능 향상"]
```
...
```mermaid
graph TD
    IneffQuery["비효율적인 쿼리"] -->|"서브쿼리"| Db1["데이터베이스"]
    Db1 --> Result1["결과 반환"]
    IneffQuery --> IneffPerfDown["성능 저하"]
    OptQuery["최적화된 쿼리"] -->|"JOIN"| Db2["데이터베이스"]
    Db2 --> Result2["결과 반환"]
    OptQuery --> OptPerfUp["성능 개선"]
```
...
```mermaid
graph TD
    TypeChoice["데이터 타입 선택"] --> IntType["정수형"]
    TypeChoice --> StrType["문자열형"]
    TypeChoice --> DateType["날짜형"]
    IntType --> IntCol["INT"]
    IntType --> SmallIntCol["SMALLINT"]
    StrType --> VarcharCol["VARCHAR"]
    StrType --> CharCol["CHAR"]
    DateType --> DateCol["DATE"]
    DateType --> TimestampCol["TIMESTAMP"]
```
...
```mermaid
graph TD
    Db["데이터베이스"] --> Monitor["성능 모니터링 도구"]
    Monitor --> QueryAnalysis["쿼리 실행 시간 분석"]
    Monitor --> CpuCheck["CPU 사용량 체크"]
    Monitor --> MemCheck["메모리 사용량 체크"]
    QueryAnalysis --> QueryOpt["쿼리 최적화"]
    CpuCheck --> HwUpgrade["하드웨어 업그레이드"]
    MemCheck --> IndexAdd["인덱스 추가"]
```
...
```mermaid
graph TD
    BeforeUpgrade["하드웨어 업그레이드 전"] -->|"CPU 사용률 90%"| PerfDown["성능 저하"]
    BeforeUpgrade -->|"RAM 부족"| DiskIoUp["디스크 I/O 증가"]
    BeforeUpgrade -->|"HDD 사용"| SlowResp["응답 속도 느림"]
    AfterUpgrade["하드웨어 업그레이드 후"] -->|"CPU 사용률 50%"| PerfUp["성능 향상"]
    AfterUpgrade -->|"충분한 RAM"| DiskIoDown["디스크 I/O 감소"]
    AfterUpgrade -->|"SSD 사용"| FastResp["응답 속도 개선"]
```
...
```mermaid
graph TD
    ApiReq["API 요청"] -->|"쿼리 최적화 전"| DbQueryBefore["데이터베이스 쿼리"]
    DbQueryBefore --> Resp500["응답 시간: 500ms"]
    ApiReq -->|"쿼리 최적화 후"| DbQueryAfter["데이터베이스 쿼리"]
    DbQueryAfter --> Resp150["응답 시간: 150ms"]
```
...
```mermaid
graph TD
    QueryMonitor["쿼리 성능 모니터링"] --> PlanAnalysis["실행 계획 분석"]
    QueryMonitor --> TimeMeasure["실행 시간 측정"]
    ResourceMonitor["리소스 사용량 모니터링"] --> CpuCheck["CPU 사용량 체크"]
    ResourceMonitor --> MemCheck["메모리 사용량 체크"]
    ResourceMonitor --> DiskIoCheck["디스크 I/O 체크"]
    LogAnalysis["로그 파일 분석"] --> ErrorCheck["오류 메시지 확인"]
    LogAnalysis --> WarnCheck["경고 메시지 확인"]
```
...
```mermaid
graph TD
    IndexCondition["인덱스 추가 조건"] --> WhereColumn["WHERE 절에 자주 사용되는 컬럼"]
    IndexCondition --> JoinColumn["JOIN 연산에 사용되는 컬럼"]
    IndexCondition --> MonitorResult["성능 모니터링 결과"]
``` 
...
```mermaid
graph LR
    UserReq["사용자 요청"] --> ApiSrv["API 서버"]
    ApiSrv --> Orm["ORM"]
    Orm --> Db["데이터베이스"]
    Db --> Monitor["모니터링 도구"]
    Monitor --> PerfAnalysis["성능 분석"]
    PerfAnalysis --> Optimize["최적화 조치"]
```
...
```mermaid
graph TD
    ApiReq["API 요청"] --> QueryOpt["쿼리 최적화"]
    ApiReq --> IndexUse["인덱스 활용"]
    ApiReq --> CachingStrategy["캐싱 전략"]
    ApiReq --> Maintenance["정기적인 유지 관리"]
    ApiReq --> MonitorUse["모니터링 도구 사용"]
    QueryOpt --> PerfGain["성능 향상"]
    IndexUse --> PerfGain
    CachingStrategy --> PerfGain
    Maintenance --> PerfGain
    MonitorUse --> PerfGain
```
