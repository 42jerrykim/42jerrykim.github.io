...
draft: false
image: "wordcloud.png"
description: "CRDT(Conflict-Free Replicated Data Types)는 분산 시스템에서 충돌 없이 데이터를 자동 병합하는 자료구조다. 상태 기반·연산 기반 모델, 교환·결합·항등 법칙, LWW·G-Counter·OR-Set 등 핵심 타입과 구현 예제, Google Docs·Figma·Redis·Yjs 등 활용 사례와 성능 최적화·OT와의 비교까지 정리한다."
...
**이 포스트의 구성**: 개요(정의·필요성·OT와의 차이) → CRDT의 기본 개념(구성 요소·수학적 속성) → CRDT의 종류(상태 기반·연산 기반·대표 타입) → 구현(LWW 레지스터·LWW 맵·병합) → 응용(협업 앱·동기화·사용 사례) → 성능 최적화 → FAQ → 관련 기술(P2P·일관성·협업 기술) → 결론 → 참고 자료 및 Reference.
...
```mermaid
graph TD
    CrdtNode["CRDT"] -->|"상태 기반"| DataMerge["데이터 병합"]
    CrdtNode -->|"충돌 방지"| MathProps["수학적 속성"]
    OtNode["OT"] -->|"연산 기반"| OpTransform["작업 변환"]
    OtNode -->|"순서 고려"| ConflictResolve["충돌 해결"]
```
...
```mermaid
graph TD
    ValueNode["값(value)"] --> StateNode["상태(state)"]
    StateNode --> MergeFunc["병합(merge) 함수"]
```
...
```mermaid
graph TD
    NodeAState["노드 A의 상태"] -->|"병합"| NodeBState["노드 B의 상태"]
    NodeBState -->|"병합"| FinalState["최종 상태"]
```
...
```mermaid
graph TD
    NodeAOp["노드 A의 연산"] -->|"전파"| NodeBOp["노드 B의 연산"]
    NodeBOp -->|"전파"| FinalStateOp["최종 상태"]
```
...
```mermaid
graph TD
    CurrentState["현재 상태"] -->|"병합"| NewState["새로운 상태"]
    StateOne["상태 1"] -->|"병합"| NewState
    StateTwo["상태 2"] -->|"병합"| NewState
```
...
```mermaid
graph LR
    StateA["상태 A"] -->|"병합"| StateB["상태 B"]
    StateB -->|"결과"| MergedState["최종 상태"]
```
...
```mermaid
graph TD
    Client1["클라이언트 1"] -->|"수정"| CrdtState["CRDT 상태"]
    Client1 -->|"전송"| ServerNode["서버"]
    ServerNode -->|"병합"| FinalStateSync["최종 상태"]
    Client2["클라이언트 2"] -->|"수정"| CrdtState
    Client2 -->|"전송"| ServerNode
```
...
```mermaid
graph TD
    CrdtRoot["CRDT"] --> DataStructOpt["데이터 구조 최적화"]
    CrdtRoot --> MergeAlgoOpt["병합 알고리즘 개선"]
    CrdtRoot --> NetworkOpt["네트워크 최적화"]
    DataStructOpt --> MemReduce["메모리 사용량 감소"]
    MergeAlgoOpt --> MergeSpeed["병합 속도 증가"]
    NetworkOpt --> BandwidthSave["대역폭 절약"]
```
...
```mermaid
graph TD
    NodeOne["노드 1"] -->|"1 증가"| CounterOne["카운터 1"]
    NodeTwo["노드 2"] -->|"2 증가"| CounterTwo["카운터 2"]
    CounterOne -->|"병합"| MaxSelect["최대값 선택"]
    CounterTwo -->|"병합"| MaxSelect
```
...
```mermaid
graph LR
    PeerA["노드 A"] -->|"수정"| PeerB["노드 B"]
    PeerB -->|"병합"| MergedResult["병합된 상태"]
```
...
```mermaid
graph TD
    CrdtCore["CRDT"] --> CollabApp["협업 애플리케이션"]
    CrdtCore --> RealtimeSync["실시간 데이터 동기화"]
    CrdtCore --> NewAppIdeas["새로운 애플리케이션 아이디어"]
    CollabApp --> GoogleDocs["Google Docs"]
    CollabApp --> Figma["Figma"]
    RealtimeSync --> IoT["IoT"]
    NewAppIdeas --> DistributedSocial["분산형 소셜 미디어"]
    NewAppIdeas --> OnlineEdu["온라인 교육 플랫폼"]
```
...
- **Wikipedia – Conflict-free replicated data type**: CRDT의 공식 정의, CvRDT/CmRDT 분류, G-Counter·PN-Counter·OR-Set 등 공식 명세와 산업 사용 사례를 한곳에 정리한 문서다.
- **An Interactive Intro to CRDTs (jakelazaroff.com)**: LWW 레지스터·LWW 맵의 인터랙티브 예제와 TypeScript 코드로 CRDT 인터페이스(merge·state·value)를 설명하며, 초보자에게 적합하다.
- **CRDTs go brrr (josephg.com)**: Automerge·Yjs 성능 비교, 플랫 리스트·Range tree 등 데이터 구조 최적화로 수천 배 성능 향상을 다룬 실전 최적화 사례다.

다음은 CRDT의 기본 개념을 설명하는 다이어그램이다.

```mermaid
graph TD
    CrdtTypes["CRDT"] --> StateBased["상태 기반 CRDT"]
    CrdtTypes --> OpBased["연산 기반 CRDT"]
    StateBased --> LwwReg["LWW 레지스터"]
    StateBased --> LwwMap["LWW 맵"]
    OpBased --> GCounter["G-Counter"]
    OpBased --> PNCounter["PN-Counter"]
    OpBased --> GSet["G-Set"]
    OpBased --> TwoPSet["2P-Set"]
    OpBased --> LwwElementSet["LWW-Element-Set"]
    OpBased --> ORSet["OR-Set"]
```
...
- [CRDT 구현하기: CRDT란? (velog, HBSPS)](https://velog.io/@hbsps/CRDT-%EA%B5%AC%ED%98%84%ED%95%98%EA%B8%B0-CRDT%EB%9E%80)
- [실시간 동시 편집: OT와 CRDT (velog, dev_hee)](https://velog.io/@heelieben/%EC%8B%A4%EC%8B%9C%EA%B0%84-%EB%8F%99%EC%8B%9C-%ED%8E%B8%EC%A7%91-OT-%EC%99%80-CRDT)
- [CRDTs go brrr – 5000x faster CRDTs (josephg.com)](https://josephg.com/blog/crdts-go-brrr/)
