...
인덱스는 테이블의 특정 열(또는 열 조합) 값을 키로 하고, 그 키에 대응하는 <strong>행의 위치(포인터)</strong>를 담은 구조다. 일반적으로 B-트리·B+트리나 해시 등으로 구현된다.
...
인덱스는 한 번 만들고 끝이 아니라, <strong>조각화(fragmentation)</strong>와 사용 패턴 변화에 맞춰 주기적으로 점검·정리하는 것이 좋다.
...
인덱스는 <strong>읽기(검색, 조인, 정렬)</strong>를 빠르게 하는 대신, **쓰기(INSERT/UPDATE/DELETE)** 시 해당 인덱스도 함께 갱신해야 해서 쓰기 비용이 늘어난다. 읽기가 많고 쓰기가 적은 열에 인덱스를 두는 것이 보통 유리하다.
