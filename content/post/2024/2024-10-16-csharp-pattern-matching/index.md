...
date: 2024-10-16
lastmod: 2026-03-17
description: "C# 패턴 일치(Pattern Matching) 개념과 null 검사, 형식 테스트, 관계형·목록 패턴, switch 식 등 실무 활용법을 최신 문법과 예제로 정리하고, Mermaid로 흐름을 보여줍니다. 42jerrykim.github.io에서 더 많은 정보를 확인할 수 있다."
title: "[CSharp] C# 패턴 일치(Pattern Matching) 개념과 실무 활용"
categories:
- CSharp
- Patterns
tags:
...
```mermaid
graph TD;
    PatternMatch[패턴 일치] --> Readability[가독성 향상]
    PatternMatch --> Maintainability[유지보수 용이]
    PatternMatch --> ErrorReduce[오류 감소]
    Readability --> Conciseness[코드 간결화]
    Maintainability --> ClearLogic[명확한 로직]
    ErrorReduce --> TypeSafety[타입 안전성]
```
...
```mermaid
graph TD;
    InputVal[입력값] --> TypeTest{"타입 검사"}
    TypeTest -->|"null"| NullMsg[입력이 null입니다.]
    TypeTest -->|"int"| PosTest{"양수 검사"}
    PosTest -->|"양수"| PosOut[양수 출력]
    PosTest -->|"음수"| NegOut[음수 출력]
    TypeTest -->|"string"| StrOut[문자열 출력]
    TypeTest -->|"기타"| UnknownType[알 수 없는 타입입니다.]
```
...
```mermaid
flowchart TD
    Obj[객체] -->|"is null"| NullProc[null 처리]
    Obj -->|"is not null"| ValidProc[유효한 객체 처리]
```
...
```mermaid
graph TD;
    InputObj[입력 객체] -->|"형식 검사"| TypeCheck{"형식"}
    TypeCheck -->|"int"| IntResult[정수형입니다.]
    TypeCheck -->|"string"| StrResult[문자열형입니다.]
    TypeCheck -->|"IList<string>"| ListResult[문자열 리스트입니다.]
    TypeCheck -->|"기타"| UnknownResult[알 수 없는 형식입니다.]
```
...
```mermaid
graph TD;
    CmdInput[Command 입력] --> EnumConv{"Enum 변환"}
    EnumConv -->|"성공"| CmdProc[명령어 처리]
    EnumConv -->|"실패"| InvalidCmd[유효하지 않은 명령어]
    CmdProc --> StartCmd[Start]
    CmdProc --> StopCmd[Stop]
    CmdProc --> PauseCmd[Pause]
    CmdProc --> ResumeCmd[Resume]
```
...
graph TD;
    Temp[온도] -->|"< 0"| Solid[고체]
    Temp -->|"0"| SolidLiquid[고체와 액체]
    Temp -->|"> 0 and < 100"| Liquid[액체]
    Temp -->|"100"| LiquidGas[액체와 기체]
    Temp -->|"기타"| Gas[기체]
```
...
```mermaid
graph TD;
    OrderNode[Order] -->|"IsMember: true"| Discount10["10% 할인"]
    OrderNode -->|"ItemCount > 5"| Discount5["5% 할인"]
    OrderNode -->|"기타"| NoDiscount[할인 없음]
    CustNode[Customer] -->|"City: Seoul"| Eligible[Eligible]
    CustNode -->|"기타"| NotEligible["Not Eligible"]
```
...
```mermaid
graph TD;
    CsvData[CSV 데이터] --> ListPattern{"목록 패턴"}
    ListPattern --> FixedLen[고정 길이 패턴]
    ListPattern --> SlicePattern[슬라이스 패턴]
    FixedLen --> DataDecomp[데이터 분해]
    SlicePattern --> IgnoreItems[무시 항목 처리]
```
...
```mermaid
graph TD;
    InputNode[입력] -->|"null"| NullCheck[null 검사]
    InputNode -->|"int"| PositiveCheck[양수 검사]
    InputNode -->|"string"| StringProc[문자열 처리]
    InputNode -->|"IList<int>"| ListProc[리스트 처리]
    InputNode -->|"기타"| UnknownInput[알 수 없는 입력]
```
...
```mermaid
graph TD;
    IfStmt[일반 조건문] -->|"느린 성능"| ComplexCond[복잡한 조건]
    IfStmt -->|"빠른 성능"| SimpleCond[단순 조건]
    PatternStmt[패턴 일치] -->|"빠른 성능"| ComplexPattern[복잡한 패턴]
    PatternStmt -->|"매우 빠른 성능"| SimplePattern[단순 패턴]
```
...
```mermaid
graph TD;
    PatternSummary[패턴 일치] --> ReadabilitySummary[가독성 향상]
    PatternSummary --> MaintainSummary[유지보수 용이]
    PatternSummary --> PerfOpt[성능 최적화]
    FutureDir[향후 발전 방향] --> MorePatterns[더 많은 패턴 추가]
    FutureDir --> PerfImprove[성능 개선]
    FutureDir --> ToolSupport[도구 지원 강화]
```
...
패턴 일치에 대한 더 깊은 이해를 원한다면 다음 공식 문서를 참고하면 좋다.

1. **[패턴 일치 개요 - C# (한국어)](https://learn.microsoft.com/ko-kr/dotnet/csharp/fundamentals/functional/pattern-matching)**  
   null 검사, 형식 테스트, 관계형·목록 패턴 등 시나리오별 설명과 예제를 제공한다.

2. **[Patterns - C# reference (영어)](https://learn.microsoft.com/en-us/dotnet/csharp/language-reference/operators/patterns)**  
   선언·타입·상수·관계형·속성·목록·var·discard 패턴 등 전체 패턴 어휘와 `is`, `switch` 식 사용법을 다룬다.

3. **[자습서: 패턴 일치를 사용하여 형식 기반 및 데이터 기반 알고리즘 빌드](https://learn.microsoft.com/ko-kr/dotnet/csharp/fundamentals/tutorials/pattern-matching)**  
   패턴 일치로 타입·데이터 기반 알고리즘을 만드는 실습 튜토리얼이다.

4. **[C# Language Specification](https://learn.microsoft.com/en-us/dotnet/csharp/language-reference/language-specification/)**  
   C# 언어 공식 사양으로, 패턴 일치 관련 세부 규격을 참고할 수 있다.
