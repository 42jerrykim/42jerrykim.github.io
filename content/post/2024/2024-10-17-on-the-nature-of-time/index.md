...
양자역학에서는 입자의 상태가 여러 가능성을 동시에 가질 수 있는 '중첩' 상태를 설명한다. 이러한 중첩 상태는 <strong>분기적 공간(branchial space)</strong>의 개념과 연결된다. 특정 사건이 발생할 때, 그 사건은 여러 경로로 나뉘어질 수 있으며, 각 경로는 서로 다른 결과를 초래할 수 있다. 역사의 여러 경로가 존재하는 것이 양자역학을 이끌며, 관찰자가 궁극적으로 **한 개의 경로만** 인식하는 것은 양자역학에서의 "측정" 현상과 연결된다.
