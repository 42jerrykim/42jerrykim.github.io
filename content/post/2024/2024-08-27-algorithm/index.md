---
title: "[Algorithm] 알고리즘의 주제별 분류와 설계 기법"
description: "정렬·탐색·그래프·동적 계획법·그리디·문자열 매칭 등 알고리즘의 주제별 분류와 결정·확률 알고리즘, 설계 기법(분할정복·DP·탐욕)을 정리한다. 각 유형별 특징·시간·공간 복잡도, 파이썬 구현 예시, FAQ·참고 문헌을 포함해 코딩 테스트와 실무 적용에 활용할 수 있도록 구성했다."
...
**이 포스트에서 다루는 내용**  
주제별 분류(기초·탐색·스트링 매칭·정렬·그래프·해시·최적화), 확률적/결정적 구분, 설계 기법(분할 정복·동적 계획법·탐욕), 그리고 각 유형별 대표 알고리즘의 개념·복잡도·구현 예시와 참고 자료를 다룬다.

**추천 대상**  
알고리즘과 자료구조를 체계적으로 정리하고 싶은 학습자, 코딩 테스트·경쟁 프로그래밍 준비자, 그리고 실무에서 알고리즘 선택 기준을 잡고 싶은 개발자에게 도움이 된다.
...
```mermaid
graph TD
    Algo["알고리즘"] --> ByNature["문제의 성격"]
    Algo --> ByMethod["해결 방법"]
    Algo --> ByTime["시간 복잡도"]
    Algo --> BySpace["공간 복잡도"]
    ByNature --> Search["탐색 알고리즘"]
    ByNature --> Sort["정렬 알고리즘"]
    ByNature --> GraphAlgo["그래프 알고리즘"]
    ByMethod --> DivConq["분할 정복"]
    ByMethod --> DP["동적 계획법"]
    ByMethod --> Greedy["탐욕 알고리즘"]
```
...
```mermaid
graph TD
    InputAB["두 수 a, b"] --> CheckB{"b가 0인가?"}
    CheckB -->|"예"| ResultA["최대공약수는 a"]
    CheckB -->|"아니오"| ModStep["나머지 r = a % b"]
    ModStep --> Update["새로운 a = b, 새로운 b = r"]
    Update --> CheckB
```
...
```mermaid
graph TD
    SearchAlgo["탐색 알고리즘"] --> Seq["순차 탐색 O(n)"]
    SearchAlgo --> Bin["이진 탐색 O(log n)"]
```
...
```mermaid
graph TD
    Text["텍스트"] -->|"패턴 검색"| KMP["KMP 알고리즘"]
    Text -->|"해시 비교"| RabinKarp["라빈-카프 알고리즘"]
    KMP --> Found["패턴 발견"]
    RabinKarp --> Found
```
...
```mermaid
graph TD
    StartNode["Start"] --> CheckVertex{"Is there an unvisited vertex?"}
    CheckVertex -->|"Yes"| Visit["Visit vertex"]
    Visit --> Mark["Mark as visited"]
    Mark --> CheckVertex
    CheckVertex -->|"No"| EndNode["End"]
```
...
```mermaid
graph TD
    StartMst["Start"] --> SelectEdge["Select edge with minimum weight"]
    SelectEdge --> CycleQ{"Does it form a cycle?"}
    CycleQ -->|"No"| AddEdge["Add edge to MST"]
    CycleQ -->|"Yes"| SelectEdge
    AddEdge --> AllVertices{"Are all vertices included?"}
    AllVertices -->|"No"| SelectEdge
    AllVertices -->|"Yes"| EndMst["End"]
```
...
```mermaid
graph TD
    StartPath["Start"] --> InitDist["Initialize distances"]
    InitDist --> Relax["Relax edges"]
    Relax -->|"Yes"| UpdateDist["Update distance"]
    Relax -->|"No"| AllProcessed{"All vertices processed?"}
    AllProcessed -->|"No"| Relax
    AllProcessed -->|"Yes"| EndPath["End"]
```
...
```mermaid
graph TD
    HashTbl["Hash Table"] --> Bucket0["Bucket 0"]
    HashTbl --> Bucket1["Bucket 1"]
    HashTbl --> Bucket2["Bucket 2"]
    HashTbl --> Bucket3["Bucket 3"]
    HashTbl --> Bucket4["Bucket 4"]
    Bucket0 --> K1["Key: apple, Value: 1"]
    Bucket0 --> K2["Key: banana, Value: 2"]
    Bucket1 --> K3["Key: orange, Value: 3"]
    Bucket2 --> K4["Key: grape, Value: 4"]
```
...
```mermaid
graph TD
    ObjFunc["목적 함수"] -->|"최대화"| Constraint1["제약 조건 1"]
    ObjFunc -->|"최대화"| Constraint2["제약 조건 2"]
    Constraint1 --> Solution["해"]
    Constraint2 --> Solution
```
...
```mermaid
flowchart TD
    InitPop["초기 개체군 생성"] --> Eval["적합도 평가"]
    Eval --> TermCond{"종료 조건?"}
    TermCond -->|"아니오"| Select["선택"]
    Select --> Crossover["교배"]
    Crossover --> Mutate["돌연변이"]
    Mutate --> Eval
    TermCond -->|"예"| Best["최적해 도출"]
```
...
```mermaid
graph TD
    Problem["문제"] --> Sub1["하위 문제 1"]
    Problem --> Sub2["하위 문제 2"]
    Sub1 --> Sub11["하위 문제 1.1"]
    Sub1 --> Sub12["하위 문제 1.2"]
    Sub2 --> Sub21["하위 문제 2.1"]
    Sub2 --> Sub22["하위 문제 2.2"]
    Sub11 --> Res1["결과 1"]
    Sub12 --> Res2["결과 2"]
    Sub21 --> Res3["결과 3"]
    Sub22 --> Res4["결과 4"]
...
```mermaid
graph TD
    Begin["시작"] --> SetMax["리스트의 첫 번째 값으로 최대값 설정"]
    SetMax --> CheckAll{"리스트의 모든 값 확인"}
    CheckAll -->|"값이 더 크면"| UpdateMax["최대값 업데이트"]
    CheckAll -->|"모든 값 확인 완료"| ReturnMax["최대값 반환"]
    UpdateMax --> CheckAll
    ReturnMax --> Finish["종료"]
```
...
```mermaid
graph TD
    InputN["입력 크기 n"] -->|"O(1)"| Const["상수 시간"]
    InputN -->|"O(log n)"| LogT["로그 시간"]
    InputN -->|"O(n)"| Linear["선형 시간"]
    InputN -->|"O(n log n)"| LineLog["선형 로그 시간"]
    InputN -->|"O(n^2)"| Poly["다항 시간"]
```
...
```mermaid
graph TD
    Learn["알고리즘 학습"] --> Solve["문제 해결 능력 향상"]
    Learn --> Code["효율적인 코드 작성"]
    Learn --> Apply["다양한 분야 응용"]
    Apply --> Data["데이터 분석"]
    Apply --> AI["인공지능"]
    Apply --> Web["웹 개발"]
    Future["알고리즘 발전 방향"] --> ML["인공지능 및 머신러닝"]
    Future --> Quantum["양자 컴퓨팅"]
    Future --> Opt["효율성 및 최적화"]
```
...
```mermaid
graph TD
    Step0["64, 34, 25, 12, 22, 11, 90"] --> Step1["34, 25, 12, 22, 11, 64, 90"]
    Step1 --> Step2["25, 12, 22, 11, 34, 64, 90"]
    Step2 --> Step3["12, 22, 11, 25, 34, 64, 90"]
    Step3 --> Step4["11, 12, 22, 25, 34, 64, 90"]
```
...
- [나무위키: 분류:알고리즘](https://namu.wiki/w/%EB%B6%84%EB%A5%98:%EC%95%8C%EA%B3%A0%EB%A6%AC%EC%A6%98)
- [나무위키: 코딩 테스트](https://namu.wiki/w/%EC%BD%94%EB%94%A9%20%ED%85%8C%EC%8A%A4%ED%8A%B8)
