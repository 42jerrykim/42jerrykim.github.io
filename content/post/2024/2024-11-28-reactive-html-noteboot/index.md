---
title: "[HTML] HTML 기반 반응형 데이터 탐색·분석 노트북"
categories: 
- HTML
- DataScience
- Programming
...
- **개요**: HTML 기반 데이터·출판 통합의 필요성, 추천 대상
- **HTML을 활용한 데이터 탐색 및 분석**: 타입셋팅, 셀 생성, `contenteditable` 동적 실행
- **Observable의 활용**: HTL, Observable Plot, 셀 상태·에러 처리
- **다양한 언어 통합**: Pyodide(Python), WebR(R), JavaScript
- **사용자 입력 및 상호작용**: Observable Inputs, `viewof`, `mutable`
- **문서화된 프로그래밍과 HTML**: 개념, 블로그 작성, 코드 단계별 이해
- **관련 기술**: Jupyter·LaTeX·Typst 비교, Heximal
- **FAQ** · **결론** · **참고 문헌**
...
- **데이터 과학자·연구자**: Jupyter·LaTeX 워크플로를 HTML 한 파일로 통합하고 싶은 경우
- **프론트엔드·풀스택 개발자**: Observable·브라우저 기반 반응형 문서를 직접 구축하려는 경우
- **기술 블로거·문서 작성자**: 코드·시각화·설명이 한 문서에 공존하는 문서화된 프로그래밍에 관심 있는 경우
...
```mermaid
graph TD
    dataCollect["데이터 수집"] --> htmlDoc["HTML 문서 생성"]
    htmlDoc --> jsProcess["JavaScript로 데이터 처리"]
    jsProcess --> dataViz["데이터 시각화"]
    dataViz --> resultConfirm["웹 브라우저에서 결과 확인"]
```
...
```mermaid
graph TD
    htmlNode["HTML"] --> typeset["타입셋팅 기능"]
    htmlNode --> cellCreate["HTML 셀 생성"]
    htmlNode --> contentEditable["contenteditable 속성"]
    typeset --> readability["가독성 향상"]
    cellCreate --> dataExplore["데이터 탐색 용이"]
    contentEditable --> dynamicScript["동적 스크립트 실행"]
```
...
```mermaid
graph TD
    htl["Hypertext Literal"] --> obsPlot["Observable Plot"]
    obsPlot --> cellState["셀 상태 관리"]
    cellState --> errHandle["에러 처리"]
```
...
```mermaid
graph TD
    userInput["사용자 입력"] --> obsInputs["Observable Inputs"]
    obsInputs --> viewofCell["viewof 셀"]
    viewofCell --> mutableFn["mutable 함수"]
    mutableFn --> stateUpdate["상태 변화 반영"]
```
...
```mermaid
graph TD
    codeFrag["코드 조각"] --> funcDef["함수 정의"]
    codeFrag --> areaCalc["면적 계산"]
    codeFrag --> funcCall["함수 호출"]
    codeFrag --> resultOut["결과 출력"]
```
...
```mermaid
graph TD
    heximal["Heximal"] --> html2["HTML"]
    heximal --> css2["CSS"]
    heximal --> js2["JavaScript"]
    html2 --> dataViz2["데이터 시각화"]
    css2 --> responsive["반응형 디자인"]
    js2 --> userInteract["사용자 상호작용"]
```
...
| 구분 | 내용 |
|------|------|
| **장점** | 단일 HTML 파일로 배포·공유 가능, 빌드·CI·제3자 플랫폼 최소화, 브라우저만으로 탐색·분석·출판 통합, Pyodide·WebR로 Python·R 실행 가능. |
| **단점** | contenteditable 기반 편집 UX는 Jupyter·전용 IDE 대비 불편할 수 있음, 모바일(iOS 등)에서 Pyodide 셀 편집 시 불안정 사례 보고됨. |
...
```mermaid
graph TD
    dataCollect2["데이터 수집"] --> dataAnalyze["데이터 분석"]
    dataAnalyze --> htmlDoc2["HTML 문서 생성"]
    htmlDoc2 --> dataViz3["데이터 시각화"]
    dataViz3 --> publishShare["출판 및 공유"]
```
...
1. **Reactive HTML notebooks (원문)** — Max Bo. [A HTML file is all you need](https://maxbo.me/a-html-file-is-all-you-need.html). contenteditable·Observable 런타임·Pyodide·WebR·HTL·Plot을 활용한 단일 HTML 노트북 구현.
2. **GeekNews 요약** — [리액티브 HTML 노트북](https://news.hada.io/topic?id=17837). 기사 요약 및 커뮤니티 논의.
3. **Hacker News 논의** — [Reactive HTML Notebooks](https://news.ycombinator.com/item?id=42170740). Heximal·Jupyter 대비 장단점·출판 vs 탐색적 분석 등 논의.

---

**한 줄 정리**: HTML 한 파일에 contenteditable·Observable 런타임·Pyodide·WebR을 결합하면, 데이터 탐색·분석·시각화·출판을 빌드 단계 없이 단일 아티팩트로 수행할 수 있다.
