...
graph TD
    ...
    functionNode --> outputNode
    functionNode --> lambdaExpr
    lambdaExpr --> evaluationNode
```
...
```mermaid
graph LR
    earlyLife["Alonzo Church의 초기 생애"]
    birth["출생: 1903년 6월 14일"]
    princetonEdu["프린스턴 대학교에서의 교육"]
    ba["학사: 1924년"]
    ma["석사: 1926년"]
    phd["박사: 1927년"]
    personalChal["개인적 도전"]
    ...
    academicAch["학문적 성취"]
    lambdaDev["λ-계산법 개발"]
    csFoundation["컴퓨터 과학 기초 다지기"]
    earlyLife --> birth
    earlyLife --> princetonEdu
    princetonEdu --> ba
    princetonEdu --> ma
    princetonEdu --> phd
    earlyLife --> personalChal
    personalChal --> criticism
    personalChal --> motivation
    earlyLife --> academicAch
    academicAch --> lambdaDev
    academicAch --> csFoundation
```
...
graph TD
    ...
    turingMachine["Turing 기계"]
    lambdaCalc --> funcDef
    lambdaCalc --> funcApply
    funcDef --> abstraction
    funcApply --> combination
    abstraction --> computability
    combination --> computability
...
graph TD
    problemDef[문제 정의]
    effectiveCheck{"효과적인 계산 가능성?"}
    algoDesign[알고리즘 설계]
    unsolvable[해결 불가능]
    result[결과 도출]
    haltingProb["Halting Problem"]
    problemDef --> effectiveCheck
    effectiveCheck -->|"예"| algoDesign
    effectiveCheck -->|"아니오"| unsolvable
    algoDesign --> result
    unsolvable --> haltingProb
```
...
```python
# λ-계산법을 사용한 간단한 예시
def lambda_calculus_example(x):
    return x + 1

result = lambda_calculus_example(5)
print(result)  # 출력: 6
```
...
```mermaid
graph TD
    entscheid[Entscheidungsproblem]
    decidable["결정 가능한 문제"]
    undecidable["결정 불가능한 문제"]
    algoExists["알고리즘 존재"]
    noAlgo["알고리즘 없음"]
    churchThm["Church의 정리"]
    limit["계산 가능성의 한계"]
    entscheid --> decidable
    entscheid --> undecidable
    decidable --> algoExists
    undecidable --> noAlgo
    noAlgo --> churchThm
    churchThm --> limit
```
...
graph TD
    ...
    turing["Alan Turing"]
    compTheory["Computational Theory"]
    mathLogic["Mathematical Logic"]
    turingMach["Turing Machine"]
    church --> kleene
    church --> rosser
    church --> turing
    kleene --> compTheory
...
```mermaid
graph TD
    churchNode["Alonzo Church"]
    lambdaCalcLegacy["λ-Calculus"]
    funcProg["Functional Programming"]
    modernLang["Modern Programming Languages"]
    haskellLang[Haskell]
    churchNode --> lambdaCalcLegacy
    lambdaCalcLegacy --> funcProg
    lambdaCalcLegacy --> modernLang
    funcProg --> haskellLang
```
...
graph TD
    ...
    headMove["헤드 이동"]
    writeSym["기호 쓰기"]
    startState --> stateTrans
    stateTrans -->|"기호 읽기"| headMove
    headMove --> writeSym
    writeSym --> startState
```
...
```mermaid
graph TD
    effectiveComp["효과적인 계산 가능성"]
    churchLambda["처치의 λ-계산법"]
    turingMach["튜링 기계"]
    allComputable["모든 계산 가능 함수"]
    effectiveComp --> churchLambda
    effectiveComp --> turingMach
    churchLambda --> allComputable
    turingMach --> allComputable
```
...
- [Alonzo Church: The Forgotten Architect of Computer Intelligence](https://onepercentrule.substack.com/p/alonzo-church-the-forgotten-architect) — The One Percent Rule (Substack)
- [Alonzo Church](https://en.wikipedia.org/wiki/Alonzo_Church) — Wikipedia
- [Entscheidungsproblem](https://en.wikipedia.org/wiki/Entscheidungsproblem) — Wikipedia
