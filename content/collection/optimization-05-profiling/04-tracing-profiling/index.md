...
draft: false
image: wordcloud.png
title: "[Profiling 05] 트레이싱 프로파일링: Perfetto·Tracy"
...
계측 트레이싱의 데이터 모델은 **스팬(span)** 혹은 **존(zone)**입니다. 코드의 한 구간에 매크로를 심으면, 진입 시각과 이탈 시각이 짝을 이룬 이벤트가 기록되고, 같은 스레드에서 중첩된 스팬들은 스택 구조를 이룹니다. C++에서는 RAII가 이 짝맞춤을 자연스럽게 처리합니다. 매크로가 스코프 진입 시 begin 이벤트를 쓰는 객체를 만들고, 스코프를 벗어날 때 소멸자가 end 이벤트를 쓰는 방식입니다. Perfetto의 `TRACE_EVENT`와 Tracy의 `ZoneScoped`가 모두 이 패턴입니다.
