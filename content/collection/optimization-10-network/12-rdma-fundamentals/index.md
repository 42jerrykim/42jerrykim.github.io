...
**전제 지식**: 이 장은 [02장: 소켓 옵션 튜닝](/post/network-optimization/socket-options-tcp-nodelay-buffer-tuning/)에서 다룬 "커널 소켓 버퍼와 시스템 콜 비용" 개념과, [09장: 메시지 프레이밍](/post/network-optimization/message-framing-length-prefix-delimiter-fixed-size/)의 "애플리케이션 레벨 메시지 경계" 개념을 전제로 합니다. 커널 우회(kernel bypass)라는 용어를 처음 접했다면 [Tr.06: 커널 바이패스 개요](/post/os-optimization/kernel-bypass-overview/)를 먼저 읽는 편이 좋습니다.
