...
draft: false
image: wordcloud.png
title: "[Network 10] Ultra Ethernet Consortium(UEC)과 차세대 저지연 이더넷"
...
이 장은 [13장: RDMA 기초](/post/network-optimization/rdma-infiniband-fundamentals/)에서 다룬 RDMA의 기본 개념(큐 페어, 원격 메모리 접근, InfiniBand·RoCEv2의 관계)을 전제로 합니다. RDMA가 무엇이고 왜 커널을 우회해 지연을 줄이는지 모른다면 13장을 먼저 읽는 것이 좋습니다. 혼잡 제어의 일반적인 개념(ECN, 윈도우 기반 제어)에 익숙하면 도움이 되며, 해당 기초는 [3장: TCP 성능 최적화](/post/network-optimization/tcp-performance-nagle-congestion-control-bbr/)에서 다룬 내용을 참고할 수 있습니다.

**이 장의 깊이**는 **전문가** 구간입니다. UEC Specification 1.0의 4계층 구조를 개괄하고, 이 트랙 다른 챕터가 다루지 않는 UEC 고유 메커니즘(LLR, CBFC, UET 혼잡 제어, 패킷 스프레이)을 심층적으로 다룹니다. **다루지 않는 것**: RDMA·InfiniBand의 기초 개념(→ 13장), 커널 바이패스·DPDK/XDP 구현 세부사항(→ [11장](/post/network-optimization/dpdk-advanced-deep-dive-smartnic-dpu/), [12장](/post/network-optimization/xdp-ebpf-network-packet-processing-advanced/)), TCP 혼잡 제어 알고리즘 일반론(→ 3장), TLS/PQC 기반 보안(→ [17장](/post/network-optimization/tls-ssl-handshake-optimization-pqc/); UEC의 Transport Security Subsystem은 AES-GCM 기반 자체 계층으로 TLS와 별개이며 이 장에서는 존재만 언급합니다).
...
**다음 장에서는** 애플리케이션 계층으로 눈을 돌려 gRPC의 성능 튜닝을 다룹니다. 이 장에서 다룬 UEC·RDMA가 데이터센터 내부 팹릭의 물리·전송 계층 최적화라면, gRPC는 그 위에서 서비스 간 호출이 실제로 겪는 직렬화·연결 관리·스트리밍 오버헤드를 다루는 상위 계층 최적화입니다. → [gRPC 최적화](/post/network-optimization/grpc-performance-tuning-optimization/) (챕터 15)

**이전 장**: [RDMA 기초](/post/network-optimization/rdma-infiniband-fundamentals/) (챕터 13)
