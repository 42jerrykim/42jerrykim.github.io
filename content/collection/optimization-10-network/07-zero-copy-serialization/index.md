...
**"zero-copy 포맷은 항상 더 빠르다"**도 워크로드에 따라 깨집니다. vtable 조회나 포인터 역참조는 상수 비용이지만 0은 아니며, 메시지의 필드를 전부 순회해야 하는 워크로드에서는 이 간접 참조 비용이 누적되어 단순 구조체 그대로를 복사하는 것보다 느릴 수 있습니다. 포맷 간 실제 처리량 차이는 06장에서 다룬 벤치마크로 확인하는 것이 정확합니다.
