...
헤더에 담을 필드는 보통 다음과 같은 역할로 나뉩니다. **매직 넘버(magic)**는 이 바이트열이 해당 프로토콜의 메시지가 맞는지 빠르게 검증하는 상수이고, **버전(version)**은 이후 등장할 additive-only 규칙의 기준점이며, **타입(type)**은 바디를 어떤 핸들러로 넘길지 결정하는 코드이고, **길이(length)**는 헤더를 포함한 전체 메시지 크기로 이후 10장에서 다룰 프레이밍과 맞물리는 필드입니다. **시퀀스 번호(sequence)**는 순서 보장이나 손실 감지가 필요할 때 단조 증가 값으로 넣습니다.
