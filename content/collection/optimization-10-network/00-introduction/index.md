...
| 10 | 네트워크 DPDK 심화 | 전문 | 심화(본 트랙); 개요는 Tr.06; NVIDIA BlueField-4 등 최신 SmartNIC/DPU 오프로드 동향 포함 |
| 11 | 네트워크 XDP/eBPF | 전문 | 심화(본 트랙); 개요는 Tr.06 |
...
**Phase B — 직렬화·프로토콜 (챕터 05~09, 14, 17~19, 21)** 메시지 설계는 CPU(Tr.05), 언어 표현(Tr.02), 할당·레이아웃(Tr.04)과 만나는 지점입니다. 챕터 07의 신흥 zero-copy 포맷 동향은 06의 기존 포맷 비교 감각을 먼저 잡은 뒤 읽으면 판단이 쉬워집니다. 압축(21)까지 포함해 CPU-대역폭 trade-off를 함께 봐야 합니다.

**Phase C — 커널 바이패스·고급 전송 (챕터 10~13, 15~16)** DPDK·XDP/eBPF **심화(본 트랙)**는 Tr.06 개요 위에 실전을 얹습니다. RDMA(12) 직후에 배치한 UEC(13)는 RoCEv2/InfiniBand 대안으로서 같은 맥락에서 비교하며 읽는 편이 좋습니다. QUIC·TLS 0-RTT는 **심화~전문**에 가깝습니다.
...
- [ ] Tr.06 챕터 07·09(커널 바이패스·XDP/eBPF 개요)와 본 트랙 챕터 10·11(DPDK·XDP/eBPF 심화)의 난이도·목적 차이를 말할 수 있는가?
