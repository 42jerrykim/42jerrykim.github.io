...
세 기법을 처음 접하면 "양자화는 결국 정보를 버리는 것이니 정확도를 크게 희생한다"고 오해하기 쉽습니다. 하지만 실제로는 값 분포 전체를 균일하게 낮은 비트로 뭉개는 것이 문제일 뿐, [AWQ](https://arxiv.org/abs/2306.00978)처럼 활성값 크기 기준으로 중요한 채널만 선별해 보호하거나 [SmoothQuant](https://arxiv.org/abs/2211.10438)처럼 activation의 outlier를 weight 쪽으로 옮겨 스케일을 맞추면, 8비트는 물론 4비트 양자화도 원본 모델과 거의 구분되지 않는 정확도를 유지할 수 있습니다. 즉 정확도 손실은 "비트 수를 줄였기 때문"이 아니라 "무엇을 어떻게 줄였는지"에 달려 있으며, 이 구분이 02장·05장에서 반복해서 다루는 핵심 축입니다.
