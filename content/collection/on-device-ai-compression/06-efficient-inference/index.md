...
lastmod: 2026-07-17
draft: true
title: "[On-Device AI(Series) 06] Efficient Transformer 추론 — Speculative Decoding과 FlashAttention"
...
01·04장에서 다룬 Pruning이 가중치를 대상으로 했다면, **H2O(Heavy-Hitter Oracle)**는 KV Cache에 쌓인 **토큰**을 대상으로 합니다.
...
**FlashAttention**은 Attention 행렬 전체를 한 번에 느린 메모리(HBM)에 올리는 대신, **타일링(tiling)**해서 일부만 빠른 메모리(SRAM)에 올려 처리하는 **Operator Fusion**의 대표 사례입니다.
