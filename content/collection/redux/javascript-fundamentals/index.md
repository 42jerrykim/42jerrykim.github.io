...
방금 본 동작의 이유는 JavaScript가 값을 저장하는 방식이 두 가지이기 때문입니다. **원시 타입**(숫자, 문자열, 불리언, `null`, `undefined`)은 변수에 **값 자체**가 저장됩니다. 반면 **참조 타입**(객체, 배열, 함수)은 변수에 값 자체가 아니라 <strong>그 값이 저장된 메모리 위치를 가리키는 참조(reference)</strong>가 저장됩니다.
...
<strong>클로저(closure)</strong>는 함수가 자신이 정의될 때의 렉시컬 스코프(주변 변수들)를 기억해, 그 함수가 스코프 밖에서 호출되더라도 그 변수에 계속 접근할 수 있는 현상입니다.
