...
**"Redux는 보일러플레이트가 많다"는 오래된 통념 바로잡기**: 이 인식은 액션 타입 문자열, 액션 생성자, `switch` 문을 전부 손으로 써야 했던 RTK 이전 시절의 이야기입니다. 16~20편에서 다룰 Redux Toolkit의 `createSlice`는 이 반복을 대부분 없애, 코드량 면에서 Zustand와의 격차를 크게 좁힙니다. 그래서 "Redux vs Zustand"를 코드량만으로 비교하는 것은 이제 정확하지 않습니다 — 실제 차이는 코드량보다 구조의 엄격함과 생태계(DevTools, 미들웨어)에 있습니다.
