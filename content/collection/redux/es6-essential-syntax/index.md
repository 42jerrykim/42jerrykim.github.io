...
description: "Redux 리듀서 코드의 대부분은 구조 분해와 스프레드 연산자로 이루어져 있습니다. 원본을 바꾸지 않고 새 객체·배열을 만드는 이 두 문법을 정확히 익히면 08편의 불변성 원칙이 훨씬 쉬워지는 이유를 코드 예제로 다룹니다."
date: 2026-07-17
lastmod: 2026-07-17
collection_order: 2
draft: false
...
<strong>구조 분해 할당(Destructuring Assignment)</strong>은 객체나 배열에서 필요한 값만 변수로 꺼내는 문법입니다.
...
<strong>스프레드 연산자(`...`)</strong>는 객체나 배열의 내용을 펼쳐서 새로운 객체·배열에 담습니다.
...
**주의**: 스프레드는 <strong>얕은 복사(shallow copy)</strong>입니다. 중첩된 객체까지 복사하지 않습니다.
...
<strong>나머지 매개변수(Rest Parameter)</strong>는 함수가 임의 개수의 인자를 배열로 받게 해줍니다. 구조 분해의 `...rest`와 문법은 같지만 위치가 다릅니다.
