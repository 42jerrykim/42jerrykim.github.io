...
description: "21편의 미들웨어 구조 위에서 redux-thunk가 정확히 무엇을 하는지 직접 구현해봅니다. thunk 함수가 action 대신 dispatch되는 원리, getState로 조건부 dispatch하는 패턴, extraArgument로 API 클라이언트를 주입하는 방법을 다룹니다."
date: 2026-07-17
lastmod: 2026-07-17
collection_order: 22
draft: false
...
- `extraArgument`로 API 클라이언트 같은 의존성을 thunk에 주입해 테스트하기 쉽게 만들 수 있다.
...
## extraArgument: API 클라이언트를 thunk에 주입하기

지금까지의 예제는 thunk 안에서 `fetch`를 직접 호출했습니다. 하지만 실무에서는 API 호출 로직을 별도의 클라이언트 객체(인증 헤더, 재시도, 공통 에러 처리를 캡슐화한 래퍼)로 분리하는 경우가 많고, 이 클라이언트를 테스트에서는 가짜 구현으로 바꿔 끼우고 싶을 때가 있습니다. Thunk 미들웨어는 이런 상황을 위해 **세 번째 인자**를 지원합니다.

```javascript
// thunk 미들웨어 설정 시 extraArgument로 API 클라이언트를 주입
export const store = configureStore({
  reducer: rootReducer,
  middleware: (getDefaultMiddleware) =>
    getDefaultMiddleware({
      thunk: { extraArgument: apiClient }, // apiClient는 fetch를 감싼 커스텀 객체
    }),
});
```

```javascript
// thunk 함수에서 세 번째 인자로 extraArgument를 받는다
function fetchTodos() {
  return async (dispatch, getState, apiClient) => { // apiClient가 extraArgument로 주입됨
    dispatch({ type: "todos/loadStarted" });
    try {
      const todos = await apiClient.get("/todos"); // fetch 대신 주입된 클라이언트 사용
      dispatch({ type: "todos/loadSucceeded", payload: todos });
    } catch (error) {
      dispatch({ type: "todos/loadFailed", payload: error.message });
    }
...
테스트 코드에서는 실제 `apiClient` 대신 가짜 객체(`{ get: () => Promise.resolve([...]) }`)를 넘겨 네트워크 없이 thunk를 검증할 수 있습니다. `dispatch`, `getState`에 이어 `extraArgument`까지 활용하면, thunk 함수가 필요로 하는 의존성을 전부 매개변수로 받아 테스트하기 쉬운 형태로 유지할 수 있습니다.
...
- API 호출 로직을 thunk 안에 하드코딩하지 않고, `extraArgument`로 주입해 테스트에서 가짜 구현으로 교체할 수 있게 했는가?
...
- `fetchTodos`를 `extraArgument`로 주입한 `apiClient`를 쓰도록 리팩터링하고, 테스트에서 가짜 `apiClient`(`{ get: () => Promise.resolve([...]) }`)를 넘겨 실제 네트워크 없이 검증해보세요.

## 요약

- `redux-thunk`는 "dispatch된 값이 함수면 그 함수를 대신 실행한다"는 단 하나의 규칙을 가진 미들웨어다.
- Thunk 함수는 `(dispatch, getState, extraArgument)`를 받아, 그 안에서 자유롭게 비동기 로직과 조건부 dispatch를 조합하고 외부 의존성을 주입받을 수 있다.
