...
Dan Abramov는 2015년 5월 React Europe 콘퍼런스에서 발표를 준비하며 Redux를 만들었습니다. 원래 목표는 새 라이브러리를 만드는 것이 아니라, **코드를 수정해 화면을 즉시 반영하는 핫 리로딩 중에도 애플리케이션 상태를 잃지 않고, 지금까지 dispatch된 Action들을 되감아 재생(시간 여행 디버깅)할 수 있는 데모**를 보여주는 것이었습니다. 이 데모가 가능하려면 상태 변경이 예측 가능하고 재현 가능해야 했고, 그래서 Flux의 단방향 흐름에 Elm(순수 함수형 언어)의 "상태를 액션에 따라 변환하는 모델"을 결합했습니다. Flux는 여러 Store와 별도의 Dispatcher 객체를 허용했지만, Redux는 이를 **하나의 Store**로 단순화하고, 함수형 프로그래밍의 순수 함수 개념을 리듀서에 도입했습니다. 이후 Andrew Clark을 비롯한 여러 기여자가 합류하며 지금의 생태계로 발전했습니다.
...
- **"단일 소스의 진실"은 앱의 모든 상태를 Redux Store에 넣어야 한다는 뜻이다**: 그렇지 않습니다. "단일 소스"는 **공유해야 할 상태**가 한곳에 모여 있어야 한다는 뜻이지, 폼 입력값이나 토글 같은 **로컬 UI 상태**까지 전부 Redux로 옮기라는 뜻이 아닙니다. 어떤 상태를 Redux에 둘지 판단하는 구체적 기준은 10편에서 다룹니다.
- **Flux와 Redux는 사실상 같은 것이다**: 위 비교표에서 보듯 Redux는 Flux의 "단방향 흐름"이라는 아이디어만 계승했을 뿐, Dispatcher를 없애고 여러 Store를 하나로 합치는 등 구조 자체는 상당히 다릅니다.
