...
이 방식이 빠른 이유는 객체가 아무리 크고 깊어도 참조 하나만 비교하면 되기 때문입니다. 하지만 이 방식이 성립하려면 <strong>"값이 바뀌었으면 반드시 새 참조를 만들어야 한다"</strong>는 규칙을 개발자가 지켜야 합니다.
...
## 이 버그를 도구가 자동으로 잡아주는 방법

`todosReducerBad` 같은 실수는 실행해도 에러가 나지 않고 그냥 리렌더만 안 되기 때문에, 눈으로 발견하기 어렵습니다. 18편에서 다룰 `configureStore`는 이 문제를 위해 **개발 환경 전용 검사 미들웨어**(`immutableStateInvariantMiddleware`)를 기본으로 켜둡니다. 매 액션이 끝날 때마다 상태 트리를 검사해, 리듀서 밖에서(또는 리듀서 안에서 Immer 없이) 상태가 직접 변경됐는지 감지하고 콘솔에 경고를 띄웁니다.

```javascript
// configureStore로 만든 Store에서 이 코드를 실행하면(리듀서 밖에서 직접 변경)
store.getState().todos.push({ id: 99, text: "잘못된 직접 변경" });

// 개발 환경 콘솔에 다음과 비슷한 경고가 출력된다:
// "A state mutation was detected between dispatches, in the path 'todos.0'..."
```

이 검사는 상태 트리 전체를 순회해야 하므로 비용이 있어 **프로덕션 빌드에서는 자동으로 꺼집니다.** 즉 개발 중에는 이 편에서 배운 규칙을 어겼을 때 조기에 알려주는 안전망 역할을 하고, 실제 배포 성능에는 영향을 주지 않습니다. 자세한 설정은 18편에서 다시 다룹니다.
...
- 개발 환경 콘솔에 `configureStore`의 상태 변경 감지 경고가 뜨면 무시하지 않고 원인을 찾아 고치고 있는가?
