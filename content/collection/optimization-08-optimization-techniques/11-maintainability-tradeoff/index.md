...
// 핫패스였고, 프로파일링(Tr.01 방식)으로 병목이 확인된 뒤 도입했다.
...
## 맥락
Tr.01 방식 프로파일링에서 add_arrays가 p99 지연의 18%를 차지함을 확인.
...
```mermaid
flowchart TD
  measure["프로파일링으로</br>병목 확인 (Tr.01)"] --> evidence{"p99·핫패스 영향</br>근거가 있는가?"}
...
| 마이크로벤치마크에서만 좋고 엔드투엔드 지표에 안 잡힘 | — | 보류, Tr.01 재측정 |
