...
**완전한 초보자?** 이 장은 **전문** 난이도로, [01장: 추상화 비용 분석](/post/cpp-optimization/abstraction-cost/)·[10장: 인라이닝 유도 기법](/post/cpp-optimization/inlining-techniques/)·[06장: 템플릿/constexpr](/post/cpp-optimization/templates-constexpr/)을 먼저 읽었다고 가정합니다. 번역 단위(TU)·링크·동적 라이브러리가 무엇인지 알고 있어야 따라가기 쉽습니다.

**이 장의 깊이**: 이 장은 **전문**에 집중합니다. ODR·ABI·가시성 같은 경계가 왜 최적화를 막는지부터 시작해, LTO가 고치는 것/못 고치는 것, 인라이닝과 코드 크기, 버전 업그레이드·ABI 안정성까지 다룹니다. **다루지 않는 것**: 인라이닝·LTO·PGO·멀티버저닝의 도구 사용법은 Tr.02 컴파일러 트랙에서, 코루틴·소거의 세부는 Tr.08·[19장](/post/cpp-optimization/type-erasure-cost-patterns/)에서 다룹니다.
...
`collection_order` 기준으로 **이전 장**은 [C++ 실행 모델·µs 최적화 어휘](/post/cpp-optimization/cpp-execution-model-microsecond-vocabulary-fundamentals/) (챕터 16)이며, 인라이닝·TU 경계를 바로 복습하려면 [인라이닝 유도 기법](/post/cpp-optimization/inlining-techniques/) (챕터 10)을 함께 열어 두면 좋습니다. 숫자·리포트의 출처는 [Tr.02 컴파일러·빌드 최적화](/post/compiler-optimization/getting-started-compiler-build-performance-tuning/)에서 확인합니다.

다음은 **Smart Pointer 비용 기초**입니다. 소유권·참조 카운트가 핫패스 비용으로 이어지는 지점을, ABI 경계에서 본 “소거 객체가 어느 모듈에 속하는가” 논의와 연결해 정리합니다.
