...
// 비권장: s1+s2 결과를 담을 임시 string, (s1+s2)+s3 결과를 담을 임시 string이
// 각각 만들어진 뒤 최종 result로 복사/이동됨
std::string concat_bad(const std::string& s1, const std::string& s2, const std::string& s3) {
  return s1 + s2 + s3;
}

// 권장: reserve로 총 길이를 미리 확보한 뒤 append(+=)만 반복
std::string concat_good(const std::string& s1, const std::string& s2, const std::string& s3) {
  std::string result;
...
**두 방식의 차이를 실제로 격리 측정**하면 다음과 같습니다. 아래는 `concat_bad`와 `concat_good`을 같은 입력으로 반복 호출해 비교하는 Google Benchmark 코드입니다.
...
static void BM_ConcatOperatorPlus(benchmark::State& state) {
  std::string a(40, 'a'), b(40, 'b'), c(40, 'c');  // SSO 임계값 초과 → 힙 버퍼 사용
  for (auto _ : state) {
    auto r = concat_bad(a, b, c);
    benchmark::DoNotOptimize(r);
  }
}
BENCHMARK(BM_ConcatOperatorPlus);

static void BM_ConcatReserveAppend(benchmark::State& state) {
  std::string a(40, 'a'), b(40, 'b'), c(40, 'c');
  for (auto _ : state) {
    auto r = concat_good(a, b, c);
    benchmark::DoNotOptimize(r);
  }
}
BENCHMARK(BM_ConcatReserveAppend);
...
- [cppreference: std::basic_string](https://en.cppreference.com/w/cpp/string/basic_string) — std::string의 멤버 함수·reserve/append·용량 관리를 다루는 표준 라이브러리 참조 문서
- [cppreference: std::basic_string_view](https://en.cppreference.com/w/cpp/string/basic_string_view) — non-owning 뷰의 정의·substr·수명 규약을 다루는 참조 문서
