...
| 커리큘럼 | 01~19 각 챕터가 다루는 주제와, 추천 수강 순서(16→18→01→02→…→05, 이후 선택)를 말할 수 있다. |
| 선행·병행 | 프로파일링 트랙 선행, 컴파일러·메모리·동시성 트랙 병행 시점을 설명할 수 있다. |

이 확인 목록은 도입 장만의 학습 성과입니다. 각 챕터(01~19)를 마친 후에는 해당 챕터의 "평가 기준"과 "이 장에서 다룬 내용"으로 구체적인 설명·선택·리팩토링 능력을 점검하면 됩니다.
...
- 커리큘럼 표(01~19)와 각 챕터 구성 원칙(정의·원칙·예시·비교·마무리).
...
- 격리 측정 → 대안 적용 → 회귀 검증의 루프를 익히고, 01~19 챕터에서 구체적 기법을 배운다.
...
**요약**: 01~05는 "비용의 종류"(추상화, 컨테이너, 문자열, 수명, 임시)를, 06~07은 컴파일 타임·Modern C++ 기능을, 08~09는 비동기·예외를, 10~15는 호출·전달·타입 소거 비용을 다룹니다. 16·18은 기초 입문으로 01 전에 읽어도 되고, 17(ABI)은 전문, 19(Type Erasure)는 심화입니다. 필요한 챕터만 골라서 읽어도 되고, 전체를 순서대로 읽으면 언어 레벨 최적화의 지도가 그려집니다.
...
이 트랙은 "언어·라이브러리 수준에서 줄일 수 있는 비용"만 다루며, 각 챕터마다 **평가 기준**(학습 성과 목표), **판단 기준**(언제 쓸고 언제 피할지), **비판적 시각**(한계·트레이드오프), **핵심 요약**과 **다음 장** 링크를 제공합니다. 도입 장(00)에서는 범위·커리큘럼·측정 방법·활용 흐름만 정리하고, 구체적인 비용 수치와 코드는 01~19에서 다룹니다.
