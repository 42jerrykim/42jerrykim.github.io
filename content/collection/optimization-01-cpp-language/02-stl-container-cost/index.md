...
**완전한 초보자?** 이 장은 [01장: 추상화 비용 분석](/post/cpp-optimization/abstraction-cost/)의 "격리 측정" 흐름과 [16장: C++ 실행 모델·µs 최적화 어휘](/post/cpp-optimization/cpp-execution-model-microsecond-vocabulary-fundamentals/)의 캐시 라인·지역성 개념을 전제로 합니다. `std::vector`·`std::map`의 기본 사용법과 빅오(O) 표기를 안다면 충분합니다.
