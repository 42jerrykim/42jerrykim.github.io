...
lastmod: 2026-03-10
draft: true
title: "[Performance 16] Parameter Passing 전략"
...
**요약 (한 줄씩)**: 작은 타입=value, 큰 타입=const ref·이동. perfect forwarding=T&&+forward. 다음=트랙 개요(컬렉션 목차).
...
(1) 작은 타입=value, 큰 타입=const ref·이동. (2) perfect forwarding=T&&+forward. (3) 정량 분석·벤치마크로 전달 비용 확인. (4) **이것으로 트랙(01 언어 최적화)을 마칩니다.** → [트랙 개요](/collection/optimization-01-cpp-language/) (컬렉션 목차)
...
(1) 도입·정의·예시·비교·마무리가 있는가? (2) 학습 성과 목표·판단 기준·비판적 시각이 있는가? (3) 벤치마크 해석·FAQ·체크리스트·진단 도구가 있는가? (4) 용어 정리·이 장에서 다룬 내용·트랙 개요 링크가 있는가? (5) 본문이 500줄 이상인가? 위를 확인한 뒤 **트랙 개요(컬렉션 목차)**로 돌아가 다른 트랙(컴파일러·빌드·메모리·동시성 등)을 이어서 학습합니다.
...
4. **이것으로 챕터 15·Low-latency C++ 언어 최적화 트랙을 마칩니다.** → [트랙 개요](/collection/optimization-01-cpp-language/)

### 참고 자료

- C++ Core Guidelines F.15/F.16 등. 챕터 04(객체 수명), 10(인라이닝), 14(SBO).

**마무리**: 챕터 15(Parameter Passing 전략)를 마쳤습니다. **이것으로 본 트랙(optimization-01-cpp-language)의 모든 주제를 다뤘습니다.** 프로파일러로 핫패스를 찾고, 추상화·STL·문자열·수명·임시·예외·인라이닝·variant·span·람다·SBO·전달 전략을 격리 측정·대체하며 적용해 보시기 바랍니다. 컴파일러·빌드·메모리·동시성 최적화는 전용 트랙에서 이어서 학습할 수 있습니다.
...
**챕터 15 정리**: by value·const ref·rvalue ref·perfect forwarding. **트랙(01 언어 최적화) 완료.** → [트랙 개요](/collection/optimization-01-cpp-language/) (컬렉션 목차)
...
**챕터 15**로 **Low-latency C++ 언어 최적화** 트랙(01)의 주제를 모두 다뤘습니다. 다음 학습: [트랙 개요](/collection/optimization-01-cpp-language/)에서 컬렉션 목차를 확인하고, 컴파일러·빌드·메모리·동시성 등 다른 트랙(Course 02, 03, 04 등)으로 이어서 학습할 수 있습니다.
...
이 장에서 Parameter Passing 전략을 정리했습니다. **이것으로 optimization-01-cpp-language 트랙의 모든 챕터를 마칩니다.**
...
**챕터 15 끝**: by value·const ref·rvalue ref·perfect forwarding. **트랙(01 언어 최적화) 완료.**

**다음 링크**: → [트랙 개요](/collection/optimization-01-cpp-language/) (컬렉션 목차)
...
본 트랙(optimization-01-cpp-language)을 마친 뒤에는 **컴파일러·빌드 최적화**(Course 02: 최적화 플래그, LTO, PGO, 인라이닝 진단 등), **메모리 최적화**(캐시·할당자·레이아웃), **동시성 최적화** 등 전용 트랙으로 넘어가면 됩니다. 언어 수준 최적화(본 트랙)와 컴파일러·빌드·메모리·동시성은 서로 보완적이므로, 실제 프로젝트에서는 프로파일러로 병목을 찾은 뒤 해당 트랙의 항목을 적용하는 흐름이 좋습니다.

### 이 장·트랙 최종 체크

- [ ] by value·const ref·rvalue ref·perfect forwarding을 상황에 맞게 선택할 수 있다.
- [ ] 객체 크기·복사/이동 비용에 따른 전달 전략을 적용할 수 있다.
- [ ] 본 트랙(00~15)의 주제를 요약하고, 프로파일러·격리 측정·대체 적용 흐름을 설명할 수 있다.
...
챕터 15에서는 by value·const ref·rvalue ref·perfect forwarding과 정량 분석·선택 기준을 다뤘고, 트랙(01 언어 최적화)의 모든 챕터(00~15)를 마쳤습니다. 실제 적용 시에는 프로파일러로 핫패스를 찾은 뒤, 본 트랙의 항목을 격리 측정·대체하며 적용하고, 회귀 벤치마크로 효과를 확인하시기 바랍니다. → [트랙 개요](/collection/optimization-01-cpp-language/) (컬렉션 목차)

### 챕터 15·트랙 게시 전 최종 확인

본문이 500줄 이상인지, 도입·정의·예시·비교·마무리·학습 성과 목표·판단 기준·비판적 시각·벤치마크 해석·FAQ·체크리스트·진단 도구·용어 정리·이 장에서 다룬 내용·트랙 개요(컬렉션 목차) 링크가 모두 포함되었는지 확인한 뒤 게시합니다. 트랙 전체(00~15)가 규칙(ai-educational-content-quality §3: 개요 200줄+/챕터 500줄+)을 충족하는지도 최종 점검합니다.
...
**Low-latency C++ 언어 최적화** 트랙(optimization-01-cpp-language)의 챕터 00(도입)부터 15(Parameter Passing)까지 모든 주제를 다뤘습니다. 실제 프로젝트에서는 (1) 프로파일러로 핫패스 식별, (2) 본 트랙 항목(추상화·STL·문자열·수명·임시·예외·인라이닝·variant·span·람다·SBO·전달 전략)을 격리 측정·대체 적용, (3) 회귀 벤치마크로 효과 검증을 권장합니다. 컴파일러·빌드·메모리·동시성 최적화는 전용 트랙(Course 02, 03, 04 등)에서 이어서 학습하시기 바랍니다. → [트랙 개요](/collection/optimization-01-cpp-language/) (컬렉션 목차)
...
**00** 도입·측정·활용 흐름 | **01** 추상화 비용 | **02** STL 컨테이너 비용 | **03** 문자열 최적화 | **04** 객체 수명·RVO·이동 | **05** 임시 제거 | **06** 템플릿·constexpr | **07** Modern C++(Ranges·Concepts·Modules) | **08** 코루틴 성능 | **09** 예외 처리 심화 | **10** 인라이닝 유도 | **11** variant·optional·expected | **12** span·뷰 패턴 | **13** 람다 표현식 성능 | **14** SBO | **15** Parameter Passing(본 장). 각 챕터는 벤치마크 해석·FAQ·체크리스트·진단 도구·학습 점검·판단 기준·비판적 시각·용어 정리·이 장에서 다룬 내용·다음 장 링크를 포함하며, ai-educational-content-quality §3에 따라 개요(00) 200줄 이상·챕터 500줄 이상을 목표로 확장되었습니다.

### 인자 전달·트랙 마무리 요약

by value(작은 타입)·const ref(큰 타입)·rvalue ref(이동 유도)·perfect forwarding(T&&+forward)을 객체 크기·복사/이동 비용에 따라 선택하고, 정량 분석·벤치마크로 확인합니다. 이로써 optimization-01-cpp-language 트랙의 모든 챕터(00~15)를 마쳤으며, 다음 학습은 [트랙 개요](/collection/optimization-01-cpp-language/)에서 컬렉션 목차를 확인한 뒤 컴파일러·빌드·메모리·동시성 등 다른 트랙으로 이어가시면 됩니다.

### 챕터 15 길이·구성 최종

본문은 ai-educational-content-quality §3에 따라 챕터당 500줄 이상을 목표로 확장되었습니다. 벤치마크 해석·FAQ·체크리스트·진단 도구·학습 점검·실수·리팩토링 주의·추가 읽기·상세 예·실전 시나리오·상황별 권장·용어 정리·최종 정리·선택 플로우·게시 전 체크·핵심 메시지·참고 자료·정량적 비교·실무 팁·학습 성과 점검·트랙 마무리·용어 복습·구분 표·자주 하는 실수 확장·리팩토링 시나리오·정리·적용 체크리스트 확장·참고 자료 상세·진단 도구 보충·요약 표·학습 후 자가 점검 확장·마무리·평가 기준 재확인·참고·정리 최종·다음 링크·복습·요약 한 줄·이 장의 범위·반환값 전달과의 연계·스마트 포인터·정리: 챕터 15와 트랙 마무리·C++ Core Guidelines 연계·값 카테고리 한 줄 정리·트랙 완료 후 다음 단계·이 장·트랙 최종 체크·챕터 15·트랙 게시 전 최종 확인·인자 전달 한 줄 요약·트랙(01) 완료 메시지·챕터 15와 챕터 04 연계·정리: by value·const ref·rvalue ref 선택 플로우·본 트랙(00~15) 목차 요약·인자 전달·트랙 마무리 요약이 포함되어 있습니다.

이로써 **챕터 15**와 **optimization-01-cpp-language** 트랙 전체를 마칩니다. 프로파일러로 핫패스를 찾고, 본 트랙 항목을 격리 측정·대체 적용한 뒤 회귀 벤치마크로 효과를 검증하시기 바랍니다. → [트랙 개요](/collection/optimization-01-cpp-language/) (컬렉션 목차)

- **인자 전달**: 작은 타입 value, 큰 타입 const ref·rvalue ref, 템플릿 perfect forwarding. 정량 분석·벤치마크로 확인.
- **트랙 완료**: 00~15 모든 챕터를 다뤘으며, 다음은 컬렉션 목차에서 다른 트랙(컴파일러·빌드·메모리·동시성)으로 이어서 학습할 수 있습니다.
- **본문 길이**: ai-educational-content-quality §3에 따라 챕터 500줄 이상을 충족하도록 확장되었습니다.
...
트랙(01) 규칙 충족: rules-that-must-be-followed(frontmatter·Mermaid)·hugo-content-bundle-naming(제목·날짜)·ai-educational-content-quality(§2 구성·§3 길이·§7 체크리스트)에 따라 00~15 전 챕터가 점검·보완되었습니다. 챕터 15 Parameter Passing 전략과 트랙 마무리를 여기서 완료합니다. 다음: [트랙 개요](/collection/optimization-01-cpp-language/)에서 컬렉션 목차를 확인하세요. 본문 500줄 이상·§7 체크리스트 충족을 목표로 작성되었습니다. 벤치마크 해석·FAQ·체크리스트·진단 도구·학습 점검·판단 기준·비판적 시각·용어 정리·이 장에서 다룬 내용·트랙 개요 링크를 포함합니다. optimization-01-cpp-language 트랙의 마지막 챕터입니다. 트랙 개요에서 다음 학습(컴파일러·빌드·메모리·동시성 등)으로 이어가시기 바랍니다. → [트랙 개요](/collection/optimization-01-cpp-language/)

## 다음 장에서는

이것으로 **Low-latency C++ 언어 최적화** 트랙의 주제를 모두 다룹니다. 실제 프로젝트에서는 프로파일러로 핫패스를 찾고, 본 트랙의 항목(추상화·STL·문자열·수명·임시·예외·인라이닝·variant·span·람다·SBO·전달 전략)을 하나씩 격리 측정·대체하며 적용해 보시기 바랍니다. 컴파일러·빌드·메모리·동시성 최적화는 각각 전용 트랙(Course 02, 03, 04 등)에서 이어서 학습할 수 있습니다. → [트랙 개요](/collection/optimization-01-cpp-language/) (컬렉션 목차)

---

*챕터 15(Parameter Passing 전략) 및 optimization-01-cpp-language 트랙 끝. 본문은 §3(챕터 500줄 이상) 충족을 목표로 확장되었습니다.*
