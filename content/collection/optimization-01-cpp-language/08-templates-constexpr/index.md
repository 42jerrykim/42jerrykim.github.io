...
**Q: 템플릿이 비용을 줄이나요?**  
A: 템플릿은 인라이닝·타입별 최적화를 유도해 런타임 비용을 줄일 수 있지만, 인스턴스 수가 늘면 컴파일 시간·코드 크기가 늘 수 있습니다. 인스턴스 수·컴파일 시간 증가는 `-ftime-trace`(Clang) 또는 바이너리 심볼 수로 확인할 수 있습니다.
...
- [cppreference: constexpr specifier](https://en.cppreference.com/w/cpp/language/constexpr) — constexpr 함수·변수의 정확한 규칙(상수 표현식 요건, 완화 이력)을 다루는 1차 출처.
- [cppreference: consteval specifier](https://en.cppreference.com/w/cpp/language/consteval) — C++20 consteval의 즉시 함수(immediate function) 의미론과 제약을 다루는 1차 출처.
