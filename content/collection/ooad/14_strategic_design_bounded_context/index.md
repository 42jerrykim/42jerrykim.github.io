...
이 세 가지 의미를 억지로 하나의 `Order` 클래스에 담으면, 영업팀 요구사항으로 필드를 추가할 때마다 물류팀 로직이 영향받을 위험이 생깁니다. **바운디드 컨텍스트(Bounded Context)**는 이 문제에 대한 답입니다. "주문"이라는 단어가 하나의 일관된 의미로 통용되는 범위를 명시적으로 정하고, 그 범위 밖에서는 같은 단어라도 다른 모델(다른 클래스, 다른 필드)로 취급합니다.
...
- 다음: [15. 전술적 설계: 엔티티와 밸류 오브젝트](../15_tactical_design_entity_value_object/)
