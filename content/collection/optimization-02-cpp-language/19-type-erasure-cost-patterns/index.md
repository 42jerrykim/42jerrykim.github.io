...
**CRTP(Curiously Recurring Template Pattern)**는 기반 클래스가 파생 타입을 템플릿 인자로 받아 **정적 다형**을 구현합니다. 런타임 간접 호출을 피할 수 있지만, **파생 타입마다 코드가 복제**될 수 있습니다. 작은 핫 함수에만 쓰고 나머지는 가상으로 두는 **하이브리드**도 흔합니다. 바이너리 크기가 I-cache를 압박하면 Tr.03·챕터 12에서 다루는 **코드 크기 vs 인라인** 균형으로 되돌아갑니다.
