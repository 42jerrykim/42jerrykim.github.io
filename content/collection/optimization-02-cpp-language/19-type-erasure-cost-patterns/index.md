...
<strong>Type erasure(타입 소거)</strong>는 구체 타입 정보를 런타임에 한 겹 감추고, **공통 인터페이스(호출 연산)** 만 노출하는 기법입니다. `std::function`, `std::any`, 많은 커스텀 “다형 콜백” 래퍼가 이 패턴을 씁니다. Low-latency 관점에서는 **간접 호출 한 번**, **SBO 한도를 넘을 때의 힙 할당**, **인라인 실패**가 한데 묶여 나타나기 쉬워, “한 가지 요인”으로 쪼개어 측정하는 것이 중요합니다.
...
위 `CallableErased`는 힙 할당과 가상 디스패치를 둘 다 씁니다. 대안으로 <strong>고정 크기 버퍼(SBO)</strong>에 호출 객체를 직접 담고, **가상 함수 대신 일반 함수 포인터**(`invoke`/`destroy`)로 연산을 표현하면 힙 할당과 vtable 로드를 피할 수 있습니다. 버퍼에 들어가는 작은 호출 객체에 한정되지만, 핫패스에서 자주 쓰는 콜백에 효과적입니다.
...
`SboCallable`은 `F`가 **trivially copyable일 것을 요구하지 않습니다**. `SboCallable` 자신의 복사·이동 생성자가 `= delete`(217~218행)이므로, `F`를 옮기는 유일한 지점은 생성자의 placement-new 한 번뿐이고 소멸도 `destroy_`가 명시적으로 `F::~F()`를 호출해 처리합니다. 즉 `sizeof(F) <= kBuf`(버퍼 크기)만 만족하면 되고, `F`의 복사 가능성 자체는 이 설계의 안전성과 무관합니다. 반대로 `SboCallable`을 복사 가능하게 확장하려면(예: 프로토타입 패턴) 그때는 `F`의 복사 생성자를 버퍼 대 버퍼로 호출하는 별도 함수 포인터(`clone_`)가 필요해집니다.
...
<strong>CRTP(Curiously Recurring Template Pattern)</strong>는 기반 클래스가 파생 타입을 템플릿 인자로 받아 **정적 다형**을 구현합니다. 런타임 간접 호출을 피할 수 있지만, **파생 타입마다 코드가 복제**될 수 있습니다. 작은 핫 함수에만 쓰고 나머지는 가상으로 두는 **하이브리드**도 흔합니다. 바이너리 크기가 I-cache를 압박하면 Tr.03·챕터 12에서 다루는 **코드 크기 vs 인라인** 균형으로 되돌아갑니다.
...
- [cppreference: std::function](https://en.cppreference.com/w/cpp/utility/functional/function) — `std::function`의 SBO·힙 전환·호출 규약을 정리한 1차 레퍼런스입니다.
- [cppreference: std::any](https://en.cppreference.com/w/cpp/utility/any) — `std::any`의 저장 방식과 `any_cast` 동작을 정리한 1차 레퍼런스입니다.

## 트랙 마무리와 다음 학습

이것으로 **Low-latency C++ 언어 최적화** 트랙(optimization-02-cpp-language)의 챕터 **00~19** 주제를 모두 다룹니다. 커리큘럼·측정 방법·선행 순서는 도입 장에서 정리되어 있으므로, 로드맵을 다시 보고 싶다면 아래로 돌아가면 됩니다.
