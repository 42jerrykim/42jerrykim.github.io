...
| 수준 | 읽을 부분 | 핵심 목표 |
|------|---------|---------|
| **초보자** | "Ranges (C++20)" ~ "Concepts (C++20)" | 기능의 목적과 런타임 비용 유무 이해 |
| **중급자** | "Modules (C++20)" ~ "기타 C++17/20/23 기능 (성능 관점)" | 핫패스에 쓸지 측정으로 판단 |
| **전문가** | "판단 기준" ~ "비판적 시각" | 표현력과 성능의 트레이드오프 선택 |
...
Ranges 라이브러리는 <strong>범위(range)</strong>를 인자로 두고, <strong>뷰(view)</strong>를 파이프 `|`로 연결해 연산을 표현합니다. 많은 뷰는 **지연 평가**되어 최종적으로 소비(순회·수집)될 때만 연산이 실행됩니다. 뷰는 대체로 **복사 없이** 원본 범위를 참조하므로 중간 컨테이너 할당을 줄일 수 있습니다.
...
- **if constexpr**: 컴파일 타임에 분기가 결정되므로 선택되지 않은 경로의 코드는 생성되지 않고 런타임 분기도 없습니다. 템플릿 안에서 타입·상수에 따라 다른 구현을 택할 때 유용합니다(챕터 08 참고).
...
<strong>"concepts를 쓰면 실행 속도가 빨라진다"</strong>도 흔한 오해입니다. concepts는 **컴파일 타임 제약**일 뿐이며, 제약을 만족하는 타입에 대해서는 concepts가 없을 때의 일반 템플릿과 **동일한 코드**가 생성됩니다. concepts가 주는 이득은 런타임 속도가 아니라 컴파일 타임 오류 메시지와 오버로드 해석의 명확성입니다.
...
이 장은 C++17/20/23 범위를 다루지만, C++26이 2026-03-29 런던 회의에서 **최종 확정**됐다는 사실은 짚고 넘어갈 가치가 있습니다. 성능과 직접 관련된 확정 기능으로는 <strong>정적 리플렉션(P2996)</strong>과 **Contracts**([[pre:]]/[[post:]])가 있으며, 둘 다 GCC 16.1(2026-04)에서 구현이 시작됐고 Clang은 아직 실험 포크 수준입니다. 즉 "예정된 기능"이 아니라 "확정된 표준, 구현은 초기 단계"로 이해하는 것이 정확합니다. 이 두 기능이 컴파일 타임 코드 생성·검증 비용에 미치는 영향은 컴파일러 지원이 성숙한 뒤 별도로 다룰 만한 주제입니다.

---

## 다음 장에서는

**이전 장**: [템플릿/constexpr](/post/cpp-optimization/templates-constexpr/) (챕터 08)

**코루틴 성능**을 다룹니다. C++20 코루틴의 suspend/resume 비용, 프레임 할당, 저지연 경로에서의 활용 기준을 정리합니다. → [코루틴 성능](/post/cpp-optimization/coroutine-performance/) (챕터 10)
