...
**이 장의 깊이**: 이 장은 **중급–전문가**를 포괄합니다. 각 컨테이너의 메모리 레이아웃·비용 모델을 정리하는 것부터 시작해, 전문가 구간에서는 `vector` vs `list` 순회, `map::find` vs 정렬 `vector`의 `lower_bound`를 마이크로벤치마크로 비교하고 캐시 효율로 해석합니다. **다루지 않는 것**: 커스텀 할당자·메모리 풀 설계(Tr.04 메모리·할당 트랙)와 동시 접근 시 락 비용(Tr.07 동시성 트랙)입니다.

## 당신의 수준에 맞는 경로

| 수준 | 읽을 부분 | 핵심 목표 |
|------|---------|---------|
| **초보자** | "vector" ~ "C++26 신규 컨테이너" | 컨테이너별 메모리 레이아웃·비용 모델 이해 |
...
## C++26 신규 컨테이너: std::inplace_vector·std::hive

C++26에는 이 챕터의 "힙 회피"라는 주제와 직접 맞닿는 컨테이너 두 개가 새로 표준화되었습니다.

**std::inplace_vector&lt;T, N&gt;**(P0843, 舊 `boost::static_vector`)은 **컴파일 타임에 고정된 최대 용량 N**을 객체 내부 버퍼에 그대로 담는 vector입니다. 요소 수가 N을 넘지 않는 한 **힙 할당이 전혀 없습니다** — 위에서 다룬 vector의 재할당 문제 자체가 사라지는 대신, N을 넘으면 `push_back`은 예외를 던지고 `unchecked_push_back`은 정의되지 않은 동작이 됩니다. 용량 상한을 미리 아는 핫패스(예: 필드 수가 고정된 프로토콜 파싱, 요청당 최대 주문 개수가 정해진 주문 처리)에 적합합니다.

```cpp
#include <inplace_vector>  // GCC 16.1+ (2026-07 기준 Clang·MSVC 미구현)

std::inplace_vector<int, 16> buf;  // 스택/멤버 안에 16개 슬롯, 힙 없음
buf.push_back(1);
buf.push_back(2);
// buf.size() == 2, buf.capacity() == 16 (항상 고정, 재할당 없음)
```

이 표준 채택은 2024년 확정되었고, GCC 16.1(2026-04 릴리스)에서 이미 구현되어 있습니다. 지금 당장 쓸 수 있는 컴파일러가 제한적이므로, 새 프로젝트에서는 `boost::static_vector`로 같은 패턴을 먼저 적용해 두고 표준 라이브러리 지원이 성숙하면 교체하는 편이 현실적입니다.

**std::hive**(P0447, 舊 `std::colony`/`plf::colony`)는 삽입·삭제가 빈번하면서도 **기존 원소의 포인터·참조·이터레이터가 삭제 시점까지 안정적으로 유지**되어야 하는 워크로드(객체 풀, 엔티티 컴포넌트 시스템, 이벤트 큐)를 위한 버킷 기반 컨테이너입니다. 표준 채택은 확정됐지만 2026-07 기준 GCC·Clang·MSVC 어디에도 구현이 없어, 아직은 이름과 용도만 알아 두는 수준입니다. 지금 이 패턴이 필요하면 원형 라이브러리인 `plf::hive`를 대신 검토합니다.
...
x86-64 데스크톱(예: GCC 13, `-O2`) 환경에서 N=100,000 순회는 vector가 list보다 대략 5–10배 빠르게 나오는 경우가 많습니다(예시 수치: vector 약 30µs, list 약 200µs). 단, **측정값은 CPU 세대·컴파일러·플래그에 따라 다름**이므로 반드시 대상 환경에서 확인합니다.

## 벤치마크 2: map::find vs 정렬 vector lower_bound

키로 조회만 하고 삽입이 드문 경우, 정렬된 `vector`에서 `std::lower_bound`로 이분 탐색하면 연속 메모리 덕분에 `map::find`보다 빠를 수 있습니다(특히 작은–중간 N). 아래 두 벤치마크는 같은 키 집합을 `map`과 정렬된 `vector`에 각각 담아 두고, 동일한 조회 패턴(키를 2씩 증가시키며 순환 조회)으로 두 구조의 조회 비용만 분리해 비교합니다.
...
N이 작거나 중간일 때(수천 이하) 정렬 vector가 map보다 빠른 경우가 흔합니다(x86-64, GCC 13, `-O2` 기준 예시 수치: `lower_bound` 약 30ns, `map::find` 약 60ns). 삽입·삭제가 빈번해 정렬 유지 비용이 커지면 map/set이 적합하므로, **측정값은 CPU 세대·컴파일러·플래그에 따라 다름**을 전제로 대상 워크로드에서 비교합니다.
...
- [ ] 용량 상한이 고정된 핫패스라면 std::inplace_vector(또는 boost::static_vector)로 힙을 아예 없앨 수 있는지 검토했는가?

### 더 읽을 거리

- [cppreference: std::vector](https://en.cppreference.com/w/cpp/container/vector) — 성장 정책·복잡도 보장 상세.
- [cppreference: std::map](https://en.cppreference.com/w/cpp/container/map) — 정렬 연관 컨테이너의 복잡도 보장.
- [cppreference: std::unordered_map](https://en.cppreference.com/w/cpp/container/unordered_map) — 해시 테이블 복잡도·load_factor.
- [cppreference: std::inplace_vector](https://en.cppreference.com/w/cpp/container/inplace_vector) — C++26(P0843), 고정 용량·힙 없는 vector.
- [cppreference: std::hive](https://en.cppreference.com/w/cpp/container/hive) — C++26(P0447), 이터레이터 안정성을 유지하는 버킷 기반 컨테이너.
