...
시스템을 점진적으로 옮기는 방법을 정했다면, 그다음 질문은 "옮긴 뒤에도 아키텍처가 의도한 형태를 유지하고 있는지 어떻게 계속 확인할 것인가"다. Neal Ford, Rebecca Parsons, Patrick Kua는 2017년 저서 『Building Evolutionary Architectures』에서 이 확인 장치를 **적합도 함수(fitness function)**라 이름 붙였다.
...
기술 부채가 "설계 결정의 위험"을 다룬다면, **오류 예산(error budget)**은 "운영 중 신뢰성의 위험"을 다루는 도구다. Google의 사이트 신뢰성 엔지니어링(SRE) 팀은 2016년 출간한 『Site Reliability Engineering』에서, 서비스의 신뢰성 목표(SLO)를 100%가 아니라 99.9% 같은 값으로 정하고, 그 차이(0.1%)를 "이번 분기에 써도 되는 장애 예산"으로 취급하는 방식을 제안했다.
