...
lastmod: 2026-07-17
draft: true
title: "[Vision AI(Series) 05] Object Detection — R-CNN에서 DETR까지"
...
예측한 박스와 정답 박스(Ground Truth)가 얼마나 겹치는지는 **IoU(Intersection over Union)**로 잽니다.

$$\text{IoU} = \frac{\text{교집합 영역}}{\text{합집합 영역}}$$

완벽히 일치하면 1, 전혀 안 겹치면 0입니다. 탐지가 "성공"했다고 인정하려면 두 가지 임계값을 모두 통과해야 합니다 — Confidence Score가 일정 기준(예: 0.5) 이상이어야 "물체가 있다"고 인정하고, IoU가 일정 기준(예: 0.5) 이상이어야 "제대로 찾았다"고 인정합니다. 이를 모두 만족해야 **TP(True Positive)**로 집계됩니다. Object Detection에서는 물체가 없는데 없다고 판단한 경우(TN)는 의미가 없어 다루지 않고, TP·**FN(놓침)**·**FP(잘못 찾음)** 세 가지만 계산합니다.
...
**Faster R-CNN**은 Selective Search를 GPU에서 처리 가능한 **RPN(Region Proposal Network)**으로 대체합니다.
...
**DETR(DEtection TRansformer)**은 기존 Detector들의 복잡한 후처리(Anchor 설계, NMS)를 없애고, Object Detection을 **집합(Set) 예측 문제**로 재정의합니다.
