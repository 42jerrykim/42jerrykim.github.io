...
**"디스크가 항상 병목이다"**도 더 이상 항상 참이 아닙니다. NVMe SSD의 랜덤 읽기 지연이 수십 µs대까지 내려온 환경에서는, 오히려 커널 경로의 CPU 카피나 시스템 콜 진입 비용이 전체 지연에서 차지하는 비중이 상대적으로 커질 수 있습니다. 병목이 저장 장치인지 커널 경로인지는 추측이 아니라 [9장: 블록 디바이스 최적화](/post/io-optimization/block-device-nvme-ssd-io-scheduler-optimization/) 수준의 측정으로 확인해야 합니다.

**"페이지 캐시를 우회하면 항상 더 빠르다"**는 것도 흔한 오해입니다. 캐시에 이미 올라온 데이터를 다시 읽는 경우 버퍼링은 디스크 접근 자체를 없애 주는 순이득이며, `O_DIRECT`로 페이지 캐시를 우회하면 그 이득을 포기하는 대신 애플리케이션이 직접 캐싱·정렬 정책을 책임져야 합니다. 이 트레이드오프는 [7장: Direct I/O](/post/io-optimization/direct-io-o-direct-page-cache-bypass/)에서 다룹니다.
...
| 흩어진 여러 버퍼를 한 번의 시스템 콜로 읽거나 쓰고 싶다 | [11장: Vectored I/O](/post/io-optimization/vectored-io-readv-writev-preadv2-pwritev2/) | readv/writev로 시스템 콜 횟수 자체를 줄이는 방법 |
...
**다음 장에서는** 이 장에서 세운 "시스템 콜과 복사는 공짜가 아니다"라는 감각을 핫패스 로깅이라는 구체적인 사례에 적용합니다. 로그 한 줄을 남기는 것도 결국 버퍼에 쓰고 언젠가 `write`(또는 `fsync`)를 호출하는 일이므로, 이 장에서 그린 복사·시스템 콜 비용 그림이 그대로 적용되며, 여기에 로그 레벨 분리와 비동기 로거 설계라는 실전 판단이 더해집니다. 이 장의 "감 잡기"에서 더 정밀한 비용 모델로 바로 넘어가고 싶다면 [1장: I/O 패턴과 비용](/post/io-optimization/io-patterns-blocking-nonblocking-cost-model/)으로 진행해도 좋습니다.

→ [로깅 성능 전략: 비동기 로거](/post/io-optimization/logging-performance-strategy-async-logger/) (챕터 17)
