...
이 장이 다루지 않는 것은 다음과 같다. `git add`·`git rebase`처럼 개별 명령의 옵션과 예시는 01장 이후 각 번호 챕터에서 다룬다. GitHub·GitLab 같은 특정 호스팅 서비스의 UI 조작법(이슈 트래커, 프로젝트 보드 등)은 이 컬렉션의 범위 밖이며, Git 자체의 명령과 개념(원격 저장소, Pull Request가 전제하는 워크플로)만 다룬다. `git daemon`·Smart HTTP 서버 설정처럼 **자체 Git 서버를 직접 구축·운영하는 절차**도 범위 밖이다 — 이 컬렉션은 개발자가 이미 존재하는 원격 저장소(GitHub·GitLab·사내 서버 무관)를 사용하는 관점에서 설계되었고, 서버 인프라 구축은 별도의 운영 주제이기 때문이다. 반대로 alias(02장)와 HTTPS/SSH 인증에 쓰는 credential helper(17장)는 클라이언트 설정 범위에 포함되므로 각 해당 챕터에서 다룬다.
...
완주 시간과 실무 투입 시점 사이에도 트레이드오프가 있다. 45개 챕터를 순서대로 정독하면 3단계 영역 모델부터 내부 객체 모델까지 빠짐없이 쌓이지만, 당장 다음 주 배포에 merge 충돌만 해결하면 되는 상황이라면 전체를 기다릴 여유가 없다. 이 컬렉션은 그래서 "당신의 수준에 맞는 경로" 표를 앞에 배치했다 — 급한 문제 해결이 필요하면 해당 Part로 바로 건너뛰고, 여유가 있을 때 나머지 Part로 돌아와 커리큘럼 순서의 이점(개념이 누락 없이 쌓이는 것)을 챙기는 절충이 가능하다. 즉 순서를 지키는 것과 필요한 부분만 골라 읽는 것은 상호 배타적이지 않고, 이 컬렉션의 목차 자체가 두 방식을 모두 지원하도록 설계되었다.
...
## 이 컬렉션을 완주한 후 달성해야 할 목표

이 45개 챕터를 모두 읽고 나면 독자는 다음을 스스로 판단할 수 있어야 한다. 먼저 스냅샷 모델과 diff 기반 모델의 차이를 설명하고, 이 차이가 `git log -p`나 `git diff`의 동작 방식에 왜 영향을 주는지 근거를 들어 답할 수 있어야 한다. 둘째, 주어진 상황에서 `merge`와 `rebase` 중 어느 쪽을 골라야 하는지 판단 기준(공유 브랜치인지, 히스토리를 선형으로 유지할 필요가 있는지)을 근거로 결정할 수 있어야 한다. 셋째, 실수로 커밋을 잃어버렸거나 원치 않는 상태로 되돌아갔을 때 `reflog`·`reset`·`revert`·`restore` 중 상황에 맞는 도구를 골라 복구할 수 있어야 한다. 넷째, blob·tree·commit·refs로 이어지는 내부 객체 모델을 근거로 `git gc`나 packfile이 왜 필요한지, 브랜치 생성이 왜 가벼운지를 설명할 수 있어야 한다. 다섯째, hooks·submodule·worktree 같은 확장 기능이 언제 팀의 협업 워크플로에 필요한지, 그리고 언제 오히려 복잡성만 늘리는지를 비판적으로 판단할 수 있어야 한다. 이 다섯 가지가 이 컬렉션이 목표로 삼는 최종 역량이며, 각 항목은 위 목차의 특정 Part(순서대로 2부, 3부, 5부, 7부, 8부)에 대응한다.
