...
draft: false
image: wordcloud.png
title: "[Network 12] Zero-copy 직렬화"
...
**선행 챕터**: [05장: 직렬화 성능 비교](/post/network-optimization/serialization-performance-protobuf-flatbuffers-capnproto/)에서 Protocol Buffers·FlatBuffers·Cap'n Proto의 처리량·메시지 크기 비교를 다뤘습니다. 이 장은 그 비교의 "왜"에 해당하는 FlatBuffers와 Cap'n Proto의 내부 설계를 파고듭니다. 숫자 비교가 필요하면 05장을 먼저 참고하세요.

**전제 지식**: C/C++에서 포인터와 오프셋의 차이, 구조체 메모리 레이아웃, 그리고 05장에서 다룬 "직렬화 비용은 CPU와 대역폭의 트레이드오프"라는 감각이 있으면 충분합니다.

**이 장의 깊이**: **심화** 수준입니다. FlatBuffers의 vtable 기반 필드 접근과 Cap'n Proto의 세그먼트·아레나 설계를 원리 수준에서 다루고, 두 설계가 공통으로 의존하는 "상대 오프셋"이라는 아이디어를 정리합니다. **다루지 않는 것**: 포맷별 처리량·지연시간의 정량 비교(→ 05장), Yandex YaFF 등 2026년 신흥 zero-copy 포맷과의 판단 기준(→ [07장](/post/network-optimization/next-gen-zero-copy-serialization-formats-yaff/)), 메시지 경계를 정하는 프레이밍 전략(→ [09장](/post/network-optimization/message-framing-length-prefix-delimiter-fixed-size/)).
...
**"zero-copy는 복사가 전혀 없다는 뜻이다"**는 정확하지 않습니다. 소켓에서 커널 버퍼로, 커널 버퍼에서 사용자 공간으로 옮기는 복사(recv 계열 시스템 콜)는 여전히 발생합니다. zero-copy 직렬화가 없애는 것은 "수신한 바이트를 별도의 파싱된 객체 트리로 다시 복사하는" 단계입니다. 전송 경로의 복사를 더 줄이려면 이 트랙의 커널 바이패스·io_uring 관련 챕터에서 다루는 기법이 필요합니다.

**"zero-copy 포맷은 항상 더 빠르다"**도 워크로드에 따라 깨집니다. vtable 조회나 포인터 역참조는 상수 비용이지만 0은 아니며, 메시지의 필드를 전부 순회해야 하는 워크로드에서는 이 간접 참조 비용이 누적되어 단순 구조체 그대로를 복사하는 것보다 느릴 수 있습니다. 포맷 간 실제 처리량 차이는 05장에서 다룬 벤치마크로 확인하는 것이 정확합니다.

**"버퍼를 그대로 신뢰해도 된다"**는 신뢰할 수 없는 입력에서 가장 위험한 오해입니다. FlatBuffers 오프셋이나 Cap'n Proto 포인터가 조작되면, 검증 없이 접근할 때 버퍼 범위 밖을 읽을 수 있습니다. FlatBuffers는 `Verifier` 클래스로 모든 오프셋·크기·문자열 종료를 사전 검사하는 기능을 제공하고, Cap'n Proto도 접근자 코드가 포인터를 따라가기 전에 유효성을 검사해 잘못된 포인터를 예외나 기본값으로 처리합니다. 다만 두 라이브러리 모두 "구조적 검증"만 하지, 필드 값의 의미(예: 나이가 음수인지)는 애플리케이션이 별도로 검증해야 합니다.
...
- 메시지 크기와 필드 접근 패턴을 05장 벤치마크 방식으로 직접 재현했는가?
...
**이전 장**: [직렬화 성능 비교](/post/network-optimization/serialization-performance-protobuf-flatbuffers-capnproto/) (챕터 05)
...
→ [차세대 Zero-copy 직렬화 포맷 동향](/post/network-optimization/next-gen-zero-copy-serialization-formats-yaff/) (챕터 07)
