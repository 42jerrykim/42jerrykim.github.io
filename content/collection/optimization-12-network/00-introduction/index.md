...
**Phase B — 직렬화·프로토콜 (챕터 05~08, 12, 15~17, 19)** 메시지 설계는 CPU(Tr.06), 언어 표현(Tr.01), 할당·레이아웃(Tr.03)과 만나는 지점입니다. 압축(19)까지 포함해 CPU-대역폭 trade-off를 함께 봐야 합니다.
...
커널 경로와 운영환경 개요가 아직 약하다면 **Tr.07**을 먼저 읽고 돌아오는 편이 안전합니다. 파일·디스크 병목과 함께 보아야 한다면 **Tr.11** 도입을 병행하세요.

- [Tr.07 Introduction: OS·런타임 Low-latency 운영환경](/collection/optimization-07-os-runtime/00-introduction/)
- [Tr.11 Introduction: Low-latency I/O 최적화](/collection/optimization-11-io-network/00-introduction/)
