...
디스크와의 데이터 이동은 시스템 콜·커널 경로·복사 횟수·페이지 캐시 정책이 겹칩니다. Tr.06에서 syscall과 io_uring **개요**를 본 뒤, 이 트랙에서는 **파일·블록·DB 패턴**으로 내려가 실전 튜닝을 합니다. 동일한 “비동기 I/O”라도 워크로드에 따라 이득이 크게 달라지므로, 처리량과 지연 분포를 **분리 측정**하는 습관이 필요합니다.
