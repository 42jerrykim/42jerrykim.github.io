...
이 장이 다루지 않는 것은 다음과 같다. `cd`·`ls`처럼 개별 명령어의 옵션과 예시는 [01장: cd, pwd](/post/bashshell/cd-pwd-change-directory-linux-commands/) 이후 각 번호 챕터에서 다룬다. 파이프라인·리다이렉션 같은 개념의 상세 동작 원리는 3부(19–22장)에서, 스크립트 작성 문법은 5부(27–35장)에서 각각 독립된 챕터로 다룬다. 이 장에서는 "왜 이런 순서로 배워야 하는가"와 "각 Part가 최종 역량에 어떻게 기여하는가"만 설명한다. 이 커리큘럼은 상호작용형 셸 사용과 스크립팅 기초에 범위를 한정한다 — 정기 작업 스케줄링(`cron`/`crontab`)이나 사용자 권한 전환(`sudo`/`su`)처럼 배포판·조직 정책마다 설정 방식이 크게 갈리는 시스템 관리 주제는 이 컬렉션의 범위 밖이며, 별도 시스템 관리 컬렉션에서 다룰 주제로 남겨둔다.
...
셸을 배우는 일은 특정 운영체제의 특기가 아니라 소프트웨어 개발 전반에 걸쳐 반복적으로 요구되는 기초 역량이다. 첫째, 자동화의 최소 단위가 셸이다. CI/CD 파이프라인의 빌드 스크립트, 배포 스크립트, `cron`이 정기적으로 실행시키는 배치 스크립트 대부분은 결국 셸 스크립트 위에서 돌아간다(`cron` 자체의 설정 문법은 이 컬렉션의 범위 밖이지만, `cron`이 실행하는 스크립트를 안정적으로 작성하는 능력은 5부에서 다룬다). 이를 이해하지 못하면 파이프라인이 실패했을 때 로그 한 줄도 스스로 해석할 수 없다. 둘째, 서버·컨테이너·클라우드 인스턴스 같은 원격 시스템을 운영하는 유일한 공통 인터페이스가 셸이다. GUI가 없는 리눅스 서버에 SSH로 접속했을 때 파일을 옮기고 프로세스를 죽이고 로그를 뒤지는 능력은 대체할 도구가 없다. 셋째, 텍스트 처리는 프로그래밍 언어를 가리지 않는 보편 기술이다. 로그 파일에서 에러 줄만 뽑아내고, CSV의 특정 열만 추출하고, 수백 개 파일에서 문자열을 한 번에 바꾸는 작업은 `grep`·`awk`·`sed`·`tr` 몇 줄이면 끝나지만, 셸을 모르면 매번 별도 스크립트 언어로 파서를 새로 짜야 한다. 이 세 가지가 이 컬렉션이 7개 Part로 나뉘는 이유이기도 하다 — 각 Part는 이 역량들을 순서대로 습득하도록 설계되어 있다.

## 핵심 개념

<strong>셸(Shell)</strong>은 사용자의 명령을 해석해 운영체제 커널에 전달하고, 그 결과를 다시 사용자에게 보여주는 프로그램이다. <strong>Bash(Bourne Again SHell)</strong>는 그중 리눅스 배포판 대부분과 WSL에서 기본값으로 쓰이는 구현체다. macOS는 2019년 Catalina부터 기본 셸이 zsh로 바뀌었지만 Bash는 여전히 사전 설치되어 있어 명시적으로 실행하면 그대로 쓸 수 있다. 셸을 처음 배우는 사람이 가장 먼저 바꿔야 할 사고방식은 "명령어를 하나씩 외운다"가 아니라 "셸이 세상을 어떻게 보는가"를 이해하는 것이다. 유닉스 철학에서 모든 것은 <strong>텍스트 스트림</strong>이다. 파일의 내용, 명령의 출력, 프로세스 목록, 심지어 환경변수까지 셸 안에서는 결국 줄 단위 텍스트로 다뤄진다. 그리고 각 명령어는 "한 가지 일을 잘하는" 작은 필터로 설계되어 있어서, `|`(파이프)로 한 명령의 출력을 다른 명령의 입력으로 흘려보내면 복잡한 작업을 여러 개의 단순한 도구 조합으로 풀 수 있다.
...
각 Part를 실무에서 마주치는 구체적인 상황과 연결하면 표만으로는 드러나지 않는 설계 근거가 분명해진다.

Part 1(셸 기초와 탐색, 01–12장)은 파일 시스템을 오가고(`cd`/`pwd`), 그 안의 내용을 확인하고(`ls`/`cat`/`less`), 필요하면 만들거나 지우는(`touch`/`mkdir`/`cp`/`mv`/`rm`) 최소 동작에 `ln`(하드링크·심볼릭링크)과 `which`/`PATH`/`.bashrc`/`man`(명령어가 어떻게 탐색되고 왜 command not found가 뜨는지)을 더해 구성했다. 이후 모든 Part의 예제는 "어떤 파일을 대상으로" 실행되므로, 그 파일을 찾고 오가는 능력이 없으면 뒤의 예제를 재현할 수조차 없다.

Part 2(텍스트 처리, 13–18장)는 `grep`/`sed`/`awk`/`cut`/`tr`/`sort`·`uniq`·`wc`처럼 파일 하나의 내용을 원하는 형태로 변형하는 도구를 모은다. 로그 파일에서 특정 시간대의 에러 줄만 추출하거나, CSV의 3번째 열만 뽑아 값을 합산하는 작업이 이 Part의 전형적인 사용 사례다.

Part 3(파이프라인과 입출력, 19–22장)은 Part 2에서 배운 개별 도구를 `|`로 연결하는 문법을 다룬다. `grep`으로 걸러낸 결과를 `sort`로 정렬하고 다시 `uniq -c`로 세는 조합, 리다이렉션(`>`/`>>`/`2>`)으로 표준 출력과 표준 에러를 분리 저장하는 법, `xargs`로 파이프 결과를 다른 명령의 인자로 넘기는 법이 여기 속한다. Part 2를 모르는 상태로 Part 3만 배우면 파이프로 무엇을 연결해야 할지 판단할 재료 자체가 없다.

Part 4(프로세스와 작업 제어, 23–26장)는 시선을 파일에서 실행 중인 프로그램으로 옮긴다. `ps`/`top`으로 지금 무엇이 돌고 있는지 확인하고, `kill`/`jobs`로 원치 않는 프로세스를 종료하거나 백그라운드로 보내고, `nohup`으로 SSH 세션이 끊겨도 작업이 죽지 않게 만든다. 오래 걸리는 빌드나 배치 작업을 서버에 올려두고 접속을 끊어야 하는 상황에서 바로 쓰이는 지식이다.

Part 5(셸 스크립팅, 27–35장)는 낱개로 실행하던 명령을 반복 가능한 프로그램으로 묶는다. 조건 분기(`if`/`test`)와 반복문(`for`/`while`)을 먼저 배치하고, 이를 확장하는 `case`·산술 연산, 사용자 입력을 받는 `read`, 여러 값을 다루는 배열, 코드를 재사용하는 함수, 마지막으로 실패를 방어하는 종료 코드·`trap` 순으로 이어진다. 함수를 모르면 `trap` 핸들러 안의 정리 로직을 재사용 가능한 형태로 짤 수 없고, 배열을 모르면 여러 파일을 순회하는 반복문을 우아하게 짤 수 없어 이 순서를 지켰다.

Part 6(파일 시스템과 권한, 36–40장)은 `chmod`/`chown`으로 권한을 조정하고, `find`로 조건에 맞는 파일을 찾고, `gzip`/`tar`로 압축·해제하고, `du`/`df`로 디스크 사용량을 확인하는 관리 작업을 다룬다. 이 Part를 스크립팅(5부) 뒤에 둔 이유는, 실무에서 이런 작업 대부분이 "오래된 로그를 압축하고 디스크 사용량이 임계치를 넘으면 알림을 보내는" 식으로 반복문·조건문과 함께 스크립트 안에서 쓰이기 때문이다.

Part 7(네트워크와 원격 접속, 41–43장)은 `curl`/`wget`으로 HTTP 자원을 내려받고, `scp`로 원격 서버에 파일을 복사하고, `ssh`로 원격 셸에 접속하는 법을 다룬다. 가장 마지막에 배치한 이유는, 원격 서버에 접속한 순간부터는 그 서버 위에서 Part 1–6의 지식(탐색·텍스트 처리·프로세스 제어·스크립팅·권한 관리)을 그대로 다시 써야 하기 때문이다. 로컬에서 이 역량을 갖추지 못한 채 원격 서버부터 접속하면, 세션이 끊기거나 응답이 느릴 때 문제를 좁혀나갈 기준이 없다.

이 순서에는 대안도 있다. 명령어를 기능 범주(파일/텍스트/네트워크)가 아니라 사용 빈도순으로 배치해 자주 쓰는 것부터 먼저 익히게 하는 커리큘럼도 흔하다. 그러나 사용 빈도순은 "이 명령어를 왜 지금 배우는가"라는 맥락을 주지 못한다 — `grep`을 배웠는데 파이프를 모르면 결과를 다른 명령으로 넘길 수 없고, 결국 사용 빈도가 높은 명령어일수록 오히려 선행 지식 없이 배워서 활용도가 떨어지는 역설이 생긴다. 이 컬렉션이 의존성 순서를 택한 이유는, 각 장을 읽을 때마다 직전 장에서 배운 도구를 실제로 조합해볼 수 있어야 학습이 누적되기 때문이다. 사용 빈도순 커리큘럼이 완전히 무의미한 것은 아니다 — 이미 기초를 갖춘 독자가 빈 지식만 골라 보강할 때는 오히려 더 효율적일 수 있으므로, 이 장의 "당신의 수준에 맞는 경로" 표가 그 대안 경로를 제공한다.
...
- [The Open Group, "Shell Command Language(POSIX.1-2017)"](https://pubs.opengroup.org/onlinepubs/9699919799/utilities/V3_chap02.html)
