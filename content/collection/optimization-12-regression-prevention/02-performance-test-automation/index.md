...
**"CI에서 한 번 실행해서 나온 숫자가 곧 진실이다"**는 잘못된 가정입니다. 단일 실행값은 그 순간의 잡음을 그대로 포함하므로, 파이프라인은 항상 여러 번 반복 실행한 통계량(중앙값·백분위수)을 저장해야 합니다. 반복 횟수를 몇 번으로 할지, 이상치를 어떻게 다룰지의 구체적 통계 기법은 [07장](/post/regression-prevention/performance-variance-noise-management/)에서 다루지만, "한 번의 숫자를 믿지 않는다"는 원칙은 파이프라인 설계 단계부터 반영되어야 합니다.
