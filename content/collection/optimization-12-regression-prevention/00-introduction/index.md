...
입문자는 **17 → 01 → 02 → 03 → 05** 순서로 먼저 읽는 것을 권장합니다. 17은 “무엇을 회귀라고 부를지”를 먼저 정의하고, 01~03은 자동화·CI·PR 게이트의 최소 골격을 만들며, 05는 기준선 개념을 안정시켜 줍니다.

이때도 표 순서는 바꾸지 않습니다. 표는 회귀 방지 체계의 전체 구성요소를 빠르게 참조하기 위한 기준이고, 위 추천 순서는 처음 팀 프로세스를 이해하는 독자를 위한 학습 순서입니다.

| 챕터 | 제목 | 난이도 | 핵심 내용 |
|------|------|--------|-----------|
| 01 | 성능 테스트 자동화 | 기초 | 성능 테스트 자동화 구축 |
| 02 | 벤치마크 CI 통합 | 중급 | 벤치마크 CI 통합, CodSpeed 2026 기능 확장(메모리 계측·AI Wizard·MCP 서버·환경 변화 탐지)과 Bencher의 베어메탈 동일성 벤치마킹 |
...
**Phase A — 자동화 기초 (챕터 01~03, 13)** 테스트·CI·PR 게이트를 연결합니다. Tr.01의 벤치 설계가 없으면 게이트가 노이즈에 무력화됩니다.

**Phase B — 예산·변동성·관측 (챕터 04~08)** performance budget과 알림은 Tr.11 SLO와 맞물립니다.
...
- [ ] 마이크로벤치마크(Tr.01)와 CI 게이트(본 트랙)의 역할 분담을 말할 수 있는가?
...
지속적 프로파일링(Tr.01)과 모니터링 대시보드(챕터 14)를 함께 설계하면, 프로덕션 꼬리 지연과 CI 벤치의 괴리를 줄일 수 있습니다.
