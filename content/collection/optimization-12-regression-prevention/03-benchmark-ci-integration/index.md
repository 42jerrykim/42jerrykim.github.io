...
**"CI에서 벤치마크를 자동으로 돌리기만 하면 회귀를 잡을 수 있다"**는 챕터 02 수준의 자동화와 이 장에서 다루는 노이즈 통제를 혼동하는 오개념입니다. 자동 실행은 필요조건일 뿐이고, 노이즈가 신호보다 큰 상태에서는 게이트가 우연히 통과·실패를 반복하는 "가짜 안정성"만 만듭니다.
