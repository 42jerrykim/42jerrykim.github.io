...
```mermaid
flowchart LR
    A["극중 설정</br>메드포드, 텍사스 동부</br>체로키 카운티·댈러스 3시간 거리"]
...
극중 세부 설정도 허구다. 쿠퍼 가족의 집 주소는 "5501 Grant Ave, Medford, Texas, 88597"로 등장하는데, 우편번호 88597은 텍사스가 아니라 뉴멕시코주에 배정된 대역(8로 시작하는 우편번호)이어서 애초에 실재할 수 없는 번호다. 텍사스 우편번호는 7로 시작한다. 이런 디테일은 제작진이 메드포드를 "그럴듯하지만 결코 실존 도시와 혼동되지 않도록" 의도적으로 설계했음을 보여준다. 마을은 체로키 카운티(Cherokee County)에 속한 것으로 설정되는데, 실제 텍사스에도 동일한 이름의 카운티(주도 러프킨)가 존재하지만 메드포드라는 마을 자체는 그 카운티 안에 없다.

가상이라고 해서 아무렇게나 그려진 것은 아니다. 쇼러너 척 로리(Chuck Lorre)와 스티븐 몰라로(Steven Molaro)는 파일럿 제작 전 실제로 텍사스 동부와 휴스턴 일대를 답사하며 현지 분위기를 조사했다고 여러 매체 인터뷰에서 밝혔다. 이 답사를 바탕으로 극중에는 닥터페퍼를 마시는 메리 쿠퍼, 복음주의 개신교 문화, 텍사스 오일러스(현재는 사라진 옛 NFL 휴스턴 프랜차이즈) 티셔츠 같은 시대·지역 고증이 곳곳에 들어갔다. 즉 메드포드는 "존재하지 않는 지명" 위에 "실재하는 동부 텍사스의 질감"을 입힌 합성 도시에 가깝다.
