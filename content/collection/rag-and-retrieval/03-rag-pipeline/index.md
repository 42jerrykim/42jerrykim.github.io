...
지금까지는 사용자의 질문 문장을 그대로 임베딩해 검색한다고 전제했지만, 질문 자체를 검색에 유리한 형태로 바꾸는 전처리(query rewriting/transformation)가 검색 품질을 크게 좌우하는 경우가 많습니다. 대표적인 기법이 <strong>HyDE(Hypothetical Document Embeddings)</strong>입니다 — 사용자의 질문을 그대로 임베딩하는 대신, LLM에게 먼저 그 질문에 대한 가상의 답변을 생성하게 하고 이 가상 답변을 임베딩해 검색합니다. 짧은 질문 문장보다 가상의 답변 문단이 어휘·구조 면에서 실제 정답 문서와 더 가깝기 때문에, 질문과 정답 문서의 표현이 크게 다른 도메인일수록 검색 정확도가 향상됩니다.

> Luyu Gao 외, "Precise Zero-Shot Dense Retrieval without Relevance Labels", *arXiv:2212.10496* (2022)

파이프라인을 구축한 뒤에는 "잘 작동하는가"를 정량적으로 확인해야 합니다. RAGAS는 정답 레이블 없이 LLM을 심사자(judge)로 활용해 세 가지 축을 측정하는 평가 프레임워크입니다 — <strong>충실도(Faithfulness)</strong>는 생성된 답변의 주장이 검색된 컨텍스트로부터 실제로 추론 가능한지(할루시네이션 방지), <strong>답변 관련성(Answer Relevance)</strong>은 답변이 원 질문에 실제로 부합하는지(불완전하거나 중복된 답변에 감점), <strong>컨텍스트 관련성(Context Relevance)</strong>은 검색된 컨텍스트 중 답변에 실제로 필요한 문장의 비율을 각각 정량화합니다.

> Shahul Es 외, "RAGAS: Automated Evaluation of Retrieval Augmented Generation", *arXiv:2309.15217* (2023)
