...
> Patrick Lewis 외, "Retrieval-Augmented Generation for Knowledge-Intensive NLP Tasks", *arXiv:2005.11401* (2020) — RAG라는 이름 자체가 이 논문에서 나왔습니다.
...
| 03 | RAG 파이프라인 구축 | 검색된 문서를 LLM에 어떻게 넘겨야 하고, 그 결과를 어떻게 평가하는가 |
...
01–02장은 검색 자체의 두 갈래(Sparse·Dense)를, 03–04장은 그 검색을 LLM 생성과 실제로 결합하는 방법을, 05–07장은 텍스트 문서를 넘어선 확장(그래프, 정형 데이터, 재정렬 모델 직접 학습)을 다룹니다. 이 순서를 따르는 이유는 05–07장에서 다루는 응용이 모두 01–04장에서 다지는 검색·유사도 계산의 기본기를 전제하기 때문입니다. BM25·Dense Retrieval에 익숙하다면 01–02장을 건너뛰고 03장부터 시작해도 무리가 없습니다. 검색 질의를 검색에 유리한 형태로 바꾸는 쿼리 재작성(HyDE)과, 완성된 파이프라인의 품질을 정량적으로 측정하는 평가 프레임워크(RAGAS)는 별도 장을 두지 않고 03장 안에서 함께 다룹니다.

```mermaid
flowchart LR
    ch01["01 Classical IR"] --> ch02["02 Dense Retrieval"]
    ch02 --> ch03["03 RAG 파이프라인"]
    ch03 --> ch04["04 근사 검색"]
    ch04 --> ch05["05 GraphRAG"]
    ch04 --> ch06["06 MCP Text2SQL"]
    ch04 --> ch07["07 Cross-Encoder"]
```
