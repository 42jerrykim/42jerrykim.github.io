...
평면 위에 축에 평행한 정사각형들이 주어질 때, 각 정사각형에서 정확히 한 점씩 선택해 만든 점 집합의 <strong>직경(diameter)</strong>을 최대화하고, 그 직경 \(D\)의 제곱 \(D^2\)을 정수로 출력하는 문제입니다.
...
3. **볼록 껍질과 직경:**  
   평면 위 점 집합의 직경(가장 먼 두 점)은 항상 **볼록 껍질** 위의 두 점(antipodal pair)으로 얻을 수 있다. 따라서 \(4n\)개 꼭짓점의 볼록 껍질을 구한 뒤, <strong>회전하는 캘리퍼스(rotating calipers)</strong>로 antipodal pair를 모두 보면서, 서로 다른 정사각형에서 나온 쌍에 대해서만 \(D^2\) 후보를 갱신한다.
