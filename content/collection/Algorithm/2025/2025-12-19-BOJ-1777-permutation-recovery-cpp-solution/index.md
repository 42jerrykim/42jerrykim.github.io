...
핵심은 이 정보를 이용해 **큰 수부터** 순열의 위치를 하나씩 확정할 수 있고, 이때 “남은 빈 칸 중 k번째”를 빠르게 찾기 위해 <strong>Fenwick Tree(BIT)</strong>를 사용한다.
...
- 현재 비어있는 위치 집합에서 <strong>k번째 빈 칸(k = v - A[v])</strong>을 찾아 그 위치에 \(v\)를 놓고,
- 그 칸을 “채움(1→0)” 처리한다.
