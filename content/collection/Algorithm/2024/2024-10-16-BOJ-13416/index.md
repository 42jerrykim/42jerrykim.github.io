...
이 알고리즘은 매일 최적의 선택을 하므로 전체 기간 동안의 최대 이익을 구할 수 있다. 시간 복잡도는 <strong>O(N)</strong>으로, **N**은 주식 데이터의 일수이다.
...
이번 문제는 각 날마다 최대 이익을 가져다주는 주식을 선택하는 간단한 **그리디 알고리즘**으로 해결할 수 있었다. 매일의 데이터에서 최대값을 선택하고, 그 값이 양수인 경우에만 총 이익에 더하는 방식으로 문제를 풀었다. 이 과정에서 시간 복잡도는 <strong>O(N)</strong>으로 매우 효율적이다.
