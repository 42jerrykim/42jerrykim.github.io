---
description: "백준 2357 최솟값과 최댓값 문제는 수열에 여러 구간 쿼리를 효율적으로 처리해야 하므로, 빠른 전처리와 쿼리 처리가 가능한 자료구조(Sparse Table, 세그먼트 트리) 사용이 핵심입니다. 각 쿼리마다 구간 내 최소·최대 값을 신속하게 반환할 수 있도록 설계해야 시간 제한을 충족할 수 있습니다."
image: "wordcloud.png"
...
이번 포스팅에서는 백준 온라인 저지의 2357번 문제인 <strong>"최솟값과 최댓값"</strong>을 다뤄보겠다. 이 문제는 수열에서 여러 구간에 대한 최솟값과 최댓값을 효율적으로 구하는 알고리즘을 요구한다. 수열의 크기가 매우 크기 때문에 단순한 방법으로는 시간 내에 해결하기 어렵다. 따라서 **Sparse Table**과 같은 효율적인 자료 구조를 활용하여 문제를 해결해보자.
