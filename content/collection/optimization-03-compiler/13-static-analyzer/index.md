...
- **미사용 결과 / 불필요한 복사**: 함수 반환값을 쓰지 않거나, 값으로 받을 때 복사가 불필요하게 일어나는 경우. `-Wunused-result`, `-Wunused-value` 또는 분석기별 "무시된 반환값", "복사 대신 이동 가능" 같은 메시지가 나올 수 있습니다. 불필요한 복사는 Tr.02(객체 수명, 파라미터 전달)과도 연결됩니다.
