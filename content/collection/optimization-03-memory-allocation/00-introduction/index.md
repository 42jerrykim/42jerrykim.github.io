...
프로파일러가 가리키는 “느린 함수” 뒤에는 종종 **할당 횟수**와 **접근 패턴**이 숨어 있습니다. 언어 트랙(Tr.01)에서 문자열·컨테이너 사용을 다듬은 뒤에도, 핫패스에 남는 비용은 **어디에 붙어 할당되는지**, **캐시 라인을 어떻게 소비하는지**로 귀결되는 경우가 많습니다. 이 트랙은 그 지점을 **데이터 구조·할당 정책·레이아웃**으로 설계해 바꿉니다. µs 예산에서는 allocation 한 번과 캐시 미스 한 번이 누적되어 꼬리 지연을 키우므로, “알고리즘은 같지만 메모리 설계만 다른” 두 구현을 벤치마크로 비교할 수 있어야 합니다.
...
챕터 01 계열이 모두 공개되기 전이라도, 현재 공개된 선행 기초 장부터 시작할 수 있습니다.
