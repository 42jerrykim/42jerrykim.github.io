...
draft: false
image: wordcloud.png
title: "[Memory 03] Virtual Memory 관리 힌트"
...
`free` 모드의 결과는 벤치마크 실행 시점의 시스템 메모리 압박 상태에 크게 좌우됩니다. 압박이 있는 환경에서 재현하면 `MADV_FREE`도 `MADV_DONTNEED`에 가까운 폴트 수를 보일 수 있으므로, "MADV_FREE가 항상 공짜"라고 일반화하지 말고 대상 서버의 실제 메모리 여유 상태에서 재현하는 것이 중요합니다. 힙 전체의 할당·회수 흐름을 더 넓게 보고 싶다면 [Tr.05 ch20: 메모리 프로파일링: 힙 분석](/post/profiling-analysis/memory-profiling-heap-analysis/)에서 다루는 heaptrack 계열 도구와 병행하는 것을 권장합니다.
