...
토큰 ID 자체는 단순한 순번이라 "5번 토큰이 3번 토큰보다 의미가 크다"는 식의 관계가 없습니다. <strong>토큰 임베딩(Token Embedding)</strong>은 각 토큰 ID를 학습 가능한 N차원 벡터로 대응시키는 조회 테이블(lookup table)입니다. "임베딩"이라는 말 자체가 어떤 대상을 N차원 벡터 공간의 한 점으로 표현한다는 뜻이며, 01장에서 다룬 코사인 유사도로 두 토큰(또는 문장)의 의미적 유사도를 잴 수 있는 것도 이 임베딩 덕분입니다.
...
`Tom ate the dog`과 `The dog ate Tom`은 같은 토큰들로 이루어졌지만 완전히 다른 의미입니다. 하지만 Transformer는 모든 토큰을 동시에 입력받기 때문에, 토큰 임베딩만으로는 이 둘을 구분할 수 없습니다. 그래서 토큰 임베딩과 같은 차원의 <strong>위치 임베딩(Positional Embedding)</strong>을 별도로 만들어 더해줍니다.
