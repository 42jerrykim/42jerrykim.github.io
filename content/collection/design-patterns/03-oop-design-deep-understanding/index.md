...
진정한 객체지향은 **현실 세계 모델링이 아니라**, **복잡성을 다루는 강력한 사고 방식**입니다. 그리고 이 사고 방식의 핵심은 <strong>"책임의 분배"</strong>와 <strong>"협력의 설계"</strong>에 있습니다.

많은 개발자들이 클래스와 객체를 만들 줄 알지만, 정작 <strong>"왜 이렇게 설계해야 하는가?"</strong>에 대한 답을 갖지 못합니다. 상속을 남용하고, 캡슐화를 getter/setter로 오해하며, 다형성을 단순한 메서드 오버라이딩 정도로 인식합니다.
...
이것은 캡슐화가 아니라 <strong>"캡슐화의 위장"</strong>입니다. 실제로는 모든 내부 상태가 외부에 노출되어 있고, 객체의 책임은 외부 코드가 대신 수행하고 있습니다.

#### 진정한 캡슐화: 책임과 지식의 은닉

진정한 캡슐화는 <strong>"객체가 무엇을 알고 있는가"</strong>와 <strong>"객체가 무엇을 할 수 있는가"</strong>를 하나로 묶는 것입니다:
...
**취약한 기반 클래스 문제:** 아래 InstrumentedHashSet 예제는 Joshua Bloch, 『Effective Java』, Item 18("상속보다는 컴포지션을 사용하라")에서 다루는 대표적인 사례를 각색한 것입니다.
```java
// 기반 클래스
...
다형성은 단순히 메서드 오버라이딩이 아닙니다. 그것은 <strong>"같은 인터페이스, 다른 구현"</strong>을 통해 **코드의 유연성과 확장성**을 제공하는 메커니즘입니다.
...
LSP는 단순히 "파생 클래스는 기반 클래스를 대체할 수 있어야 한다"는 것이 아닙니다. 그것은 <strong>"행동적 호환성"</strong>을 보장하는 원칙입니다.
...
SRP는 "클래스는 하나의 책임만 가져야 한다"가 아닙니다. 정확히는 <strong>"클래스가 변경되는 이유는 오직 하나여야 한다"</strong>입니다.
...
OCP는 <strong>"확장에는 열려있고, 수정에는 닫혀있어야 한다"</strong>는 원칙입니다. 이는 **Strategy 패턴**과 **Template Method 패턴**의 이론적 기반입니다.
...
- Tell, Don't Ask 원칙에 따라 "Ask" 스타일 코드를 "Tell" 스타일로 리팩터링할 수 있다.
- InstrumentedHashSet 사례(Effective Java Item 18)를 근거로 상속이 컴포지션보다 위험한 이유를 설명할 수 있다.
- LSP 위반(Rectangle/Square)과 ISP 위반(Worker 인터페이스) 사례를 각각 올바른 설계로 고칠 수 있다.
- Content/Common/Control/Data/Message 결합도 5단계를 좋음/나쁨 순서로 나열할 수 있다.
- SOLID 5원칙과 그에 대응하는 GoF 패턴(Strategy, Decorator, Factory Method 등)을 짝지을 수 있다.

---

### 결론: 객체지향 설계의 본질

객체지향은 <strong>"현실 세계 모델링"</strong>이 아닙니다. 그것은 **복잡한 소프트웨어 시스템을 다루기 위한 강력한 사고 도구**입니다. 이 사고 도구는 다음 네 가지 가치로 요약되며, 각 가치는 특정 디자인 패턴을 통해 구체적으로 실현됩니다.

1. **책임의 명확한 분배**: 각 객체는 명확한 책임을 완전히 수행합니다. → Strategy 패턴은 OCP와 DIP를 구현하여 책임을 분리합니다.

2. **변경에 대한 유연한 대응**: 추상화와 다형성을 통해 요구사항 변경에 유연하게 대응할 수 있습니다. → Observer 패턴은 느슨한 결합과 메시지 기반 통신으로 이를 실현합니다.

3. **복잡성의 효과적 관리**: 캡슐화를 통해 복잡성을 국소화하고, 인터페이스를 통해 단순화합니다. → Factory 패턴은 DIP를 바탕으로 객체 생성 책임을 분리합니다.

4. **재사용성과 확장성**: 잘 설계된 객체지향 코드는 재사용과 확장이 용이합니다. → Decorator 패턴은 OCP와 컴포지션을 활용해 이를 달성합니다.

객체지향을 이해한다는 것은 단순히 클래스와 상속을 아는 것이 아닙니다. 그것은 <strong>"어떻게 책임을 분배하고, 어떻게 협력하게 할 것인가"</strong>에 대한 깊은 통찰을 갖는 것입니다.
