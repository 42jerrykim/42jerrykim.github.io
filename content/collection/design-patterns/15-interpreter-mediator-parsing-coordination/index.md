...
title: "[Design Patterns] 15. 인터프리터와 미디에이터: 파싱과 조정의 패턴"
...
`BooleanExpressionParser`는 재귀 하강 파서(recursive descent parser)로 동작한다. 재귀 하강 파서는 문법의 각 규칙을 그대로 하나의 메서드로 옮기고, 낮은 우선순위 규칙의 메서드가 높은 우선순위 규칙의 메서드를 호출하는 방식으로 우선순위를 코드 구조 자체에 반영한다. 이 파서는 `parseOr` → `parseAnd` → `parseNot` → `parseAtom` 순서로 메서드를 계층화하는데, 호출 순서가 곧 연산자 우선순위다: `OR`가 가장 낮고 `AND`가 그다음이며 `NOT`이 가장 높다. `parseOr`는 먼저 `parseAnd`를 호출해 왼쪽 피연산자를 얻은 뒤 `OR` 토큰이 남아 있는 동안 반복해서 오른쪽을 파싱하므로 `A OR B OR C`는 왼쪽 결합(left-associative)으로 `(A OR B) OR C` 트리가 된다. 이 계층을 BNF 문법으로 옮기면 다음과 같다.
...
and    ::= not ("AND" not)*
not    ::= "NOT" atom | atom
atom   ::= "(" or ")" | "true" | "false" | identifier ["(" ")"]
```

괄호는 `parseAtom`이 `(`를 만나면 최상위 규칙인 `parseOr`를 재귀적으로 다시 호출해 처리하는데, 이 재귀 호출이 바로 괄호 안 표현식이 우선순위와 무관하게 하나의 단위로 묶이는 이유다.
...
`BooleanExpressionParser`가 `"isVip OR (isSenior AND isLoyalCustomer)"`를 파싱하면, 문자열이 아래와 같은 `Expression` 객체 트리(AST)로 변환됩니다. `interpret(Context)`는 이 트리를 루트에서부터 재귀적으로 내려가며 평가하는데, `AndExpression`은 왼쪽이 `false`면 오른쪽을 평가하지 않는 단락 평가(short-circuit)를 수행합니다.

```mermaid
flowchart TD
    Or["OrExpression"]
    Var1["VariableExpression<br/>(isVip)"]
    And["AndExpression"]
    Var2["VariableExpression<br/>(isSenior)"]
    Var3["VariableExpression<br/>(isLoyalCustomer)"]
...
### 흔한 오개념: Mediator와 Observer 혼동

"여러 객체에 한 곳에서 메시지를 전달한다"는 설명만 보면 Mediator와 Observer가 같은 패턴처럼 느껴지기 쉽습니다. 하지만 위 표의 "중재자 역할" 행이 가리키듯 둘의 차이는 통신 방향이 아니라 **결정 로직이 어디에 있는가**입니다. Observer의 Subject는 상태가 바뀌었다는 사실만 통지할 뿐, 누가 어떻게 반응할지는 전혀 알지 못하고 관여하지도 않습니다. 반면 `AdvancedChatRoom`은 단순히 메시지를 뿌리는 것이 아니라, 어떤 사용자가 어느 그룹에 속해 있는지 확인하고(`sendGroupMessage`), 필터를 통과했는지 판단하고(`applyFilters`), 누구를 수신자 목록에서 제외할지 스스로 결정합니다 — 이런 능동적 판단이 바로 Mediator를 Mediator이게 하는 부분입니다. 실무에서 흔한 실수는 "여러 Colleague가 서로 조율해야 하는" 문제(예: 그룹 멤버십 검사, 메시지 필터링 순서 결정)를 각 Colleague가 독립적으로 반응하는 Observer 여러 개로 흉내 내려는 것입니다. 이 경우 조율 로직이 사라진 채 여러 Observer에 흩어지면서, 원래 Mediator 하나로 집중되어야 할 결정이 다시 여러 객체에 분산되는 결과를 낳습니다.
