...
## Christopher Alexander와 건축 패턴의 혁명
...
하지만 Alexander의 진정한 혁신은 **패턴들 간의 관계**를 발견한 것이었습니다. 패턴들은 독립적으로 존재하는 것이 아니라, 서로 연결되어 하나의 <strong>"언어"</strong>를 형성했습니다.
...
당시 객체지향 프로그래밍이 등장했지만, 여전히 <strong>"어떻게 좋은 객체지향 설계를 할 것인가?"</strong>에 대한 명확한 가이드라인이 없었습니다.
...
소프트웨어의 유일한 상수는 <strong>"변화"</strong>입니다. 패턴은 변화하는 부분과 변화하지 않는 부분을 분리합니다:
...
| 연도 | 주요 사건 | 기여자/저작 | 의미 |
|------|----------|------------|------|
| 1977 | 건축 패턴 언어 탄생 | Christopher Alexander, 『A Pattern Language』 | 253개 건축 패턴으로 패턴 개념 정립 |
| 1987 | 소프트웨어 패턴 최초 적용 | Kent Beck, Ward Cunningham | Smalltalk 프로그래밍에 패턴 적용 |
| 1991 | Erich Gamma 박사 논문 | Erich Gamma | 소프트웨어 설계 패턴 체계적 연구 |
| 1991 | Gang of Four 결성 | Gamma, Helm, Johnson, Vlissides | 객체지향 설계 지혜 체계화 시작 |
| 1994 | GoF 책 출간 | 『Design Patterns: Elements of Reusable Object-Oriented Software』 | 23개 GoF 패턴 정립, 업계 표준화 |
| 2000년대 | 엔터프라이즈 패턴 | Martin Fowler, 『PoEAA』 | 엔터프라이즈 애플리케이션 패턴 확장 |
| 2010년대~ | 마이크로서비스 패턴 | Chris Richardson 외 | 분산 시스템 패턴 발전 |

### 패턴 사상의 핵심 원천

| 원천 | 핵심 개념 | 소프트웨어 적용 |
|------|----------|----------------|
| 건축 패턴 | Quality Without a Name, 전체성, 점진적 성장 | 좋은 설계의 본질적 품질 추구 |
| 인지과학 | 청킹(Chunking), 스키마(Schema) | 코드 블록을 의미 단위로 인식 |
| 수학적 미학 | 대칭성, 재귀성, 단순성 | 예측 가능하고 우아한 구조 |

### GoF 패턴이 해결하는 근본 문제

| 문제 유형 | 설명 | 대표 패턴 |
|----------|------|----------|
| 복잡성 관리 | 우연적 복잡성을 본질적 복잡성으로 전환 | Chain of Responsibility, Composite |
| 변경 대응 | 변화하는 부분과 불변 부분 분리 | Strategy, Observer |
| 코드 재사용 | 설계 레벨의 재사용 가능한 구조 | Factory Method, Template Method |
| 의사소통 | 개발자 간 공통 어휘 제공 | 모든 GoF 패턴 |

### 패턴 분류 체계

| 분류 기준 | 유형 | 포함 패턴 수 |
|----------|------|-------------|
| 목적별 | 생성(Creational) | 5개 |
| | 구조(Structural) | 7개 |
| | 행동(Behavioral) | 11개 |
| 범위별 | 클래스 (상속 관계) | 4개 |
| | 객체 (객체 관계) | 19개 |
