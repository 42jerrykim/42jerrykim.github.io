...
**Strategy 패턴**은 <strong>"알고리즘의 캡슐화"</strong>에 초점을 맞춥니다. 런타임에 알고리즘을 교체할 수 있는 유연성을 제공하며, "어떻게 할 것인가(How)"의 문제를 해결합니다.

**State 패턴**은 <strong>"상태 기반 행동 변화"</strong>에 초점을 맞춥니다. 객체의 내부 상태에 따라 행동이 자동으로 변하며, "언제 무엇을 할 것인가(When & What)"의 문제를 해결합니다.
...
Strategy와 State 패턴은 <strong>"캡슐화"</strong>라는 공통 목표를 가지지만, 서로 다른 관점에서 접근합니다:
...
두 패턴 모두 **개방-폐쇄 원칙**을 실현하고 **코드의 유연성**을 높이는 강력한 도구입니다. 핵심은 <strong>"무엇을 캡슐화하려는가?"</strong>를 명확히 하는 것입니다.
