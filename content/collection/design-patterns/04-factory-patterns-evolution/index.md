...
이 사례는 Factory Method의 **"골격은 상위 클래스가, 구체적 생성은 하위 클래스가 담당한다"**는 특징을 그대로 보여줍니다. `AbstractList`는 `iterator()`라는 Template Method를 통해 순회 알고리즘의 골격을 고정하면서도, 실제 `ListIterator` 생성은 `listIterator()`라는 Factory Method로 위임합니다. 그 결과 `ArrayList`와 `LinkedList`는 각자의 내부 자료구조에 최적화된 Iterator를 반환하면서도, 클라이언트 코드는 동일한 `Iterator` 인터페이스로 순회할 수 있습니다.
...
public class EuropeWestFactory implements AWSServiceFactory {
    ...
    @Override
    public AmazonRDS createRDSClient() {
...
이 사례는 Abstract Factory가 **인프라 설정의 일관성**을 강제하는 도구로도 쓰일 수 있음을 보여줍니다. `USEastFactory`와 `EuropeWestFactory`는 각각 하나의 리전에 고정된 자격증명·엔드포인트로 S3·EC2·RDS·SQS 클라이언트를 생성하므로, 개발자가 실수로 서로 다른 리전의 클라이언트를 섞어 쓸 위험이 원천적으로 차단됩니다. 이는 GUI Look & Feel 일관성과 동일한 원리가 인프라 계층에도 적용된 예입니다.
...
예시 수치이며 실측치가 아닙니다. 실제 성능은 JVM 버전, 하드웨어, JIT 최적화 여부에 따라 달라지므로 프로젝트 환경에서 직접 벤치마크해야 합니다.
