...
앞 장에서 다룬 Bridge·Flyweight의 이론을 코드로 직접 검증하는 것이 이 장의 목표다. 세 과제는 각각 다른 능력을 확인하며, 각 항목의 "완성 기준"은 과제를 마쳤다고 판단할 수 있는 구체적 조건이다.

1. **Bridge 패턴으로 다중 플랫폼 파일 시스템 구현** — 추상화(`FileManager`)가 구현(`FileSystemImpl`)의 구체 클래스가 아니라 인터페이스에만 의존하는지 확인한다. 완성 기준은 "런타임에 `FileSystemImpl` 구현체를 Windows→Linux로 바꿔도 `FileManager`의 비즈니스 로직 코드를 한 줄도 수정·재컴파일하지 않고 동작하는가"이다.
2. **Flyweight 패턴으로 메모리 효율적인 텍스트 렌더링 구현** — 내재적 상태(문자·폰트 조합)와 외재적 상태(좌표)를 필드 수준에서 정확히 분리했는지 확인한다. 완성 기준은 "100만 개 문자를 렌더링해도 `CharacterFlyweightFactory`가 보유한 인스턴스 수가 실제 문자·폰트 조합 수 이하로 유지되는가"이다.
3. **패턴 적용 전후 성능 비교 분석** — 패턴 적용이 항상 이득은 아니라는 것을 수치로 확인한다. 완성 기준은 "Bridge의 간접 호출 오버헤드(%)와 Flyweight의 메모리 절약률(%)을 각각 벤치마크 실측치로 제시하고, 그 수치가 어떤 조건(호출 빈도, 문자·폰트 다양성)에서 유효한지 설명할 수 있는가"이다.
...
구현에 들어가기 전에 어떤 필드가 Flyweight 내부에 저장되고(내재적) 어떤 값이 매 호출마다 외부에서 전달되는지(외재적)를 먼저 표로 정리해두면, 아래 참조 구현에서 왜 `x`, `y`가 생성자가 아니라 `render()`의 매개변수로만 나타나는지 이해하기 쉽다.

| 상태 구분 | 필드 | 저장 위치 | 근거 |
|-----------|------|----------|------|
| 내재적(Intrinsic) | `character`, `fontFamily`, `fontSize`, `isBold`, `isItalic` | `ConcreteCharacterFlyweight` 인스턴스 필드(`final`) | 같은 문자·폰트 조합은 문서 전체에서 반복되므로 공유해도 안전하고, 공유해야 인스턴스 수가 줄어든다 |
| 외재적(Extrinsic) | `x`, `y` (렌더링 좌표) | `TextDocument`가 문자 위치마다 별도로 보관 | 문자마다 위치가 다르므로 Flyweight에 저장하면 공유가 불가능해져 패턴 자체가 무의미해진다 |

이 구분을 어기는 가장 흔한 실수는 `x`, `y`를 `ConcreteCharacterFlyweight`의 필드로 선언하는 것이다. 그렇게 하면 같은 `('A', "Arial", 12, false, false)` 조합이라도 위치가 다른 두 문자를 위해 서로 다른 인스턴스가 필요해지므로, 캐시 재사용률이 0에 가까워지고 Flyweight를 적용하기 전과 메모리 사용량이 동일해진다.
