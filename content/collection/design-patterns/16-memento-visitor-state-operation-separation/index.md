...
class BadCircle extends BadShape {
    ...
    @Override
    public double calculateArea() { /* calculateArea 구현 */ }
    
    @Override
    public void draw() { /* draw 구현 */ }
    
    @Override
    public String exportToSVG() { /* exportToSVG 구현 */ }
    
    @Override
    public void applyTexture() { /* applyTexture 구현 */ }
...
### 왜 4개의 서로 다른 Visitor가 공존하는가

`AreaCalculatorVisitor`, `DrawingVisitor`, `SVGExportVisitor`, `ValidationVisitor`는 모두 같은 `Shape` 계층(`Circle`, `Rectangle`, `Triangle`, `CompoundShape`)을 순회하지만, 서로의 존재를 전혀 모릅니다. 이것이 Visitor 패턴의 핵심 이득입니다 — "넓이 계산"과 "그리기"와 "SVG 내보내기"와 "유효성 검증"은 서로 관련 없는 별개의 관심사인데, Visitor 없이 구현했다면(`BadShape`처럼) 이 네 가지 연산이 전부 `Circle` 클래스 안에 메서드로 뒤섞여 있어야 합니다. Visitor로 분리하면 각 연산은 자신만의 상태(`totalArea`, `canvas` `StringBuilder`, `svg` `StringBuilder`, `errors`/`warnings` 리스트)를 독립적으로 들고 다니며, 하나를 수정해도 나머지 세 개에 영향을 주지 않고, 다섯 번째 연산(예: JSON 직렬화)을 추가할 때도 `Circle`이나 `Rectangle`의 소스 코드를 단 한 줄도 건드릴 필요가 없습니다. 즉 4개의 Visitor는 "우연히 네 개가 된 것"이 아니라, 하나의 고정된 구조 위에서 독립적으로 확장·테스트·배포될 수 있는 연산의 개수만큼 자연스럽게 늘어난 것입니다.
...
두 단계의 호출이 각각 다른 타입에 의해 결정되기 때문에 "이중" 디스패치라고 부른다. 1단계는 `element`의 실제 런타임 타입에 따라 오버라이드된 `accept()`가 선택되고, 2단계는 그 `accept()` 안에서 `visitor.visit(this)`를 호출할 때 컴파일 타임에 `this`의 정적 타입(예: `Circle`)으로 오버로드된 `visit()`가 선택된다. 단일 디스패치만 지원하는 자바에서 두 타입의 조합에 따라 실행할 코드를 결정하려면 이렇게 두 번의 가상 호출을 거쳐야 한다.

```mermaid
sequenceDiagram
    participant Client
    participant Circle as "circle: Circle"
    participant Visitor as "visitor: AreaCalculatorVisitor"

    Client->>Circle: accept(visitor)
    Note over Circle: 1단계 디스패치</br>Circle의 accept() 선택
    Circle->>Visitor: visit(this)
    Note over Visitor: 2단계 디스패치</br>Circle 오버로드 선택
    Visitor-->>Circle: (Circle 전용 연산 수행)
    Circle-->>Client: 반환
```
...
`DocumentHistory`의 `undo()`/`redo()`는 두 스택(undoStack, redoStack) 사이에서 Memento를 주고받으며 상태를 이동시킨다. 편집 동작이 일어나면 redo 스택은 비워지는데, 이는 "되돌린 뒤 새로 편집하면 이전에 되돌렸던 미래는 더 이상 유효하지 않다"는 불변식을 강제하기 위함이다. 아래 다이어그램은 이 세 가지 전이(저장, undo, redo)가 두 스택의 상태를 어떻게 바꾸는지 보여준다.

```mermaid
stateDiagram-v2
    [*] --> Empty
    Empty --> HasUndo: saveState()</br>undoStack.push(memento)

    HasUndo --> HasUndo: saveState()</br>undoStack.push(memento)
    HasUndo --> HasBoth: undo()</br>redoStack.push(current)</br>restore(undoStack.pop())
    HasBoth --> HasUndo: undo()</br>redoStack.push(current)</br>restore(undoStack.pop())
    HasBoth --> HasBoth: redo()</br>undoStack.push(current)</br>restore(redoStack.pop())
    HasBoth --> HasUndoOnly: 새 편집 발생</br>redoStack.clear()

    HasUndoOnly --> HasBoth: undo()</br>redoStack.push(current)</br>restore(undoStack.pop())
    HasUndoOnly --> HasUndoOnly: saveState()</br>undoStack.push(memento)
```
...
- Memento 패턴이 Originator/Memento/Caretaker 세 역할로 캡슐화를 유지하며 상태를 저장·복원하는 원리를 설명할 수 있다.
- Visitor 패턴의 Double Dispatch가 단일 디스패치 언어(Java)에서 왜 두 번의 가상 호출을 필요로 하는지 설명할 수 있다.
- Undo 스택과 Redo 스택이 편집·undo·redo 각 동작에서 어떻게 상태를 주고받는지, 그리고 새 편집이 redo 스택을 비우는 이유를 설명할 수 있다.
- Memento와 Visitor 중 어떤 패턴을 적용할지 "적용 시나리오 비교"·"적용 체크리스트" 표를 근거로 판단할 수 있다.
- Visitor 패턴이 새로운 Element 타입 추가에는 취약하고 새로운 연산 추가에는 강한 이유(개방-폐쇄 원칙이 적용되는 축이 다름)를 설명할 수 있다.

## 흔한 오개념

**오해 1: "Memento는 항상 전체 상태를 깊은 복사해야 한다"**

`DocumentEditor.createMemento()`가 `content` 전체를 매번 복사하는 모습만 보면 Memento는 항상 전체 스냅샷을 떠야 하는 패턴처럼 보입니다. 하지만 GoF의 정의는 "캡슐화를 유지하며 상태를 저장·복원할 것"만 요구할 뿐, 저장 방식은 규정하지 않습니다. 이 글의 "메모리 관리 전략" 표에서 다룬 차분(diff) 저장, 주기적 스냅샷, LRU 캐시, 그리고 `OptimizedDocumentMemento`의 GZIP 압축까지 모두 유효한 Memento 구현입니다. 실무에서는 상태 변경 빈도와 크기에 따라 전체 복사가 차분 계산·재생 비용보다 오히려 저렴할 수 있어, "무조건 깊은 복사"도 "무조건 차분 저장"도 아닌 상황별 트레이드오프 문제로 다뤄야 합니다.

**오해 2: "Visitor는 새 Element 추가에도 유연하다"**

Visitor는 "확장에 열려 있다"는 개방-폐쇄 원칙의 대표 사례로 자주 소개되지만, 이 유연함은 **새 연산 축**에만 적용됩니다. `ShapeVisitor` 인터페이스에 새로운 도형(예: `Polygon`)을 추가하려면 `visit(Polygon)` 메서드를 인터페이스에 추가해야 하고, 그 순간 이미 존재하는 `AreaCalculatorVisitor`, `DrawingVisitor`, `SVGExportVisitor`, `ValidationVisitor` 네 클래스 전부가 컴파일 에러를 내며 새 오버로드 구현을 요구합니다. 즉 Visitor는 "연산 축"에서는 개방-폐쇄 원칙을 지키지만 "타입(Element) 축"에서는 정확히 반대로 위반합니다. 이 비대칭이 바로 Visitor가 Element 계층은 안정적이고 연산만 자주 추가되는 도메인(컴파일러 AST 분석, 문서 포맷 변환기)에 적합하고, 반대로 Element 타입이 자주 늘어나는 도메인에는 적합하지 않은 이유입니다.
...
어느 패턴을 적용할지는 앞서 정리한 "적용 시나리오 비교"와 "적용 체크리스트" 표를 기준으로 판단하면 충분하며, 여기서는 두 패턴 모두에 공통되는 실무 주의점만 짚는다. Memento는 상태 전체를 복사해 저장하므로 히스토리가 길어질수록 메모리 사용량이 늘어나 압축이나 차분 저장 같은 최적화가 필요하고, Visitor는 새로운 Element 타입이 하나 추가될 때마다 모든 ConcreteVisitor의 `visit()` 오버로드를 함께 수정해야 하므로 Element 계층이 자주 바뀌는 도메인에는 적합하지 않다. 두 패턴 모두 과도하게 적용하면 필요 이상의 클래스 계층만 늘어나는 오버엔지니어링으로 이어질 수 있다는 점도 공통적이다.
...
두 패턴 모두 <strong>"시간의 조작"</strong>과 <strong>"기능의 확장"</strong>이라는 복잡한 문제를 우아하게 해결하는 마법 같은 도구입니다. 현대 소프트웨어에서도 여전히 강력한 도구로 활용되고 있다.

다음 글에서는 **패턴의 조합과 상호작용**을 탐구하겠습니다. 여러 패턴을 함께 사용할 때의 시너지 효과와 아키텍처 설계 전략을 살펴보겠습니다.

## 토론 주제

1. **캡슐화 vs 편의성**: Memento를 Originator의 Nested Class로 구현하면 캡슐화는 지켜지지만 Originator와 강하게 결합된다. `Serializable` 기반 직렬화로 Memento를 구현하면 이 트레이드오프가 어떻게 달라지는가?
2. **Redo 스택 초기화**: `saveState()`가 항상 redo 스택을 비우는 설계는 사용자의 기대와 일치하는가? 되돌린 뒤 다른 분기로 편집을 이어가야 하는 브랜치형 히스토리가 필요하다면 이 스택 기반 설계로 충분한가?
3. **Visitor의 확장 축**: Element 계층과 연산 집합 중 어느 쪽이 더 자주 바뀔지 예측하기 어려운 도메인이라면, Visitor 대신 어떤 대안(패턴 매칭, `instanceof` 분기 등)을 검토할 수 있는가? 각 대안이 개방-폐쇄 원칙을 지키는 축은 Visitor와 같은가, 다른가?
4. **메모리와 복원 비용의 트레이드오프**: 차분(diff) 저장으로 Memento를 최적화하면 메모리는 절약되지만 복원 시 여러 차분을 순차 재생해야 한다. 어떤 상황에서 이 트레이드오프가 정당화되는가?
5. **Command와의 경계**: Command 패턴도 역연산 실행으로 Undo를 구현할 수 있다. Memento 기반 Undo와 Command 기반 Undo는 각각 어떤 상황에서 더 적합한 선택이 되는가?

## 참고 자료

- Erich Gamma, Richard Helm, Ralph Johnson, John Vlissides, *Design Patterns: Elements of Reusable Object-Oriented Software* (Addison-Wesley, 1994) — Memento, Visitor 챕터. 이 시리즈가 다루는 패턴 정의의 1차 출처.
- Joshua Bloch, *Effective Java* (3rd Edition, Addison-Wesley, 2018) — 불변 객체 설계와 방어적 복사 원칙(Memento의 불변 스냅샷 설계와 직결).
