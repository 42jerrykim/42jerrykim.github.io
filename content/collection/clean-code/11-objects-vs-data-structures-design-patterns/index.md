...
**디미터 법칙(Law of Demeter)**은 Ian Holland가 1987년 노스이스턴 대학 디미터 프로젝트에서 제안한 원칙으로, "모듈은 자신이 조작하는 객체의 속사정을 몰라야 한다"고 요약된다. 구체적으로는 클래스 `C`의 메서드 `f`가 호출할 수 있는 대상을 `C` 자신, `f`의 인수, `f`가 생성한 객체, `C`의 인스턴스 변수로 제한한다 — 이 객체들이 반환한 객체의 메서드는 호출하지 않아야 한다.
