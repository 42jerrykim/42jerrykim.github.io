...
마지막으로, 한 개념에는 한 단어만 고수해야 한다. `fetch`, `retrieve`, `get`을 프로젝트 전역에서 뒤섞어 쓰면 독자는 이 세 단어가 다른 동작을 뜻하는지 매번 확인해야 한다. 반대로 서로 다른 두 동작에 같은 단어를 재사용하는 **말장난(pun)**도 피해야 한다 — 기존 `add`가 "두 값을 더한다"는 뜻으로 쓰이고 있다면, "컬렉션에 값을 추가한다"는 새 동작에는 `insert`나 `append`처럼 다른 단어를 골라야 한다.
