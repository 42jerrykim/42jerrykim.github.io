...
이 비대칭은 실무에서 구체적인 결과로 나타난다 — 프레임워크에 버그를 리포트해도 당신의 우선순위대로 고쳐지지 않고, 메이저 버전이 올라가면서 당신이 의존하던 API가 아무 통보 없이 제거되기도 한다. 저자는 자신의 로드맵을 따를 뿐, 당신과의 계약을 맺은 적이 없기 때문이다.

### 결혼 vs 데이트

이 비대칭적 관계를 코드 수준으로 옮기면 "결혼"과 "데이트"의 차이가 된다. 프레임워크의 기반 클래스를 상속하거나 전역 어노테이션을 붙이는 것은 되돌리기 어려운 **결혼**이고, 인터페이스 뒤에서 필요한 만큼만 사용하는 것은 언제든 갈아탈 수 있는 **데이트**다.
...
지금까지 살펴본 위험과 해법을 규칙으로 정리하면 다음과 같다 — 공통적으로 "프레임워크의 존재를 비즈니스 코드에서 숨긴다"는 하나의 원칙에서 나온다.
...
이 원칙을 극단까지 밀어붙이면, 프레임워크는 코어에 꽂혀 있는 하나의 **플러그인**이 된다. 코어는 자신이 어떤 프레임워크와 연결되어 있는지 전혀 모르고, 플러그인 쪽에서 코어가 정의한 인터페이스를 구현해 연결을 시도한다 — 화살표가 항상 플러그인에서 코어로만 향한다는 점이 [32장에서 다룬 의존성 규칙](/post/clean-architecture/clean-architecture-concentric-circles-dependency/)과 정확히 같은 형태다.
