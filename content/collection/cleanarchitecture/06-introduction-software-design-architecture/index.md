...
마틴은 책의 서문을 여는 첫 문장에서부터 이렇게 단언한다: 소프트웨어를 올바르게 만드는 일은 어렵다(Martin, *Clean Architecture*, 2017, 서문).
...
마틴은 이를 이렇게 요약한다: 제대로 된 소프트웨어를 만들면 아주 적은 인력만으로도 새로운 기능을 추가하거나 유지보수할 수 있다(Martin, *Clean Architecture*, 2017, 서문). 구체적으로는 아래 세 가지 결과로 나타난다.
...
마틴은 자신의 수십 년 컨설팅 경력을 근거로, 다음과 같은 프로젝트를 직접 경험했다고 말한다(Martin, *Clean Architecture*, 2017, 서문):
...
마틴은 자신을 포함해 대다수의 개발자가 "천국"보다는 "지옥"에 훨씬 가까운 경험을 더 자주 한다고 인정한다(Martin, *Clean Architecture*, 2017, 서문). 아래는 그런 "지옥"의 전형적인 모습이다 — 원저자도, 리팩토링을 시도한 사람도, 지금 유지보수하는 사람도 이 코드의 전체 동작을 설명하지 못한다.
...
마틴은 이 책의 목표를 이렇게 정리한다: 좋은 소프트웨어 아키텍처가 무엇인지, 어떻게 만드는지를 설명하는 것이며, 비용은 최소화하고 생산성은 최대화할 수 있는 설계를 만들기 위해 알아야 할 모든 것을 다룬다(Martin, *Clean Architecture*, 2017, 서문).
