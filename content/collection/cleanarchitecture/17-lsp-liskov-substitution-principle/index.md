...
로버트 마틴은 LSP가 클래스 설계에 그치지 않고 아키텍처 수준까지 확장되어야 한다고 말한다. 치환 가능성을 조금이라도 위배하면 그 예외를 처리하기 위한 특별한 메커니즘이 시스템 곳곳에 스며들어, 결국 아키텍처 전체가 오염되기 때문이다(Martin, 『Clean Architecture』, 2017).
