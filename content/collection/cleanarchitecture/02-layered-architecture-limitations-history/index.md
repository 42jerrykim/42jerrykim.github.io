...
**MVC(Model-View-Controller)** 패턴은 트리그베 린스코그(Trygve Reenskaug)가 1979년 제록스 PARC에서 Smalltalk 환경을 위해 고안했으며, 이후 웹 애플리케이션의 표준 아키텍처가 되었다.
...
MVC는 책임을 셋으로 나눈다. Model이 상태와 규칙을 갖고, View는 그 상태를 보여주기만 하며, Controller가 사용자 입력을 받아 Model을 변경하고 적절한 View를 선택한다. 이 분리 덕분에 같은 Model을 여러 View(웹, 모바일 등)에서 재사용할 수 있다.
...
시간이 지나며 다양한 변형이 등장했는데, 갈라진 지점은 대체로 "View가 얼마나 수동적이어야 하는가"와 "데이터 흐름이 양방향인가 단방향인가"다.

- **MVP (Model-View-Presenter)**: View가 완전히 수동적이 되고, View와 Model 사이의 모든 로직을 Presenter가 중개해 View를 테스트하기 쉽게 만든다.
- **MVVM (Model-View-ViewModel)**: ViewModel이 View와 데이터 바인딩으로 연결되어, Controller가 수동으로 View를 갱신하는 코드를 줄인다.
- **MVI (Model-View-Intent)**: 사용자의 의도(Intent)가 단방향으로 흘러 상태를 변경하므로, 상태 변화를 추적하고 디버깅하기 쉽다.
