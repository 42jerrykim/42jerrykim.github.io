...
이 장의 원칙을 "아키텍처가 항상 최우선"으로 단순화하지 않는 것이 중요하다. 실제 판단 기준은 "지금 이 기능을 구조 개선 없이 그냥 구현하면, 나중에 되돌리는 비용이 얼마나 되는가"이다. 되돌리기 쉬운 영역은 빠르게 기능부터 만들고, 되돌리기 어려운 핵심 경계는 처음부터 구조에 투자하는 것이 현실적인 절충안이다.
