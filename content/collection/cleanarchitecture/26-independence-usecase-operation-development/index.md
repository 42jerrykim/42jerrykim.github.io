...
이 격자 구조에서 각 열(주문·결제·배송)은 독립된 유스케이스이자 독립된 팀의 작업 단위이고, 각 행(UI·비즈니스·데이터)은 같은 유스케이스 안에서도 서로 다른 이유로 바뀌는 코드다. 열 방향으로 나누면 유스케이스·개발 독립성이, 행 방향의 구분을 유지하면 계층 간 관심사 분리가 함께 성립한다.
