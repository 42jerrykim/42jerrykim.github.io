...
| 05 | SLO/SLA 정의 | 심화 | SLO/SLA 정의와 팀 합의, 에러버짓의 조직적 거버넌스화 트렌드(단, C++ 저지연 도메인 특화 근거는 약함) |
...
| 11 | 성능 코드 리뷰 | 중급 | 성능 관점 코드 리뷰 가이드, AI 코드리뷰(Claude Code Review 등) 워크플로우와 "AI 리뷰어(조기 시맨틱 피드백)+정적분석(결정론적 게이트)" 역할 분담 |
...
| 15 | 규제·보안 제약 하 성능 | 전문 | 규정 준수·보안 통제·성능 예산의 충돌과 완화 (Tr.06·Tr.10 연계) |
...
**Phase B — 예산·SLO·아키텍처 (챕터 04~09)** 지연 vs 처리량, 캐싱, DB 접근은 제품 요구와 직결됩니다. Tr.04·Tr.09·Tr.10와 **질문을 나누는** 연습이 필요합니다.

**Phase C — 조직·운영 (챕터 10~16)** 팀 문화, 코드 리뷰, 용량 계획, 부하 테스트, 클라우드 비용 및 규제/안전성 트레이드오프는 **심화~전문**입니다. Tr.12(회귀 방지)과 세트로 읽으면 “목표→검증→유지”가 닫힙니다.
