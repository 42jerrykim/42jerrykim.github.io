...
description: "CPU 스케줄링은 제한된 CPU 코어를 여러 프로세스·스레드에 어떤 순서로 배정할지 결정하는 운영체제의 핵심 기능입니다. FCFS, SJF, 라운드 로빈을 비교하고, 콘보이 효과·기아·우선순위 역전 문제와 각 정책을 언제 선택해야 하는지까지 다룹니다."
tags:
- Technology(기술)
- Education(교육)
...
```mermaid
sequenceDiagram
    participant A as "A (낮은 우선순위)"
    participant B as "B (중간 우선순위)"
    participant C as "C (높은 우선순위)"
    A->>A: "락 획득"
    C->>A: "락 대기 시작"
    B->>B: "A를 선점하고 실행"
    Note over A,C: B가 실행되는 동안 A는 락을 놓지 못하고, C는 계속 대기
    B->>A: "실행 종료, A로 복귀"
    A->>C: "락 해제"
    C->>C: "드디어 실행"
```

이 다이어그램에서 C가 가장 급한 작업임에도 B에 의해 간접적으로 지연되는 것이 우선순위 역전의 핵심이다. 우선순위 상속을 적용하면 C가 A의 락을 기다리는 순간 A의 우선순위가 일시적으로 C 수준까지 올라가, B가 A를 선점하지 못하게 된다.
...
이 표에서 실무 판단의 기준은 **워크로드의 성격**이다. 배치 처리처럼 사용자가 응답을 기다리지 않고 전체 처리량과 평균 대기 시간이 중요하다면 SJF(또는 그 근사치)가 유리하다. 반대로 사용자가 즉시 반응을 기대하는 대화형·실시간 시스템(데스크톱 OS, 게임)에서는 라운드 로빈처럼 모든 작업에 짧은 주기로 CPU를 골고루 나눠주는 방식이 필요하다. 구현 단순성과 예측 가능성이 최우선이고 작업 길이 편차가 크지 않다면 FCFS로도 충분한 경우가 있다. 실제 리눅스 커널이 쓰는 CFS(Completely Fair Scheduler)는 이 세 정책 중 하나를 그대로 쓰지 않고, "각 작업이 CPU를 공정하게 나눠 가졌는가"를 가상 실행 시간으로 추적하는 별도 방식을 쓴다.
...
스케줄링이 여러 실행 흐름을 번갈아 실행시키기 때문에, 공유 자원에 동시에 접근하는 문제([레이스 컨디션과 락](/post/computerterms/race-conditions-and-locks/), [데드락](/post/computerterms/deadlocks/))가 발생한다. 우선순위 역전에서 다룬 락 경쟁은 [데드락](/post/computerterms/deadlocks/) 챕터와 직접 연결된다.
