...
description: "레이스 컨디션은 여러 스레드가 공유 자원에 동시에 접근할 때 실행 순서에 따라 결과가 달라지는 버그입니다. 뮤텍스로 임계 구역을 보호하는 원리를 C 코드로 재현하고 고치며, 그 대가인 성능 비용과 대안까지 다룹니다."
tags:
- Technology(기술)
- Education(교육)
- Concurrency(동시성)
...
sequenceDiagram
    ...
    participant Mem as "counter (메모리)"
    ...
    T1->>Mem: "0+1=1 쓰기"
    T2->>Mem: "0+1=1 쓰기"
    Note over Mem: "두 번 증가했지만 결과는 1 — 갱신 하나가 유실됨"
```
...
**언제 뮤텍스를 쓰고, 언제 다른 도구로 대체할 것인가**는 임계 구역이 무엇을 보호하느냐에 달려 있다. 이 예제처럼 정수 하나를 증가시키는 것이 임계 구역의 전부라면, 뮤텍스의 lock/unlock 오버헤드 없이 하드웨어가 직접 원자적으로 처리하는 연산으로 대체할 수 있다 — 동시성 갈래의 이후 챕터에서 다룰 [원자적 연산과 CAS](/post/computerterms/atomic-operations-and-cas/)가 이런 경우에 더 적합하다. 반대로 임계 구역이 여러 줄에 걸친 복합 로직(예: 계좌 잔액을 확인한 뒤 차감)이라면 원자적 연산 하나로는 표현할 수 없으므로 뮤텍스가 필요하다. 즉 "단일 값의 단순 갱신인가, 여러 단계로 이뤄진 복합 연산인가"가 뮤텍스와 원자적 연산 중 무엇을 선택할지 가르는 기준이다.
