...
description: "힙은 완전 이진 트리를 배열에 담아 최솟값 삽입·삭제를 O(log n)에 처리하는 자료구조입니다. sift-up/down 원리와 우선순위 큐로서의 쓰임을 컴파일 가능한 C 코드로 다루고, 배열 기반 힙이 다익스트라의 decrease-key를 직접 지원하지 못하는 한계와 실무 우회법까지 설명합니다."
tags:
- Technology(기술)
- Education(교육)
- Data-Structure(자료구조)
...
값 `10, 5, 3, 8, 1`을 이 순서로 삽입한 최소 힙은 배열 `[1, 3, 5, 10, 8]`이 되며(뒤에 나올 sift-up 규칙을 그대로 적용한 결과), 트리와 배열 인덱스는 다음과 같이 대응한다.

```mermaid
graph TD
    I0["idx 0: 1"] --> I1["idx 1: 3"]
    I0 --> I2["idx 2: 5"]
    I1 --> I3["idx 3: 10"]
    I1 --> I4["idx 4: 8"]
```

`idx 1`(값 3)의 부모는 `(1-1)/2 = idx 0`, 자식은 `2*1+1 = idx 3`과 `2*1+2 = idx 4`로, 배열 인덱스만으로 트리 상의 위치가 그대로 계산됨을 확인할 수 있다.
...
이 탐색 불가 성질은 앞서 예로 든 다익스트라 알고리즘에서 실제 문제가 된다. 다익스트라는 정점의 거리가 갱신될 때마다 힙에 이미 들어 있는 그 정점의 우선순위를 낮춰야 하는데(decrease-key), 배열 기반 힙은 "정점 v가 배열의 몇 번 인덱스에 있는가"를 알 방법이 없어 이 갱신 대상을 O(n) 선형 탐색 없이는 찾을 수 없다. [최단 경로 알고리즘](/post/computerterms/shortest-path-algorithms/)의 구현이 이 문제를 우회하는 방법은 두 가지다. 하나는 정점 번호를 배열 인덱스로 매핑하는 별도 표(인덱스 힙, Indexed Priority Queue)를 함께 유지해 O(log n)에 위치를 찾는 것이고, 다른 하나는 갱신할 때마다 기존 항목을 지우지 않고 새 (거리, 정점) 쌍을 그냥 추가로 push한 뒤(Lazy Deletion), pop 시점에 "이미 처리된 정점인지"만 확인해 오래된 항목을 건너뛰는 것이다. 후자가 구현이 단순해 실무에서 더 널리 쓰인다.
