...
description: "CPU가 하드웨어 이벤트에 즉시 반응하는 인터럽트와, 사용자 프로그램이 커널 기능을 요청하는 시스템 콜의 원리를 사용자 모드·커널 모드 전환과 함께 다룹니다."
tags:
- Technology(기술)
- Education(교육)
...
시스템 콜로 커널에 진입한 뒤 프로세스가 생성·종료되는 과정은 다음 챕터인 데몬과 좀비 프로세스에서 `fork()`·`wait()` 시스템 콜과 함께 이어진다. 타이머 인터럽트가 스케줄링의 기반이 되는 부분은 스케줄링 챕터에서 더 다룬다.
