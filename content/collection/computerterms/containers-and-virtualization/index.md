...
description: "하이퍼바이저 기반 VM과 커널 네임스페이스·cgroup 기반 컨테이너의 격리 수준·오버헤드 차이를 비교하고, unshare 시스템 콜로 PID 네임스페이스를 직접 만들어보는 코드와 언제 무엇을 선택할지의 판단 기준을 다룹니다."
tags:
- Technology(기술)
- Education(교육)
...
이 표에서 실무 판단의 기준은 **격리 수준과 자원 효율 중 무엇이 더 중요한가**다. 이기종 OS를 함께 돌려야 하거나, 신뢰할 수 없는 코드를 커널 수준까지 격리해야 하는 멀티테넌트 환경이라면 VM이 안전하다. 반대로 같은 리눅스 커널 위에서 짧은 시간에 많은 인스턴스를 배포·확장해야 하거나(마이크로서비스, CI 빌드 작업 등) 이미지 크기와 시작 속도가 중요하다면 컨테이너가 유리하다. 실무에서는 두 방식을 함께 쓰는 경우도 흔하다 — 예를 들어 클라우드 환경은 먼저 VM으로 테넌트 간 강한 격리를 확보한 뒤, 그 VM 내부에서 다시 컨테이너로 애플리케이션을 배포해 두 계층의 이점을 모두 취한다.
...
**"도커가 컨테이너를 발명했다"** — 리눅스 네임스페이스(2002년 리눅스 2.4.19의 마운트 네임스페이스를 시작으로 단계적으로 커널에 추가)와 cgroup(2006년 구글 엔지니어 Paul Menage·Rohit Seth가 "프로세스 컨테이너"라는 이름으로 처음 기여, 2007년경 "cgroups"로 명명되어 2008년 리눅스 2.6.24에 병합)은 도커(2013년 공개) 이전부터 존재했다. LXC(Linux Containers, 2008년)도 도커보다 먼저 이 커널 기능들을 조합해 컨테이너를 구현했다. 도커의 기여는 이미지 빌드·배포·레지스트리 같은 **개발자 경험**을 표준화해 컨테이너를 대중화한 것이지, 격리 메커니즘 자체를 발명한 것이 아니다.
