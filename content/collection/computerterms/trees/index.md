...
description: "트리는 노드 사이에 계층 관계를 갖는 자료구조로, 이진 탐색 트리와 균형 트리를 중심으로 순회 방식·탐색 성능·불균형 문제를 다룹니다. 정렬된 값을 순서대로 삽입하면 O(n)으로 퇴화하는 이유와, AVL·레드-블랙 트리가 회전으로 이를 막는 원리를 컴파일 가능한 C 코드와 함께 설명합니다."
tags:
- Technology(기술)
- Education(교육)
- Data-Structure(자료구조)
...
`{5, 3, 8, 1, 4, 7, 9}`를 이 순서로 삽입하면 다음과 같은 트리 모양이 된다.
...
중위 순회(왼쪽→현재→오른쪽)로 이 트리를 따라가면 1 → 3 → 4 → 5 → 7 → 8 → 9 순으로 방문해, 코드 출력과 정확히 일치한다.
...
이 재조정의 핵심 메커니즘이 **회전(Rotation)**이다. 예를 들어 오른쪽 자식만 계속 늘어나는 편향이 감지되면, 현재 노드를 왼쪽으로 한 단계 내리고 그 오른쪽 자식을 새 부모로 끌어올리는 **좌회전(Left Rotation)**을 수행한다 — 이때 끌어올려진 노드의 원래 왼쪽 서브트리는 내려간 노드의 오른쪽 서브트리로 옮겨 붙는데, BST의 정렬 규칙(왼쪽<현재<오른쪽)은 이 재배치 후에도 그대로 유지된다. AVL 트리는 각 노드마다 왼쪽·오른쪽 서브트리의 높이 차(균형 인수)가 1을 넘는 순간 삽입·삭제 경로를 따라 올라가며 회전으로 바로잡고, 레드-블랙 트리는 노드에 색(빨강/검정)을 부여해 "루트에서 리프까지의 검정 노드 수가 모두 같다"는 규칙이 깨질 때만 회전과 재색칠을 수행한다. 두 방식 모두 회전 자체가 O(1)이고 삽입·삭제당 회전 횟수가 O(log n)을 넘지 않도록 설계되어 있어, 재조정 비용을 더해도 전체 삽입·삭제는 여전히 O(log n)에 머문다.
