...
다음에 다룰 **해시테이블**은 평균 O(1) 탐색으로 균형 BST의 O(log n)보다 빠르지만, 정렬된 순서로 순회하는 기능을 잃는다는 트레이드오프가 있다 — "정렬이 필요한가"가 둘 중 하나를 고르는 핵심 판단 기준이 된다. 그래프 챕터에서는 트리가 "사이클이 없는 연결 그래프"의 특수한 경우임을 다룬다.
