...
WAF가 검사하는 HTTP 요청 구조는 [HTTP와 HTTPS](/post/computerterms/http-and-https/)에서 다룬 요청·응답 모델을 그대로 쓰고, WAF의 배치 위치는 [정방향·역방향 프록시](/post/computerterms/forward-and-reverse-proxies/)에서 다룬 리버스 프록시 패턴과 겹친다. 다음 챕터에서는 이 공개키 기반 서명이 "이 데이터를 누가 작성했는가"를 증명하는 디지털 서명과 인증서로 이어진다.
