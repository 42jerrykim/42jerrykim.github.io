...
description: "가상 메모리는 각 프로세스에게 실제 물리 메모리보다 넓고 독립된 주소 공간을 제공하는 추상화입니다. 페이징과 페이지 폴트, 스와핑을 malloc 동작과 함께 다루고, mmap과 malloc 중 언제 무엇을 선택할지의 판단 기준까지 설명합니다."
tags:
- Technology(기술)
- Education(교육)
...
## malloc vs mmap: 언제 무엇을 쓰는가

작은 메모리 블록을 자주 할당·해제한다면 `malloc`이 유리하다. 표준 라이브러리가 내부적으로 힙 영역을 관리하며 `free`된 블록을 재사용하므로, 매번 커널에 시스템 콜을 요청하지 않고도 빠르게 할당·해제를 반복할 수 있다. 반면 수십 MB 이상의 큰 블록을 한 번에 할당하거나, 파일 내용을 메모리처럼 다루고 싶거나(파일 매핑), 여러 프로세스가 매핑을 공유해야 한다면 `mmap`을 직접 쓰는 것이 유리하다 — glibc의 `malloc` 구현도 내부적으로 일정 크기(기본 128KB)를 넘는 요청은 자동으로 `mmap`으로 전환하는 임계값(`M_MMAP_THRESHOLD`)을 둔다. 페이지 폴트 지연 자체를 피해야 하는 실시간성이 중요한 상황이라면, 할당 직후 모든 페이지를 미리 터치해 물리 프레임을 강제로 붙이거나 `mmap`의 `MAP_POPULATE` 플래그로 첫 접근 지연을 없앨 수 있다.
