...
description: "한 서비스의 장애가 이를 호출하는 다른 서비스로 연쇄 전파되는 문제와, 실패율이 임계값을 넘으면 타임아웃 중인 호출을 차단해 장애 확산을 막는 서킷 브레이커의 Closed/Open/Half-Open 상태 전이, 그리고 서비스 메시 계층에서의 적용까지 다룹니다."
tags:
- Technology(기술)
- Education(교육)
...
**서킷 브레이커(Circuit Breaker)**는 가정용 전기 회로의 차단기에서 이름을 딴 패턴으로, 과전류가 흐르면 회로 전체가 타버리기 전에 차단기가 회로를 끊어버리는 것과 같은 원리를 소프트웨어에 적용한다. 호출하는 쪽(B)이 호출받는 쪽(C)의 최근 실패율을 계속 관찰하다가, 실패율이 미리 정한 임계값을 넘으면 **그 이후의 호출을 아예 C로 보내지 않고 즉시 실패로 처리**한다. 이 패턴은 마틴 파울러(Martin Fowler)가 2014년 글에서 정리하며 마이크로서비스 커뮤니티에 널리 알려졌으며, 원래는 Michael T. Nygard가 저서 『Release It! Design and Deploy Production-Ready Software』(Pragmatic Bookshelf, 2007)의 "Stability Patterns" 부에서 안정성 패턴 중 하나로 제시한 개념이다. 이런 패턴들을 통칭해 시스템이 장애를 겪고도 계속 기능할 수 있는 능력을 **회복 탄력성(Resilience)**이라 부르는데, 서킷 브레이커는 재시도·타임아웃·폴백과 함께 회복 탄력성을 구성하는 대표적인 구성 요소다.
...
[로드 밸런싱](/post/computerterms/load-balancing/)의 헬스 체크는 로드 밸런서가 각 서버 인스턴스에 주기적으로 핑을 보내 "이 서버가 살아있는가"를 판단하고, 죽은 서버를 트래픽 분배 대상에서 제외하는 방식이다. 서킷 브레이커는 이와 유사해 보이지만 관측 지점이 다르다. 헬스 체크는 로드 밸런서가 **별도의 검사 요청**으로 서버 상태를 능동적으로 확인하는 반면, 서킷 브레이커는 **실제 호출의 성공/실패 결과**와 **타임아웃(Timeout)** 발생 여부를 관찰해 판단한다. 또한 헬스 체크는 여러 인스턴스 중 죽은 것을 골라내는 것이 목적이지만, 서킷 브레이커는 특정 의존 서비스(또는 그 서비스의 특정 엔드포인트) 전체로의 호출 자체를 끊는 것이 목적이다. 실무에서는 두 기법을 함께 쓴다 — 로드 밸런서가 죽은 인스턴스를 제외하는 동안, 서킷 브레이커는 그 서비스 전체가 불안정할 때 호출 시도 자체를 줄여준다.

애플리케이션 코드마다 서킷 브레이커 로직을 직접 구현하는 대신, **서비스 메시(Service Mesh)** 계층에 위임하는 방식도 널리 쓰인다. Istio나 Linkerd 같은 서비스 메시는 각 서비스 옆에 사이드카 프록시(예: Envoy)를 두고, 그 프록시가 타임아웃·재시도·서킷 브레이커 정책을 애플리케이션 코드 변경 없이 설정 파일만으로 적용한다. 이 접근의 장점은 언어·프레임워크가 다른 여러 서비스에 동일한 **장애 허용(Fault Tolerance)** 정책을 일관되게 적용할 수 있다는 것이고, 단점은 프록시 계층이 추가되며 발생하는 지연과 운영 복잡도다. 애플리케이션 레벨 라이브러리(resilience4j 등)와 서비스 메시 중 무엇을 선택할지는, 서비스가 단일 언어 스택인지 여러 언어가 섞여 있는지, 그리고 이미 쿠버네티스·서비스 메시 인프라를 운영 중인지에 따라 갈린다.
