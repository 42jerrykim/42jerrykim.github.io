...
```javascript
async function transferData(client) {
...
**"ACID 속성 4개는 서로 독립적이다"** — 실제로는 서로 얽혀 있다. 예를 들어 고립성이 깨지면(다른 트랜잭션의 커밋 전 상태를 봄) 일관성 규칙 위반으로 이어질 수 있고, 원자성이 없으면 영구성이 "일부만 완료된 상태"를 영구히 저장해버리는 문제로 이어진다. 4가지를 독립된 체크리스트 항목으로만 보면, 한 속성을 지키기 위한 구현(잠금, 로그)이 다른 속성에 미치는 영향을 놓치기 쉽다.
