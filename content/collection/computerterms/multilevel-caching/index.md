...
description: "CPU의 L1/L2/L3 캐시 계층 개념을 애플리케이션 레벨로 확장합니다. 브라우저 캐시부터 CDN, Redis, DB 캐시까지 이어지는 계층 구조와 캐시 미스 전파를 다룹니다."
tags:
- Technology(기술)
- Education(교육)
- Caching(캐싱)
...
**"캐시 계층이 많을수록 무조건 빠르다"** — 계층이 늘어날수록 각 계층에서 무효화해야 할 사본의 수도 함께 늘어난다. [캐싱과 캐시 무효화](/post/computerterms/caching-and-invalidation/)에서 다룬 무효화 문제가 계층마다 독립적으로 발생하므로, 데이터가 바뀌었을 때 네 계층 중 하나라도 무효화를 빠뜨리면 사용자마다 다른 계층에서 서로 다른 버전의 데이터를 볼 수 있다. 계층 수는 "성능과 일관성 관리 비용의 트레이드오프"로 판단해야 한다.
