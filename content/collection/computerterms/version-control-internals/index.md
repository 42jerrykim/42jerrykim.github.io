...
description: "Git은 파일의 변경 이력을 저장하는 것이 아니라, 매 커밋마다 전체 스냅샷을 해시로 저장하는 그래프 기반 시스템입니다. 커밋·브랜치·머지의 내부 자료구조를 다룹니다."
tags:
- Technology(기술)
- Education(교육)
...
각 커밋은 자신의 내용(트리, 부모, 메타데이터)을 [암호화와 해싱](/post/computerterms/encryption-and-hashing/)에서 다룬 암호학적 해시 함수(SHA-1, 최신 버전은 SHA-256으로 이전 중)로 해싱해 만든 40자리(또는 64자리) 문자열을 자신의 ID로 쓴다. 이 방식의 핵심 성질은, 내용이 조금이라도 바뀌면 해시값이 완전히 달라진다는 [암호화와 해싱](/post/computerterms/encryption-and-hashing/)의 **눈사태 효과**다.
...
이 성질 덕분에, 커밋 해시가 같다는 것은 그 커밋의 내용(그리고 그 이전 모든 히스토리)이 절대 변조되지 않았다는 것을 수학적으로 보장한다 — [ACID Transactions](/post/computerterms/acid-transactions/)에서 다룬 데이터 무결성 보장을 해시 체인으로 구현한 것이다.
...
**"git rebase는 커밋 내용을 수정하는 것이다"** — rebase는 기존 커밋을 "수정"하는 것이 아니라, 같은 변경 내용을 담은 **새 커밋을 만들어** 다른 부모 위에 다시 쌓는 것이다. 새 커밋은 부모가 다르므로 해시값도 달라진다 — 원본 커밋은 그대로 남아있고(가비지 컬렉션 전까지), 브랜치 포인터만 새 커밋 체인을 가리키도록 바뀐다. 이미 원격에 공유된 커밋을 rebase하면 안 되는 이유가 여기 있다 — 다른 사람은 여전히 원본 해시를 참조하고 있는데 내 쪽만 새로운 해시로 히스토리가 바뀌어 그래프가 어긋난다.
...
- [Git Internals (공식 문서)](https://git-scm.com/book/en/v2/Git-Internals-Git-Objects) — 커밋·트리·블롭 객체의 실제 저장 구조
- [Git Internals - Git Objects](https://git-scm.com/book/en/v2/Git-Internals-Git-References) — 브랜치가 포인터일 뿐이라는 것을 실습으로 확인하는 공식 가이드
