...
description: "CDN은 사용자와 지리적으로 가까운 엣지 서버에 콘텐츠를 캐싱해 지연시간을 줄이는 기술입니다. 오리진과 엣지의 캐시 흐름, 정적·동적 콘텐츠에서 효과 차이를 다루고, GeoDNS가 사용자를 가장 가까운 엣지로 연결하는 원리까지 함께 설명합니다."
tags:
- Technology(기술)
- Education(교육)
- Networking(네트워킹)
...
서울의 사용자가 미국 동부에 있는 서버에 요청을 보내면, 실제 라우팅 경로와 중간 장비를 거치는 오버헤드까지 감안할 때 왕복에 대략 150~200밀리초가 걸린다(경로·통신사·측정 시점에 따라 달라진다). 이 지연은 서버 성능을 아무리 높여도 줄지 않는다 — 광케이블을 통과하는 신호가 물리적 거리를 이동하는 시간 자체가 병목이기 때문이다. **CDN(Content Delivery Network)**은 이 문제를 서버 성능이 아니라 서버 위치로 푼다. 원본 콘텐츠를 갖고 있는 **오리진 서버(Origin Server)** 외에, 세계 각지에 분산된 **엣지 서버(Edge Server)**에 콘텐츠 사본을 캐싱해 두고, 사용자를 지리적으로 가장 가까운 엣지 서버로 연결한다. 서울 사용자는 서울(또는 인접 지역) 엣지 서버에서 응답을 받으므로 왕복 지연이 수 밀리초 수준으로 줄어든다.
