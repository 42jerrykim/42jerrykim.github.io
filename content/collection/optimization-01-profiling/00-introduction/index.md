...
BPF(챕터 16)·벤더 전용 도구(챕터 06 VTune, 챕터 13 μProf)로 확장하기 전에 확인할 조건이 있습니다. 커널이 요구 BPF 기능(CO-RE, `CAP_BPF`)을 지원하는지, 프로덕션 환경에서 해당 도구를 실행할 권한을 받았는지, 그리고 표준 도구(perf, VTune)로 얻은 병목 후보를 먼저 재현했는지입니다. 전문 도구의 결과는 표준 도구 결과와 대조해 검증한 뒤에만 채택하십시오 — 단독 결과만으로 프로덕션 변경을 정당화하지 않습니다.
