...
slug: command-categories
title: "[CMD] 부록. 명령어 종류 총정리 - 카테고리별 핵심 명령어 가이드"
...
| 명령어 | 역할 |
|---|---|
| dir | 디렉터리 안의 파일·하위 디렉터리 목록을 표시 |
| cd | 현재 디렉터리를 표시하거나 변경 |
| md (mkdir) | 디렉터리를 생성 |
| rmdir (rd) | 디렉터리를 삭제 |
| copy | 파일을 다른 위치로 복사 |
| xcopy | 디렉터리 트리를 포함해 파일을 복사 |
| robocopy | 재시작·재시도·미러링을 지원하는 고급 복사 유틸리티 |
| move | 파일·디렉터리를 다른 위치로 이동 |
| del (erase) | 파일을 삭제 |
| ren (rename) | 파일 이름을 변경 |
| type | 텍스트 파일 내용을 화면에 출력 |
| tree | 디렉터리 구조를 트리 형태로 표시 |
| attrib | 파일 속성(읽기 전용·숨김 등)을 표시·변경 |
| comp | 두 파일을 바이트 단위로 비교 |
| fc | 두 파일의 내용 차이를 줄 단위로 표시 |
| find | 파일이나 출력에서 문자열을 검색 |
| findstr | 정규식을 지원하는 문자열 검색 |
| mklink | 심볼릭 링크·하드 링크를 생성 |
| replace | 대상 디렉터리의 파일을 원본 파일로 교체 |
| sort | 텍스트 입력을 정렬해 출력 |
| more | 긴 출력을 한 화면씩 나눠 표시 |
| pushd-popd | 현재 디렉터리를 스택에 저장하고 복원 |
| path | 실행 파일 검색 경로(PATH)를 표시·설정 |
...
복사 계열 세 명령어(copy·xcopy·robocopy)는 기능이 겹쳐 보이지만 다루는 범위가 다르다. 파일 한두 개를 옮길 때는 `copy`로 충분하고, 하위 디렉터리 구조까지 통째로 복사해야 하면 `xcopy`가 필요하다. 네트워크 드라이브 복사처럼 중간에 끊길 수 있는 대용량 작업이거나 원본·대상을 동기화(미러링)해야 한다면, 재시도·로그·증분 복사를 지원하는 `robocopy`를 우선 고려한다.
...
| 명령어 | 역할 |
|---|---|
| chkdsk | 디스크 오류를 검사하고 상태 보고서를 표시 |
| chkntfs | 부팅 시 디스크 검사 여부를 표시·변경 |
| diskpart | 디스크 파티션을 대화형으로 관리 |
| format | 드라이브를 지정한 파일 시스템으로 포맷 |
| convert | FAT 볼륨을 NTFS로 변환 |
| compact | NTFS 파일·폴더의 압축 상태를 표시·변경 |
| fsutil | 파일 시스템 속성을 조회·구성하는 저수준 도구 |
| label | 디스크 볼륨 레이블을 생성·변경·삭제 |
| vol | 디스크 볼륨 레이블과 일련번호를 표시 |
| subst | 경로를 가상 드라이브 문자에 연결 |
| recover | 손상된 디스크에서 읽을 수 있는 데이터를 복구 |
| verify | 파일이 디스크에 올바로 기록됐는지 검증할지 설정 |
...
| 명령어 | 역할 |
|---|---|
| tasklist | 현재 실행 중인 프로세스 목록을 표시 |
| taskkill | 실행 중인 프로세스를 종료 |
| sc | Windows 서비스를 조회·생성·구성 |
| schtasks | 예약 작업을 생성·조회·삭제 |
| start | 프로그램이나 명령을 별도 창에서 실행 |
...
| 명령어 | 역할 |
|---|---|
| icacls | 파일·디렉터리의 ACL을 표시·수정·백업·복원 |
| cacls | ACL을 표시·수정하는 구버전 도구(icacls 권장) |
| openfiles | 파일 공유에서 원격 사용자가 열어 둔 파일을 표시 |
...
OS 버전, 설치된 드라이버, 콘솔 장치 설정처럼 시스템 자체의 상태를 조회하거나 구성하는 명령어들이다. 어떤 정보를 찾느냐에 따라 시작점이 달라진다 — OS 빌드·메모리·핫픽스 목록처럼 시스템 전체 개요가 필요하면 `systeminfo`, 특정 하드웨어의 드라이버 버전·서명 상태가 궁금하면 `driverquery`, WMI 클래스 단위로 더 세밀하게 조회·제어해야 하면 `wmic`(단, 아래 설명대로 지원 중단 예정)을 택한다.

| 명령어 | 역할 |
|---|---|
| systeminfo | 컴퓨터의 하드웨어·OS 구성 정보를 표시 |
| ver | Windows 버전을 표시 |
| driverquery | 설치된 장치 드라이버 상태를 표시 |
| gpresult | 적용된 그룹 정책 정보를 표시 |
| mode | 콘솔·시리얼 포트 등 시스템 장치를 구성 |
| wmic | WMI를 통해 시스템 정보를 조회·제어(레거시, PowerShell 권장) |
| date-time | 시스템 날짜·시간을 표시·설정 |
| chcp | 활성 코드 페이지 번호를 표시·설정 |
| color | 콘솔의 전경·배경색을 설정 |
| cls | 화면을 지움 |
...
Microsoft는 Windows 10 21H1에서 `wmic`(WMIC) 지원 중단을 발표했고, 이후 릴리스에서는 WMIC 기능이 기본 설치에서 빠지는 선택적 기능(Feature on Demand)으로 전환될 예정이다. 신규 스크립트에서는 `Get-CimInstance` 등 PowerShell 대체 명령을 권장한다. 기존 배치 스크립트를 유지 보수하는 상황이 아니라면 새로 작성하는 자동화에는 PowerShell을 우선 고려하는 것이 좋다.
...
| 명령어 | 역할 |
|---|---|
| echo | 메시지를 표시하거나 화면 표시 여부를 설정 |
| if | 조건에 따라 분기 처리 |
| for | 파일 집합·범위를 순회하며 명령을 반복 실행 |
| call | 다른 배치 파일이나 레이블을 하위 루틴처럼 호출 |
| goto | 지정한 레이블로 실행 흐름을 이동 |
| set | 환경 변수를 표시·설정·제거 |
| setlocal-endlocal | 환경 변수 변경 범위를 배치 파일 내로 국한 |
| shift | 배치 매개변수(%1, %2 ...)의 위치를 한 칸씩 이동 |
| pause | 처리를 일시 중단하고 메시지를 표시 |
| exit | cmd.exe 세션이나 배치 스크립트를 종료 |
| title | 콘솔 창의 제목을 설정 |
| prompt | 명령 프롬프트 표시 형식을 변경 |
| break | 확장된 Ctrl+C 검사를 설정·해제 |
| doskey | 명령줄 기록·매크로를 관리 |
| rem | 배치 파일에 주석을 작성 |
...
| 명령어 | 역할 |
|---|---|
| assoc | 파일 확장명과 연결된 파일 형식을 표시·수정 |
| ftype | 파일 형식에 연결된 실행 명령을 표시·수정 |
| bcdboot | 시동 구성 데이터(BCD)를 관리 |
| bcdedit | 부팅 로더 항목을 설정 |
| cmd | Windows 명령 인터프리터의 새 인스턴스를 시작 |
| help | Windows 명령에 대한 도움말 정보를 제공 |
| graftabl | 그래픽 모드에서 확장 문자 세트 표시를 설정 |
| print | 텍스트 파일을 프린터로 인쇄 |

---

## 이 글로 확인할 것

이 색인을 훑은 뒤 아래 항목에 스스로 답할 수 있다면 카테고리 구조를 실무에 적용할 준비가 된 것이다.

- 파일 하나를 지우려는데 명령어 이름이 기억나지 않을 때, "파일·디렉터리 조작" 카테고리부터 찾아야 한다는 것을 안다.
- 내부 명령어와 외부 명령어의 차이를 `where` 명령으로 직접 구분할 수 있다(`where dir`은 실패하고 `where robocopy`는 경로를 반환한다).
- `cacls` 대신 `icacls`를 써야 하는 이유와, 기존 배치 파일의 `cacls` 옵션을 `icacls`로 그대로 옮길 수 없는 이유를 설명할 수 있다.
- `taskkill /F`나 `diskpart`의 `clean`처럼 되돌릴 수 없는 명령을 실행하기 전에 어떤 확인 절차를 거쳐야 하는지 말할 수 있다.
