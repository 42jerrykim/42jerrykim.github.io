...
85챕터 안에도 의도적으로 넣지 않은 명령이 있다. `reg`(레지스트리 조회·설정)는 잘못 다루면 시스템을 손상시킬 수 있는 별도의 위험 영역이라 이 커리큘럼의 범위 밖에 남겨두었고, `netsh`(네트워크 인터페이스·방화벽 구성)와 `net use`/`net share`/`net view`(네트워크 파일 공유)는 8부가 다루는 로컬 진단(`ipconfig`/`ping`/`nslookup`)보다 도메인·서버 환경 지식을 더 요구해 제외했다. `takeown`(소유권 강제 획득)은 60–61장이 다루는 ACL 관리(`icacls`/`cacls`)와 인접하지만 별도 명령이고, `whoami`·`setx`는 각각 계정 조회·영구 환경 변수 설정이라는 좁은 용도라 우선순위에서 밀렸으며, `choice`는 `if`/`for`(32–33장)로도 흉내 낼 수 있는 대화형 분기라 별도 챕터를 두지 않았다. `sfc`(시스템 보호 파일 무결성 검사·복구)도 마찬가지다 — 42장의 `chkdsk`가 디스크·파일시스템 구조를 검사하는 것과 달리 `sfc`는 그 위 계층인 OS 시스템 파일의 무결성을 다루는 별개 도구라 5부의 초점(디스크·파일시스템)에서 벗어난다고 판단해 제외했다. 이 명령들은 완주 후 개별 학습으로 남겨둔다.
...
이 순서를 무시하고 필요한 장만 골라 읽는 것도 가능하지만, 그 경우 흔히 겪는 실패 패턴이 있다. 2–3부(파일 조작·텍스트 검색) 없이 4부(배치 스크립팅)부터 펼치면 `for /f`가 반복하는 대상이 파일 이름인지 명령어 출력의 한 줄인지 구분하지 못해, 예제를 그대로 옮겨 써도 원하는 결과가 나오지 않는다. 6부(프로세스·서비스와 권한 관리) 없이 5부(디스크 관리)의 `diskpart`부터 실행하면, 대상 볼륨을 점유한 프로세스를 `tasklist`로 먼저 확인해야 한다는 안전 절차 자체를 놓치기 쉽다. 이런 실패는 대개 그 챕터가 다루는 명령어의 문법을 몰라서가 아니라, 그 명령어가 당연히 전제하는 앞 단계의 개념이 비어 있어서 생긴다 — 이 컬렉션이 알파벳순이나 사전순이 아니라 의존성 순서로 설계된 이유가 여기에 있다.
...
Part 6(프로세스·서비스와 권한 관리, 54–62장)은 실행 중인 프로그램(`tasklist`/`taskkill`), 백그라운드 서비스와 예약 작업(`sc`/`schtasks`), 그리고 파일 접근 권한(`icacls`/`cacls`/`openfiles`)을 다룬다. 디스크 관리(5부) 다음에 배치한 이유는, 실무에서 이 작업 대부분이 "디스크 공간이 부족해 특정 프로세스를 종료하고 관련 서비스를 재시작한다"처럼 앞서 배운 지식과 이어지는 시나리오로 등장하기 때문이다. 권한 관리를 이 Part의 마지막에 둔 것도 같은 논리다 — 프로세스와 서비스를 다룰 줄 알아야, 그 프로세스가 왜 특정 파일에 접근하지 못하는지도 진단할 수 있다.

Part 7(시스템 정보와 구성, 63–71장)은 `systeminfo`/`ver`/`driverquery`/`gpresult`로 시스템 상태를 조회하고, `mode`/`wmic`/`date`-`time`/`chcp`/`color`로 장치·환경을 구성하는 명령을 모은다. 지금까지 배운 조작·관리 능력을 바탕으로 "이 시스템이 지금 어떤 상태인가"를 진단하는 시야로 확장하는 단계다. 이 Part에 이르면 개별 파일·프로세스 단위의 조작에서 시스템 전체의 구성 정보로 관심사가 한 단계 더 넓어진다는 점에서, 8부(네트워크)로 넘어가기 직전의 자연스러운 디딤돌 역할을 한다.
...
이 9개 Part를 "도입"에서 제시한 세 가지 실무 시나리오에 다시 대응시켜 보면 순서의 의도가 더 뚜렷해진다. 레거시 배치 스크립트를 읽고 고치는 역량은 1–4부(탐색·파일 조작·텍스트 검색·배치 스크립팅)가 직접 책임진다 — 스크립트 한 줄 한 줄이 결국 이 네 Part의 명령 조합이기 때문이다. GUI 없는 서버를 원격으로 관리하는 역량은 5–7부(디스크·프로세스/권한·시스템 정보)가 맡는데, 세 Part 모두 "지금 이 시스템이 어떤 상태인가"를 화면 없이 텍스트만으로 파악하는 능력을 요구한다는 공통점이 있다. PowerShell과의 상호운용은 특정 Part 하나가 아니라 전 챕터의 "이식성" 절에 걸쳐 분산되어 있는데, 이는 상호운용이 "언제 전환할지"의 문제가 아니라 "지금 실행 중인 명령이 어느 계열인지 매 순간 구분하는" 습관의 문제이기 때문이다. 8–9부(네트워크·부팅)는 앞의 여섯 Part로 로컬 문제를 먼저 좁혀낼 수 있어야 비로소 의미가 생기는 확장 영역이라, 의도적으로 커리큘럼의 끝에 배치했다.

## 선수 지식

이 과정을 시작하는 데 필요한 사전 지식은 많지 않다. Windows에서 명령 프롬프트(`cmd.exe`)를 열고 텍스트로 명령을 입력하는 것에 거부감이 없으면 충분하며, 프로그래밍 경험은 필수가 아니다. 다만 4부(배치 스크립팅)에 들어가면 변수·조건문·반복문 같은 프로그래밍의 기본 개념을 다루므로, 다른 언어나 셸(Bash, PowerShell 등)에서 `if`/`for`를 써본 경험이 있으면 그 구간을 훨씬 빠르게 소화할 수 있다. 관리자 권한이 필요한 명령(`diskpart`, `format` 등)을 다루는 5부부터는 실습용 가상 머신이나 스냅샷을 되돌릴 수 있는 환경을 준비해두는 편이 안전하다 — 이 컬렉션의 예제 대부분은 읽는 것만으로도 이해되도록 설계했지만, 직접 실행해보며 익히는 쪽이 체화 속도가 훨씬 빠르기 때문이다.
...
이 역량들은 85챕터로 완결되지 않는다는 점도 분명히 해둘 필요가 있다. 앞서 "이 장이 다루지 않는 것"에서 밝힌 `reg`·`netsh`·`net use` 같은 명령이 실제 업무에서 필요해지는 순간은 반드시 온다. 그때 이 컬렉션이 주는 것은 그 명령의 사용법 자체가 아니라, 처음 보는 CMD 명령을 만났을 때 내부·외부 명령어 구분(핵심 개념 절)과 `help`(02장)·`<명령어> /?` 패턴만으로 두려움 없이 문법을 스스로 파악하는 습관이다. 85개 명령어를 암기하는 것이 아니라 86번째 명령어를 만났을 때도 당황하지 않는 것, 그것이 이 커리큘럼이 실제로 목표하는 완주 기준이다.

## 다음 장에서는

다음은 [01장: cmd](/post/cmd/cmd-command-interpreter-new-instance-windows/) — `cmd` 명령 자체와 내부·외부 명령어 구분을 시작으로 CMD 기초와 탐색(1부)을 다룬다. 전체 목차는 [84장: print](/post/cmd/print-command-send-file-printer-windows-cmd/)까지 위 "커리큘럼 전체 구성" 표를 참고한다.
