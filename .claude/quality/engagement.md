...
| 2026-09-07 | utterances(gh api) | — | 야간 자율 실행(스케줄 태스크), 대화형 세션 아님. GSC·GA4는 API 자격 여전히 미설정(`~/.config/gcp/` 없음) + Chrome 확장 미연결(`list_connected_browsers` 결과 빈 배열) — **미수집**. utterances는 `gh api`로 조회, 이슈 4건 모두 지난 스냅샷(2026-08-31)과 댓글·반응 수 동일 — 신규 변동 없음. 아래 GSC·GA4 표는 갱신할 데이터가 없어 유지, 백로그도 근거 데이터가 없어 갱신 없음. 참고: 지난 스냅샷 이후 Git 컬렉션(45챕터)·Tmux 컬렉션(22챕터)이 새로 게시됨(PR #118, #119) — 신규 색인·검색 유입은 다음 GSC 수집 재개 시 확인 필요 |
| 2026-09-14 | utterances(gh api) | — | 야간 자율 실행(스케줄 태스크), 대화형 세션 아님. GSC·GA4는 API 자격 여전히 미설정(`~/.config/gcp/` 없음) + Chrome 확장 미연결(`list_connected_browsers` 결과 빈 배열) — **미수집**. utterances는 `gh api`로 조회, 이슈 4건 모두 지난 스냅샷(2026-09-07)과 댓글·반응 수 동일 — 신규 변동 없음. 아래 GSC·GA4 표는 갱신할 데이터가 없어 유지, 백로그도 근거 데이터가 없어 갱신 없음. 참고: 지난 스냅샷 이후 optimization 컬렉션 2챕터(profiling 00, cpp-language 01)·zsh SIGINT 버그 글·Claude Code 세션 위생 글·프레데터 영화 리뷰가 새로 게시됨 — 신규 색인·검색 유입은 다음 GSC 수집 재개 시 확인 필요 |
