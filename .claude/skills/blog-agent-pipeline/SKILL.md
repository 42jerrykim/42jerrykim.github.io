...
description: >-
  Hugo 블로그 글을 끝까지 작성하는 멀티스테이지 파이프라인. Research(자료·링크 검증) →
  Draft(blog-post-writing 준수) → QA(메타·Mermaid·링크) → PublishPrep(draft 해제 전 최종 확인) 순으로
  한 세션에서 순차 실행한다. **한 세션에서 전체 사이클을 자동 진행**할 때 사용한다. 단계별 작성 규칙 자체가
  필요하면 `blog-post-writing`을 참고한다.
---

# 블로그 작성 에이전트 파이프라인

이 스킬은 **한 세션**에서 아래 네 단계를 **순서대로** 수행하도록 지시한다. 각 단계가 끝날 때마다 짧은 **단계 요약**(산출물, 미해결 이슈, 다음 단계 입력)을 사용자에게 보고한다.

> **관계**: 전역 규칙은 [`rules-that-must-be-followed`](../rules-that-must-be-followed/SKILL.md)가 우선한다. 초안 구조·경로·태그 전략은 [`blog-post-writing`](../blog-post-writing/SKILL.md) 및 [`reference.md`](../blog-post-writing/reference.md)를 따른다.

---

## 시작 전 함께 호출할 스킬 (매 파이프라인 실행 시)

작업을 시작하기 **전에** 아래 스킬/파일을 확인한다 (이미 세션에 있으면 생략 가능).
...
| 대상 포스트가 속한 컬렉션 | 아래 컬렉션별 스킬 표 참고 | 컬렉션 전용 규칙 |

**컬렉션별 작성 스킬 (`content/collection/<컬렉션>/` 글일 때)**
...
1. **검증된 링크 목록**: 각 URL에 대해 HTTP(또는 WebFetch 등) 접근 가능함을 확인한 것만 포함. 404·5xx·차단 시 제외하거나 대체 URL 명시.
2. **작성 메모**: 목차 초안, 반드시 넣을 섹션, 인용·수치 출처, 피해야 할 주장.
3. **경로 결정**: `content/post/<연도>/` vs `content/collection/<컬렉션>/<연도>/`, 폴더명 `YYYY-MM-DD-<slug>` (컬렉션별 예외는 해당 스킬을 따름).

**체크리스트**

- [ ] 날짜는 추측하지 않음 — 이후 Draft에서 터미널로 `Get-Date -Format "yyyy-MM-dd"` 확인 예정임을 메모에 명시.
- [ ] 외부 링크는 **검증 후** 목록에만 포함.
- [ ] Movies/TV-Show 등은 해당 컬렉션 스킬에서 요구하는 조사 항목(출처 수 등)을 메모에 반영.
...
**교육 시리즈·특수 컬렉션**: `educational-content-writing`, 컬렉션별 작성 스킬(위 표) 등 적용 대상 스킬을 이 단계에서 함께 호출한다.
...
4. 적용 대상 컬렉션 스킬의 체크리스트(영화 리뷰 Act 구조, Vocabulary 섹션 등)가 있으면 전부 대조.
5. **안티패딩 디덤프 패스**(교육 글): 닫는 절을 종류별 1개로 정리하고, 중복 요약·답 없는 번호 리스트·작성자용 자기인증("게시 전 자가 점검" 등)·존재하지 않는 파일 참조를 **삭제**한다 ([`educational-content-writing`](../educational-content-writing/SKILL.md) §2.5).
6. **코드·수치 점검**: 핵심 코드 블록이 컴파일 가능한지(주석뿐·미정의 타입 아님), "정량적"·"측정"을 표방한 글에 실제 수치 또는 실행 가능한 벤치가 있는지 확인한다 ([`educational-content-writing`](../educational-content-writing/SKILL.md) §0/§4.4).

**체크리스트**

- [ ] Phase 5 항목 전부 통과
- [ ] 컬렉션 전용 스킬 체크리스트 통과
- [ ] 교육 글인 경우 `educational-content-writing` 구조·**안티패딩**·**코드 컴파일 가능**·**수치 근거** 요구 충족
...
4. Stage 3(QA) 이후에도 점수 기반으로 반복 개선이 필요하면(임계값 도달까지 자동 재작성), 4단계 직선 진행 대신 [`post-quality-loop`](../post-quality-loop/SKILL.md)로 전환할 수 있다.
