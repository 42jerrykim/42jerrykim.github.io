...
- **(b) 기존 게시글**: [`scoreboard.md`](../../quality/scoreboard.md)에서 대상을 고른다. **야간 자율 모드는 항상 (b)**로 동작하며, 이 스킬은 **00 챕터(컬렉션 커리큘럼/개요 글) 중심으로 동작**하므로 다음 우선순위로 큐를 소진한다.
  ...
  선정에 앞서 각 행의 유효성을 판별한다(**만료 판정**, `scoreboard.md` 상단 설명과 동일한 기준): 행의 `루브릭버전`이 [`rubric.md`](rubric.md)의 현재 버전과 다르거나, `채점일`로부터 365일이 지났으면 그 행은 `상태`가 `통과`/`에스컬레이션`이어도 **선정 목적으로는 `미채점`과 동일하게 취급**한다(표의 `상태` 값 자체는 재채점 전까지 그대로 둔다). 이 판별을 마친 뒤 아래 우선순위로 큐를 소진한다.
  ...
  3. 미채점(만료 포함)이 없으면 최저 점수 글로 넘어간다(00 챕터든 일반 챕터든 동일하게).
...
- **시딩(최초 1회)**: 스코어보드 본문이 비어 있으면, `Glob`으로 `content/post/**/index.md`와 `content/collection/**/index.md`(및 `**/*.md`)를 수집해, `draft: false`이거나 `draft` 필드가 없는 **게시 상태** 글을 `상태: 미채점`(`루브릭버전: -`)으로 등록한다. 초안(`draft: true`)은 시딩 대상이 아니다(작성 중이므로 신규 초안 모드로 다룬다). 표 스키마에는 00 챕터 여부를 별도 컬럼으로 두지 않는다 — frontmatter `collection_order: 0`으로 선정 시점에 즉시 판별 가능하기 때문이다.
