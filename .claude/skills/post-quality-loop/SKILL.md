...
```mermaid
flowchart TD
    A["0. Select: 대상 글 경로 확정<br/>(00 챕터 우선)"] --> B["1. Score: quality-critic 호출<br/>루브릭 채점 + 결함목록"]
...
- **(b) 기존 게시글**: [`scoreboard.md`](../../quality/scoreboard.md)에서 대상을 고른다. **야간 자율 모드는 항상 (b)**로 동작하며, 이 스킬은 **00 챕터(컬렉션 커리큘럼/개요 글) 중심으로 동작**하므로 다음 우선순위로 큐를 소진한다.
  1. `상태: 미채점`이고 frontmatter `collection_order: 0`(00 챕터)인 글을 먼저 채점한다(선정 시 대상 글의 frontmatter를 읽어 판별 — 스코어보드 스키마 자체에는 별도 컬럼을 두지 않는다).
  2. 00 챕터가 모두 `통과` 또는 `에스컬레이션` 상태면, 나머지 `상태: 미채점` 글로 넘어간다.
  3. 미채점이 없으면 최저 점수 글로 넘어간다(00 챕터든 일반 챕터든 동일하게).
...
- **시딩(최초 1회)**: 스코어보드 본문이 비어 있으면, `Glob`으로 `content/post/**/index.md`와 `content/collection/**/index.md`(및 `**/*.md`)를 수집해, `draft: false`이거나 `draft` 필드가 없는 **게시 상태** 글을 `상태: 미채점`으로 등록한다. 초안(`draft: true`)은 시딩 대상이 아니다(작성 중이므로 신규 초안 모드로 다룬다). 표 스키마에는 00 챕터 여부를 별도 컬럼으로 두지 않는다 — frontmatter `collection_order: 0`으로 선정 시점에 즉시 판별 가능하기 때문이다.
- 자율 모드는 **00 챕터(`collection_order: 0`) 미채점 → 나머지 미채점 → 최저 `통과/에스컬레이션`** 순으로 큐를 소진한다(Stage 0 우선순위와 동일). 신규 초안은 루프 종료 시 행을 추가한다.
