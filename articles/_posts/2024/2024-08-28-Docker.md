...
##  Containerization has revolutionized the software industry, but using them
blindly for everything without considering their drawbacks or alternatives
leads to poor outcomes.

#  Why we use containers

Once upon a time, not so long ago, the techno-sphere was divided into two
hemispheres: development (Dev) and operations (Ops). The job of the people in
Dev was to hack together an application. When they were done, they threw it
over the wall to Ops who had to figure out how to deploy it and keep it
running in production. This way of working was a nightmare on many levels,
which is well described in the fiction book [ The Phoenix Project
](https://itrevolution.com/product/the-phoenix-project/) by Kim, Behr and
Spafford. The technical reasons boil down to:
...
<!--
컨테이너는 Linux, Windows, Mac OS, 가상 머신, 베어메탈, 개발자 PC, 데이터 센터, 온프레미스 환경, 퍼블릭 클라우드
등 사실상 어느 환경에서나 구동되므로 개발 및 배포가 크게 쉬워집니다.
...
도커는 Containerfile이란 메니페스트를 만들고, 이미지를 공유할 수 있는 Container Hub를 만들면서, Container
기술은 급속히 발전하게 됩니다.

Google에서 여러 머신에 걸쳐 사용되는 컨테이너를 통합하여 오케스트레이션하는 쿠버네티스라는 프로젝트를 오픈하면서 급속히 발전하게
되었습니다.
...
이외에도 rht, OCI, CRI-O 등 표준 기술들이 발전하였고, 레드햇은 Kubernetes 기반으로 OpenShift를 개발하고
있습니다.
...
<!--
지난 2000년 Unix OS인 FreeBSD에서 OS 가상화 기능인 FreeBSD Jail을 발표합니다. FreeBSD Jail은
chroot처럼 파일 액세스를 제어하는 것이 아니라 호스트 OS와 Jail이라는 OS 가상화 환경에서 파일 시스템, 프로세스, 네트워크를
분리 할 수 있는 획기적인 기술을 제공했습니다. 이것이 컨테이너의 시작입니다.

Jail을 통해 사용자 별로 환경을 분리하여 안정성을 확보하여 임대 서버나 호스팅 등의 서비스에 활용하였습니다.

다음 해인 2001년에는 Linux에서도 커널에 Linux-Vserver라는 기능을 추가하여 OS 가상화 환경을 이용할 수 있게 되었습니다.
또한 2004년에는 상용 Unix OS인 Solaris에서도 Solraris Zone이라는 OS 가상화 기능이 제공됩니다.

2006년, Google은 Process Containers는 프로세스 자원 이용량을 제어하는 기능을 발표합니다. 이듬해에 이 기능은
cgroup으로 이름을 바꿉니다.

2008년에는 Red Hat에서 논리적으로 시스템 자원을 분할하는 Namespace를 발표하고, 비슷한 시기에 IBM에서 LXC(LinuX
Containers)를 발표합니다. LXC가 cgroup과 Namespace를 사용하여 구현한 최초의 Linux 컨테이너 엔진입니다.

LXC는 이후 컨테이너 엔진의 형태를 갖추게 되었습니다. 바로 현재 컨테이너 기술의 시초라고 할 수 있습니다.

상용 제품으로는 Sun Microsystem(현재 Oracle)의 UNIX 계열 OS인 Solraris 10에 탑재된 Zone이라는 기능에
의해 처음으로 구현되었습니다.

그리고 이 기술은 Parallels의 “Parallels Virtuozzo Containers”라는 형태로 x86용 Linux와
Windows로 전파됩니다.
...
](/products/container-runtime) . Available for both Linux and Windows-based
applications, containerized software will always run the same, regardless of
the infrastructure. Containers isolate software from its environment and
ensure that it works uniformly despite differences for instance between
...
<!--
ì§€ë‚œ í�¬ìŠ¤íŒ…ì—�ì„œ ìš°ë¦¬ëŠ” ì»¨í…Œì�´ë„ˆ ëŒ€í‘œ ê¸°ìˆ ì�€ Dockerì™€ ë
ˆë“œí–‡ì�˜ ìƒˆë¡œìš´ ì»¨í…Œì�´ë„ˆ ê¸°ìˆ ì�¸ Podmanì—� ëŒ€í•´
ì•Œì•„ë³´ì•˜ìŠµë‹ˆë‹¤. ì�´ë²ˆì—�ëŠ” ì–¸ì œë¶€í„° ì»¨í…Œì�´ë„ˆ ê¸°ìˆ ì�´
ì‚¬ìš©ë�˜ì—ˆê³ , ì–´ë–»ê²Œ ë°œì „í•´ì„œ ì§€ê¸ˆì�˜ podman ì�´ë�¼ëŠ”
ì»¨í…Œì�´ë„ˆ ê¸°ìˆ ê¹Œì§€ ë‚˜ì™”ëŠ”ì§€ë¥¼ ì•Œì•„ë³´ë�„ë¡� í•˜ê² ìŠµë‹ˆë‹¤.

ì•„ë�˜ ê·¸ë¦¼ì�€ ì»¨í…Œì�´ë„ˆ ê¸°ìˆ ì�´ ì–´ë–»ê²Œ ë°œì „ë�˜ì–´ ì™”ëŠ”ì§€ë¥¼
ë³´ì—¬ì£¼ëŠ” ì»¨í…Œì�´ë„ˆ ê¸°ìˆ í�ˆìŠ¤í† ë¦¬ì�…ë‹ˆë‹¤.

![](https://blog.kakaocdn.net/dn/bzujNT/btqMxo0XfTT/vz6uyFblT39PiNRlOFiQS0/img.png)

ì»¨í…Œì�´ë„ˆ ê¸°ìˆ ì�€ 2000ë…„ë�„ì—� ì˜¤í”ˆì†ŒìŠ¤ Unixì�¸ FreeBSD 4.0ì�„
í†µí•´ ë�„ì�…ë�œ Jailì—�ì„œ ì‹œì�‘ë�˜ì—ˆìŠµë‹ˆë‹¤.

Jailì�˜ í•œê¸€ ì�˜ë¯¸ë¥¼ ë³´ë©´ ê·¸ë¦¼ê³¼ ê°™ì�´ ê°�ì˜¥ì�…ë‹ˆë‹¤. ì‰½ê²Œ
ë§�í•´ì„œ êµ�ë�„ì†Œì—�ëŠ” ê°�ì˜¥ì�´ ì—¬ëŸ¬ê°œê°€ ì�ˆê³ , ê·¸ ì•ˆì—�
ë²”ì£„ì��ë“¤ì�´ êµ¬ì†�ì�´ ë�©ë‹ˆë‹¤. íŠ¹ì • ê°�ì˜¥ ì•ˆì�˜ ë²”ì£„ì��ëŠ” ë‹¤ë¥¸
ê°�ì˜¥ì—� ì�ˆëŠ” ë²”ì£„ì��ì™€ ì†Œí†µì�„ í• ìˆ˜ ì—†ìŠµë‹ˆë‹¤. ì�´ì²˜ëŸ¼ FreeBSD
Unix ì‹œìŠ¤í…œì�„ Jailì�´ë�¼ê³ ë¶€ë¥´ëŠ” ë�…ë¦½ë�œ ì�‘ì�€ êµ¬íš�ì—� ë„£ì–´
ì‹œìŠ¤í…œì�„ ë¶„í• í•˜ì—¬ ì‚¬ìš©í• ìˆ˜ ì�ˆëŠ” ê²ƒì�´ íŠ¹ì§•ìœ¼ë¡œ Jail
ì•ˆì—�ì„œ ì‹¤í–‰ë�˜ëŠ” í”„ë¡œì„¸ìŠ¤ëŠ” Jail ë°–ì�˜ í”„ë¡œì„¸ìŠ¤ì—� ëŒ€í•´
ì˜�í–¥ì�„ ì¤„ ìˆ˜ ì—†ë‹¤ëŠ” íŠ¹ì§•ì�´ ì�ˆìŠµë‹ˆë‹¤.

ê·¸ ì�´í›„ì—�ëŠ” ì�´ì™€ ë¹„ìŠ·í•œ ì»¨í…Œì�´ë„ˆ ê°œë…�ë“¤ì�€ ê³„ì†�í•´ì„œ
ì‚¬ìš©ë�˜ê³ ê°œë°œë�˜ì–´ ì™”ìŠµë‹ˆë‹¤.

ê·¸ëŸ¬ë‹¤ê°€, 2008ë…„ 8ì›”ì—� ì²˜ì�Œ ë¦´ë¦¬ì¦ˆ ë�œ LXC(Linux Containers)ëŠ”
ìµœì´ˆë¡œ ê°€ì�¥ ì™„ë²½í•œ ë¦¬ëˆ…ìŠ¤ ì»¨í…Œì�´ë„ˆ ê´€ë¦¬ì��ë¥¼
êµ¬í˜„í–ˆìŠµë‹ˆë‹¤. ì™œëƒ�í•˜ë©´, ë‹¨ì�¼ ì»¨íŠ¸ë¡¤ í˜¸ìŠ¤íŠ¸ ìƒ�ì—�ì„œ
cgroupê³¼ namespaceë¥¼ ì‚¬ìš©í•˜ì—¬ ì—¬ëŸ¬ ê°œì�˜ ë�…ë¦½ë�œ ì»¨í…Œì�´ë„ˆë“¤ì�„
ì‹¤í–‰í• ìˆ˜ ì�ˆë�„ë¡� ìš´ì˜� ì‹œìŠ¤í…œ ë ˆë²¨ì�˜ ê°€ìƒ�í™”ë¥¼ í–ˆê¸°
ë•Œë¬¸ì�…ë‹ˆë‹¤.

ë§�ì�€ ì‚¬ë�Œë“¤ì�´ ì•Œê³ ì�ˆëŠ” DockerëŠ” 2013ë…„ 3ì›”ì—� Docker Inc(êµ¬
DotCloud)ë¥¼ í†µí•´ ë¦´ë¦¬ì¦ˆë�œ ì»¨í…Œì�´ë„ˆ  ê¸°ìˆ ì�…ë‹ˆë‹¤. DockerëŠ”
ê¸°ì¡´ ì»¨í…Œì�´ë„ˆ ê¸°ìˆ ê³¼ëŠ” ë‹¤ë¥´ê²Œ ìƒˆë¡œìš´ ê³„ì¸µí™”ë�œ ì�´ë¯¸ì§€ë¥¼
ì‹¤í–‰í•˜ê³ êµ¬ì¶•í•˜ëŠ” ê°„í�¸í•œ CLIì™€ ì„œë²„ ë�°ëª¬, ì‚¬ì „ êµ¬ì¶•ë�œ
ì»¨í…Œì�´ë„ˆ ì�´ë¯¸ì§€ ë�¼ì�´ë¸ŒëŸ¬ë¦¬ ë°� ë ˆì§€ìŠ¤íŠ¸ë¦¬ ì„œë²„ì�˜ ê°œë…�
ë“±ì�„ ì„ ë³´ì˜€ìŠµë‹ˆë‹¤.

![](https://blog.kakaocdn.net/dn/byTgTE/btqMtzJm82N/IWeFSFxwd093CswSiZhYa1/img.png)

Dockerê°€ ë‚˜ì˜¨ì§€ ì–¼ë§ˆ ì•ˆë�˜ 2014ë…„ 6ì›”ì—� Googleì�€ 15ë…„ê°„ì�˜
ë…¸í•˜ìš°ê°€ ë‹´ê¸´ Kubernetesë¥¼ ì˜¤í”ˆ ì†ŒìŠ¤í™” í•˜ì˜€ìŠµë‹ˆë‹¤.
KubernetesëŠ” ì»¨í…Œì�´ë„ˆí™” ë�œ ì�‘ìš© í”„ë¡œê·¸ë�¨ì�˜ ë°°í�¬, í™•ì�¥ ë°�
ê´€ë¦¬ë¥¼ ì��ë�™í™”í•˜ê¸° ìœ„í•œ ì˜¤í”ˆ ì†ŒìŠ¤ ì‹œìŠ¤í…œìœ¼ë¡œ, ì�‘ìš©
í”„ë¡œê·¸ë�¨ì�„ êµ¬ì„±í•˜ëŠ” ì»¨í…Œì�´ë„ˆë¥¼ ë…¼ë¦¬ ë‹¨ìœ„ë¡œ ê·¸ë£¹í™”í•˜ì—¬
ì‰½ê²Œ ê´€ë¦¬í•˜ê³ ê²€ìƒ‰ í• ìˆ˜ ì�ˆìŠµë‹ˆë‹¤.

![](https://blog.kakaocdn.net/dn/bNSP1X/btqMqA95HbB/2SLFcdiWOP2qgSf43wxev0/img.png)

Kubernetesê°€ ë‚˜ì˜¤ë©´ì„œ ì»¨í…Œì�´ë„ˆ ì‹œì�¥ì�€ ê¸‰ì†�ë�„ë¡œ ë°œì
„í–ˆìŠµë‹ˆë‹¤. ê·¸ëŸ¬ë©´ì„œ 2015ë…„ 7ì›”ì—� OCI(Open Container
Initiative)ë�¼ëŠ” ë¦¬ëˆ…ìŠ¤ ì»¨í…Œì�´ë„ˆì—� ëŒ€í•œ ê³µê°œ í‘œì¤€ì�„
ì„¤ê³„í•˜ê¸° ìœ„í•´ Dockerì™€ ì»¨í…Œì�´ë„ˆ ê¸°ìˆ ì�„ ë¦¬ë“œí•˜ëŠ” ì—¬ëŸ¬
íšŒì‚¬ ë°� ì»¤ë®¤ë‹ˆí‹°ê°€ í•¨ê»˜ ì„¤ë¦½í•œ í”„ë¡œì �íŠ¸ë¡œ Linux
Foundationì—�ì„œ ì‹œì�‘ë�˜ì—ˆìŠµë‹ˆë‹¤.

![](https://blog.kakaocdn.net/dn/bwsU8S/btqMwmI4OQQ/CfoQ5DkYTKDrJxshRDBc8k/img.png)

OCIì—�ì„œëŠ” ì»¨í…Œì�´ë„ˆ ëŸ°íƒ€ì�„ ì‚¬ì–‘ê³¼ ì�´ë¯¸ì§€ ì‚¬ì–‘ì�„
í‘œì¤€í™”í•˜ê³ êµ¬í˜„í•˜ì˜€ìŠµë‹ˆë‹¤. OCIê°€ ì„¤ë¦½ë�˜ë©´ì„œ Runtime spec,
Runtime tools, Runc, Image spec, Image toolsì™€ ê°™ì�€ ì„¸ë¶„í™”ë�œ ê¸°ìˆ
í‘œì¤€ì�„ ë§Œë“¤ì–´ ê°”ìŠµë‹ˆë‹¤. Apache MesosëŠ” OCI Image specì�„, DockerëŠ”
OCI Image Layoutì�˜ í‘œì¤€ ë°� êµ¬í˜„ì—� ì°¸ì—¬í•˜ì˜€ìŠµë‹ˆë‹¤. CoreOSëŠ”
rkt(rocket)ë�¼ ë¶ˆë¦¬ëŠ” OCI container ë°� podì—� ëŒ€í•œ runtime í‘œì¤€ ë°�
êµ¬í˜„ì—� ì°¸ì—¬í•˜ì˜€ìœ¼ë©°, Cloud Foundryì—�ì„œ OCI container managerë¥¼
ì§€ì›�í•˜ì˜€ìŠµë‹ˆë‹¤.

CNCF(Cloud Native Computing Foundation) ëŠ” 2015ë…„ 11ì›”ì—� ì„¤ë¦½ë�œ
ë‹¨ì²´ë¡œ Kubernetes ê¸°ìˆ ì�„ ì¤‘ì‹¬ìœ¼ë¡œ ë‹¤ì–‘í•œ ê¸°ìˆ ë°œì „ê³¼
ì»¤ë®¤ë‹ˆí‹° í™œë�™ì�„ ì�¥ë ¤í•˜ë©°, OCIë¥¼ í†µí•´ Kubernetes í‘œì¤€í™”
ì—…ë¬´ì—� ì°¸ì—¬í•˜ì˜€ìŠµë‹ˆë‹¤.

ì�´ëŸ° í”„ë¡œì �íŠ¸ ë°� ë‹¨ì²´ë¥¼ í†µí•´ ì»¨í…Œì�´ë„ˆ ì�´ë¯¸ì§€ ê¸°ìˆ ê³¼
ëŸ°íƒ€ì�„ ê¸°ìˆ ì�´ ë°œì „í•˜ë©´ì„œ 2016ë…„ 1ì›”ì—�ëŠ” OCI ì»¨í…Œì�´ë„ˆ
ì�´ë¯¸ì§€ì™€ Docker V2 ì�´ë¯¸ì§€ì™€ í˜¸í™˜ë�˜ëŠ” Skopeoê°€ ë‚˜ì™”ìŠµë‹ˆë‹¤.
SkopeoëŠ” ì»¨í…Œì�´ë„ˆ ì�´ë¯¸ì§€ ë°� ì�´ë¯¸ì§€ ë¦¬í�¬ì§€í† ë¦¬ì—�ì„œ ë‹¤ì–‘í•œ
ì�‘ì—…ì�„ ìˆ˜í–‰í•˜ëŠ” ëª…ë ¹ ì¤„ ìœ í‹¸ë¦¬í‹°ì�…ë‹ˆë‹¤.

![](https://blog.kakaocdn.net/dn/cgVpch/btqMqCfNJb0/YYo8WklJkiNuCYdzyWr3bk/img.png)

CNCFëŠ” 2017ë…„ 3ì›” OCIì™€ í•¨ê»˜ Kubernetes container runtime í‘œì¤€í™”
ì—…ë¬´ë¥¼ ìœ„í•´ CRI-O(Container Runtime Interface using OCI) í”„ë¡œì �íŠ¸ë¥¼
ìƒ�ì„±í•©ë‹ˆë‹¤. CRI-OëŠ” OCI í˜¸í™˜ ëŸ°íƒ€ì�„ê³¼ kubelet ê°„ì�˜ í†µí•©
ê²½ë¡œë¥¼ ì œê³µí•˜ê¸°ìœ„í•œ ê²ƒìœ¼ë¡œ, OCI í˜¸í™˜ ëŸ°íƒ€ì�„ì�„ ì‚¬ìš©í•˜ì—¬
Kubelet ì»¨í…Œì�´ë„ˆ ëŸ°íƒ€ì�„ ì�¸í„°í�˜ì�´ìŠ¤ (CRI)ë¥¼ êµ¬í˜„í•©ë‹ˆë‹¤.

BuildahëŠ” 2017ë…„ 6ì›”ì—� ë¦´ë¦¬ì¦ˆë�˜ì—ˆìœ¼ë©°, OCI ì»¨í…Œì�´ë„ˆ
ì�´ë¯¸ì§€ë¥¼ ì‰½ê²Œ ìƒ�ì„±í•˜ê¸° ìœ„í•œ íˆ´ì�…ë‹ˆë‹¤. 2018ë…„ 2ì›”ì—�
ë¦´ë¦¬ì¦ˆí•œ Podmanì�€ í�¬ë“œ(Pod)ì—�ì„œ OCI ê¸°ë°˜ ì»¨í…Œì�´ë„ˆë¥¼
ì‹¤í–‰í•˜ê¸°ìœ„í•œ ë�¼ì�´ë¸ŒëŸ¬ë¦¬ ë°� ë�„êµ¬ì�…ë‹ˆë‹¤. Kubernetesê°€
ëŒ€ì¤‘í™” ë�˜ë©´ì„œ Container Pod ê°œë…�ì�„ ì‚¬ìš©í•˜ëŠ” ì�‘ìš©
í”„ë¡œê·¸ë�¨ì�„ ìœ„í•œ ë�¼ì�´ë¸ŒëŸ¬ë¦¬ë¥¼ ì œê³µí•©ë‹ˆë‹¤. Podmanì�€ í�¬ë“œ,
ì»¨í…Œì�´ë„ˆ, ì»¨í…Œì�´ë„ˆì�´ë¯¸ì§€ ë°� ì»¨í…Œì�´ë„ˆë³¼ë¥¨ì�„ ê´€ë¦¬í•©ë‹ˆë‹¤.

![](https://blog.kakaocdn.net/dn/93cyI/btqMtx5VMWI/zZZcPmcc3Ozrpr0pAqimJk/img.png)

RedHatì�€ OCI(Open Container Initiative)ì�˜ Platinum ë©¤ë²„ë¡œì„œ ë‹¤ì–‘í•œ
ì»¨í…Œì�´ë„ˆ í‘œì¤€í™” ë°� êµ¬í˜„ì—� ì°¸ì—¬í•˜ì˜€ìœ¼ë©°, 2017ë…„ 3ì›”
Dockerì—�ì„œ Enterprise Editionì�„ ë¦´ë¦¬ì¦ˆí•˜ë©´ì„œ 2018ë…„ 1ì›”ì—� OCI
ë©¤ë²„ ì¤‘ í•˜ë‚˜ì˜€ë�˜ CoreOSë¥¼ ì�¸ìˆ˜í•˜ì˜€ìŠµë‹ˆë‹¤. ê·¸ë¦¬ê³ , CoreOSì™€
í•¨ê»˜ Podmanë¥¼ ì¶œì‹œí•˜ë©´ì„œ, CoreOSì�˜ í•µì‹¬ ì»¨í…Œì�´ë„ˆ ê¸°ìˆ ì�¸
Podman, Skopeo, Buildahë¥¼ í�¬í•¨í•˜ì—¬ 2019ë…„ 4ì›”ì—� Red Hat Enterprise
Linux 8.0ì�„ ë¦´ë¦¬ì¦ˆí•˜ì˜€ìŠµë‹ˆë‹¤. ê·¸ ì�´í›„ë¡œ Rhel 8ì�„ ê¸°ë°˜ìœ¼ë¡œ
í•œ Red Hat OpenShift Container Platform 4, Red Hat OpenStack Platform 16ì�€
Podmanì�´ Dockerë¥¼ ëŒ€ì‹ í•˜ê²Œ ë�˜ì—ˆìŠµë‹ˆë‹¤.

ì�´ë ‡ê²Œ í•´ì„œ ì»¨í…Œì�´ë„ˆ ì—­ì‚¬ë¥¼ ì•Œì•„ë³´ì•˜ìŠµë‹ˆë‹¤. ì»¨í…Œì�´ë„ˆ
ë°� í�´ë�¼ìš°ë“œ ê¸°ìˆ ì�„ ì ‘í•˜ë©´ì„œ ë§�ì�´ ë“£ë�˜ ìš©ì–´ë“¤ì�´ ëª¨ë‘�
ì»¨í…Œì�´ë„ˆ ê¸°ìˆ ì�„ í‘œì¤€í™”í•˜ê¸° ìœ„í•œ OCI(Open Container
Initiative)ë�¼ëŠ” Linux Foundation ì—�ì„œ ì‹œì�‘í•œ í”„ë¡œì �íŠ¸ë¥¼
ì‹œì�‘ìœ¼ë¡œ ëª¨ë‘� ì—°ê²°ë�˜ì–´ ì�ˆì—ˆìœ¼ë©°, ë ˆë“œí–‡ì�´ ì»¨í…Œì�´ë„ˆ ê¸°ìˆ
ì�„ ë¦¬ë“œí•˜ëŠ” ì˜¤í”ˆì†ŒìŠ¤ ê¸°ì—…ìœ¼ë¡œì�¨ ì°¸ì—¬ìœ¨ì�´ ë†’ë‹¤ëŠ”ê²ƒì�´
ì°¸ìœ¼ë¡œ ì�¸ìƒ�ì �ì�´ì—ˆìŠµë‹ˆë‹¤. ë‹¤ì�Œ í�¬ìŠ¤íŒ…ì—�ì„œëŠ” Red Hat
OpenStack Platformì�´ ì–´ë–»ê²Œ ì»¨í…Œì�´ë„ˆ ê¸°ìˆ ì�„ ì‚¬ìš©í•˜ê³
ì�ˆëŠ”ì§€ì—� ëŒ€í•´ ì•Œì•„ë³´ê² ìŠµë‹ˆë‹¤.
...
[ https://blog.aquasec.com/a-brief-history-of-containers-from-1970s-chroot-to-
docker-2016 ](https://blog.aquasec.com/a-brief-history-of-containers-
from-1970s-chroot-to-docker-2016)
...
[ https://github.com/docker-archive/containerd ](https://github.com/docker-
archive/containerd)
