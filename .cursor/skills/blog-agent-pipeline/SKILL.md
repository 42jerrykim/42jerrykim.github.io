...
5. **안티패딩 디덤프 패스**(교육 글): 닫는 절을 종류별 1개로 정리하고, 중복 요약·답 없는 번호 리스트(질문 24/메모 20 류)·작성자용 자기인증("게시 전 자가 점검", "평가 기준 재확인")·존재하지 않는 파일 참조를 **삭제**한다(educational-content-writing §2.5).
6. **코드·수치 점검**: 핵심 코드 블록이 컴파일 가능한지(주석뿐·미정의 타입 아님), "정량적"·"측정"을 표방한 글에 실제 수치 또는 실행 가능한 벤치가 있는지 확인한다(§4.4/§5.2).
